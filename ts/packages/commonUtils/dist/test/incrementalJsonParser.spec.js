...
const stringifiedTestInput = [
    ...testInput.map(([name, input]) => [name, JSON.stringify(input)]),
    ["escaped", '{"a":"a\\b\\f\\r\\t\\n\\"\\u1234\\\\\\/"}'],
    [
        "space",
...
describe("Incremental Json Parser", () => {
    describe.each(["full", "leaf"])("%s", (mode) => {
        it.each(stringifiedTestInput)("%s", (name, input) => {
            ...
            const parser = createIncrementalJsonParser(cb, mode === "full"
                ? {
                    full: true,
                }
                : undefined);
...
//# sourceMappingURL=incrementalJsonParser.spec.js.map