...
export function getElapsedString(elapsedMs, showMinutes = true) {
    const seconds = elapsedMs / 1000;
    const minutes = showMinutes ? Math.floor(seconds / 60) : 0;
    const minuteStr = minutes > 0 ? `${minutes}m ` : "";
    const secondStr = `${minutes > 0 ? Math.floor(seconds - minutes * 60) : seconds.toFixed(3)}s`;
    return `${minuteStr}${secondStr}`;
}
export function getColorElapsedString(elapsedMs) {
 ...
 */
export function printRecord(record, title, sort = false, minKeyColWidth = 16) {
    if (title) {
    ...
    function printField(name, value, paddedNameLength) {
...
export function printRecordAsHtml(record, title, sort = false, cellPadding = 5) {
    const keys = Object.keys(record);
    if (sort) {
        keys.sort();
    }
