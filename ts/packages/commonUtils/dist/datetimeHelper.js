...
import { addMilliseconds, addMonths, format, startOfMonth, startOfWeek, addDays, isSameMonth, isValid, parse, parseISO, } from "date-fns";
...
export function parseFuzzyDateString(dateString) {
...
export function getISODayStartTime(currentDate) {
    const curDay = new Date(currentDate);
    curDay.setHours(0, 0, 0, 0);
    return curDay.toISOString();
}
export function getISODayEndTime(currentDate) {
...
export function parseDuration(durationString) {
    ...
    const regex = /(\d+)\s*(?:hours?|hrs?|h)(?:\s+and\s+(\d+)\s*(?:minutes?|mins?|m))?/i;
...
export function getDateTimeUsingDuration(startDateTimeString, durationString, fLocaleTime = true) {
...
export function getDateRelativeToTodayAlt(relativeDate) {
...
export function getDateRelativeToDayV3(relativeDate) {
    const currentDate = new Date();
    const currentYear = currentDate.getFullYear();
    const currentMonth = currentDate.getMonth();
    const relativeDateRegex = /^(\d{1,2})?\s*(\w+)?\s*(of)?\s*(this|next)?\s*(\w+)?\s*(month)?$/i;
...
export function getDateRelativeToDayV2(relativeDate) {
    const daysOfWeek = [
        "sunday",
        "monday",
        "tuesday",
    ...
    const relativeDateRegex = new RegExp(`^(?:(this|next)\\s+)?(${daysOfWeekRegex})$`, "i");
    const match = relativeDateRegex.exec(relativeDate.trim());
    if (!match) {
...
export function getDateRelativeToDayV1(relativeDate) {
    const daysOfWeek = [
        "sunday",
        "monday",
        "tuesday",
...
export function getDateRelativeToToday(relativeDate) {
    if (relativeDate.toLowerCase() === "today" || relativeDate === "") {
        return new Date();
    }
    let parsedDate = getDateRelativeToDayV2(relativeDate);
...
export function formatTime(time) {
    ...
    const determinedPeriod = period || (parseInt(hourStr, 10) == 12 ? "PM" : "AM");
...
function getDateFromNLP(relativeDate) {
    let date;
...
export async function getNormalizedDateRange(inputDate, startTime, endTime, duration, fLocaleTime = true) {
    let currentDate = getShortDate(new Date(inputDate));
    if (currentDate !== undefined) {
        if (startTime != undefined) {
            let startDateTime = combineDateTime(currentDate, parseTimeString(formatTime(startTime)));
            startTime = fLocaleTime
                ? startDateTime.toLocaleString()
                : startDateTime.toISOString();
            if (endTime === undefined) {
                endTime = getDateTimeUsingDuration(startDateTime.toISOString(), duration != undefined ? duration : "1 hour", fLocaleTime);
            }
            else {
                let endDateTime = combineDateTime(currentDate, parseTimeString(formatTime(endTime)));
                endTime = fLocaleTime
                    ? endDateTime.toLocaleString()
                    : endDateTime.toISOString();
            }
...
export async function getNormalizedDateTimes(day, startTime, endTime, duration, fLocaleTime = true) {
    let curDay;
...
export function getUniqueLocalId(timestamp) {
    timestamp ??= new Date();
