...
import { AzureTokenScopes, createAzureTokenProvider, getEnvSetting, } from "aiclient";
import { openai } from "aiclient";
import { env } from "process";
// Maps token provider
const tokenProvider = createAzureTokenProvider(AzureTokenScopes.AzureMaps);
/**
 ...
 */
export function exifGPSTagToLatLong(exifLat, exifLatRef, exifLong, exifLongRef) {
    if (exifLat !== undefined &&
        exifLong !== undefined &&
        exifLatRef !== undefined &&
        exifLongRef !== undefined) {
        return {
            latitude: exifLatRef.value == "S"
                ? parseFloat("-" + exifLat.description)
                : parseFloat(exifLat.description),
            longitude: exifLongRef.value == "W"
 ...
 */
export async function findNearbyPointsOfInterest(position, settings, maxSearchRadius = 10000, minResultCount = 1) {
    ...
    try {
        ...
        const radii = [5, 10, 25, 50, 100, 1000, 10000];
        let index = 0;
        do {
...
export async function reverseGeocode(position, settings) {
    if (position === undefined) {
        return [];
    }
    try {
