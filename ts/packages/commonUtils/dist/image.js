...
export class CachedImageWithDetails {
    constructor(exifTags, storageLocation, image) {
        this.exifTags = exifTags;
        this.storageLocation = storageLocation;
        this.image = image;
    }
}
export function getImageElement(imgData) {
    return `<img class="chat-input-image" src="${imgData}" />`;
}
export function extractRelevantExifTags(exifTags) {
    let tags = "";
...
export function extractAllExifTags(exifTags) {
    let tags = "";
 ...
 */
export async function downloadImage(uri, fileName, storage) {
 ...
 */
export async function addImagePromptContent(role, image, includeFileName, includePartialExifTags, includeAllExifTags, includePOI, includeGeocodedAddress) {
    const content = [];
    const retValue = {};
    ...
    // include POI
    retValue.nearbyPOI = await findNearbyPointsOfInterest(exifGPSTagToLatLong(image.exifTags.GPSLatitude, image.exifTags.GPSLatitudeRef, image.exifTags.GPSLongitude, image.exifTags.GPSLongitudeRef), openai.apiSettingsFromEnv());
    if (includePOI !== false) {
        content.push({
            type: "text",
    ...
    // include address
    retValue.reverseGeocode = await reverseGeocode(exifGPSTagToLatLong(image.exifTags.GPSLatitude, image.exifTags.GPSLatitudeRef, image.exifTags.GPSLongitude, image.exifTags.GPSLongitudeRef), openai.apiSettingsFromEnv());
 ...
 */
export function getDateTakenFuzzy(filePath) {
    const fileName = path.basename(filePath).toLowerCase();
    let datePart = fileName.substring(0, fileName.indexOf("."));
    if (fileName.startsWith("img_")) {
        datePart = fileName.substring(5);
    }
    let retValue = new Date("1/1/1900 00:00:00");
    if (datePart.length != 15) {
        ...
        retValue.setFullYear(parseInt(datePart.substring(0, 4)), parseInt(datePart.substring(4, 6)) - 1, parseInt(datePart.substring(6, 8)));
        retValue.setHours(parseInt(datePart.substring(9, 11)), parseInt(datePart.substring(11, 13)), parseInt(datePart.substring(13, 15)));
    }
    return retValue;
}
