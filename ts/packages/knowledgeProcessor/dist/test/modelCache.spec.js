...
describe("modelCache", () => {
    ...
    testIf("cache", () => hasTestKeys(), async () => {
        const cacheSize = 3;
    ...
    testIf("cache.batch", () => hasTestKeys(), async () => {
        const cacheSize = 3;
        const embeddingModel = openai.createEmbeddingModel();
        const model = createEmbeddingCache(embeddingModel, cacheSize);
        let embeddings = getData(await model.generateEmbeddingBatch(texts.slice(1, cacheSize + 1)));
        expect(embeddings).toHaveLength(cacheSize);
        expect(model.cache.size).toEqual(cacheSize);
        embeddings = getData(await model.generateEmbeddingBatch(texts.slice(0, cacheSize + 2)));
