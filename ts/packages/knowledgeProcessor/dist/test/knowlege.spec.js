...
import { createTestModels, getRootDataPath, hasTestKeys, loadData, testIf, } from "./testCore.js";
...
describe("KnowledgeExtractor", () => {
    ...
    testIf("generate", () => hasTestKeys(), async () => {
        const context = getContext();
        const testFile = "test/data/play.txt";
        const blocks = await loadData(testFile);
        const extractor = conversation.createKnowledgeExtractor(context.models.chat);
        const knowledge = await asyncArray.mapAsync(blocks, 4, (block) => extractor.extract(block.value));
    ...
    testIf("nameTags", () => hasTestKeys(), async () => {
    ...
    async function addTags(store, items, tag) {
        await addItems(store, items);
        let itemIds = await asyncArray.mapAsync(items, 1, (item) => store.add(item));
        await store.addTag(tag, itemIds);
        let foundIds = await store.getByTag(tag);
        expect(foundIds).toBeDefined();
        expect(foundIds).toEqual(itemIds);
    }
    async function addItems(store, items) {
    ...
    async function createStore(name) {
        ...
        return await createKnowledgeStore({
            caseSensitive: false,
            semanticIndex: false,
            concurrency: 1,
...
//# sourceMappingURL=knowlege.spec.js.map