 ...
 * @returns
 */
export async function interactivelyProcessUserInput(ux, initialUserInput, value, translate, evaluate, maxAttempts = 3) {
    let previousUserInputs = [];
    let userInput = initialUserInput;
    let attempt = 1;
    while (true) {
        if (!value) {
            assert(userInput);
            const translationResult = await translate(userInput, previousUserInputs);
        ...
        // Evaluate the value
        const evaluationResult = await evaluate(userInput, previousUserInputs, value);
