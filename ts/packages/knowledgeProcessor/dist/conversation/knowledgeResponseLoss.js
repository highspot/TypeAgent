...
export async function createLocalSemanticMap(_model) {
    const map = new Map();
    return {
        setMultiple(items) {
...
function normalizeActionName(verbs) {
...
class NormalizedActions {
    ...
    mergeActionsToNorm(normAction, action2) {
        return {
            verbs: normAction.verbs,
            verbTense: normAction.verbTense,
            params: getUniqueElements(normAction.params, action2.params),
            subjectEntityNames: getUniqueElements(normAction.subjectEntityNames, [action2.subjectEntityName]),
            objectEntityNames: getUniqueElements(normAction.objectEntityNames, [
                action2.objectEntityName,
            ]),
            indirectObjectEntityNames: getUniqueElements(normAction.indirectObjectEntityNames, [action2.indirectObjectEntityName]),
            subjectEntityFacets: normAction.subjectEntityFacets?.concat(action2.subjectEntityFacet || []),
        };
    }
}
...
async function stringListLoss(refStrings, genStrings, model) {
    if (!refStrings) {
        return 0;
    }
    if (!genStrings) {
    ...
    const map = await createSemanticMap(model);
...
function entityFacetMatch(value1, value2) {
...
async function facetLoss(refFacets, genFacets, model) {
    let loss = 0;
    ...
    const genMap = await createSemanticMap(model);
...
async function entityTypeLoss(refType, genType, model) {
    return stringListLoss(refType, genType, model);
}
async function entityLoss(refEntities, generatedEntities, model) {
    ...
    const potentialLossPerEntity = potentialNameLoss + potentialTypeLoss + potentialFacetLoss;
    const potentialLossTotal = refEntities.entities.length * potentialLossPerEntity;
    const genMap = await createSemanticMap(model);
    genMap.setMultiple(generatedEntities.entities.map((entity) => [entity.name, entity]));
    let loss = 0;
        ...
        if (genEntityScored === undefined) {
            ...
            if (entity.facets && entity.facets.length > 0) {
                if (!genEntity.facets || genEntity.facets.length === 0) {
                    ...
                    const genFacetLoss = await facetLoss(entity.facets, genEntity.facets, model);
...
async function paramsLoss(refParams, genParams, model) {
    ...
    let loss = 0;
    const genMap = await createSemanticMap(model);
    genMap.setMultiple(genParams.map((param) => {
    ...
    for (const param of refParams) {
        ...
        if (genParamScored === undefined) {
            ...
            if (typeof param !== "string") {
                const genParam = genParamScored.item.valueOf();
...
async function actionsLoss(refActions, genActions, model) {
    ...
    const potentialLossPerAction = potentialVerbLoss +
    ...
    let loss = 0;
    const genMap = await createSemanticMap(model);
    genMap.setMultiple(genActions.map((action) => [normalizeActionName(action.verbs), action]));
        ...
        if (genActionScored === undefined) {
            ...
            if (action.params) {
                if (!genAction.params) {
                    ...
                    const prmsLoss = await paramsLoss(action.params, genAction.params, model);
                    loss += potentialParamLoss * prmsLoss;
                }
            }
            const subjLoss = await stringListLoss(action.subjectEntityNames, genAction.subjectEntityNames, model);
            loss += potentialSubjectLoss * subjLoss;
            const objLoss = await stringListLoss(action.objectEntityNames, genAction.objectEntityNames, model);
            loss += potentialObjectLoss * objLoss;
            const indObjLoss = await stringListLoss(action.indirectObjectEntityNames, genAction.indirectObjectEntityNames, model);
            ...
            if (action.subjectEntityFacets) {
                if (!genAction.subjectEntityFacets) {
                    ...
                    const genFacetLoss = await facetLoss(action.subjectEntityFacets, genAction.subjectEntityFacets, model);
...
class NormalizedKnowledgeResponse {
    ...
    async loss(normGenResponse, model) {
        const potentialEntityLoss = 5;
        const potentialActionLoss = 3;
        const potentialTopicLoss = 1;
        const potentialLossTotal = potentialEntityLoss + potentialActionLoss + potentialTopicLoss;
        let genEntityLoss = await entityLoss(this.entities, normGenResponse.entities, model);
        let actionLoss = await actionsLoss(this.response.actions, normGenResponse.response.actions, model);
        let topicLoss = await stringListLoss(this.response.topics, normGenResponse.response.topics, model);
        return ((genEntityLoss * potentialEntityLoss +
            actionLoss * potentialActionLoss +
            topicLoss * potentialTopicLoss) /
            potentialLossTotal);
    }
}
export function gatherTerms(knowledge) {
    const termsSet = new Set();
    const structuredTermsSet = new Set();
    function addTerm(term) {
        termsSet.add(term);
    }
    function addStructuredTerm(term) {
        addTerm(term);
...
export async function knowledgeResponseLoss(refResponse, generatedResponse, model) {
...
export async function simpleKnowledgeResponseLoss(refResponse, generatedResponse, model) {
    const potentialTermsLoss = 2;
    const potentialStructuredTermsLoss = 1;
    const potentialLossTotal = potentialTermsLoss + potentialStructuredTermsLoss;
    const refTerms = gatherTerms(refResponse);
    const genTerms = gatherTerms(generatedResponse);
    const termsLoss = await stringListLoss(refTerms.terms, genTerms.terms, model);
    const structuredTermsLoss = await stringListLoss(refTerms.structureTerms, genTerms.structureTerms, model);
    return ((termsLoss * potentialTermsLoss +
        structuredTermsLoss * potentialStructuredTermsLoss) /
        potentialLossTotal);
}
//# sourceMappingURL=knowledgeResponseLoss.js.map