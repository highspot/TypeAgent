 ...
 */
export async function createImageMemory(model, answerModel, name, rootPath, settings, storageProvider) {
    const storePath = path.join(rootPath, name);
    //settings.initializer ??= setupEmailConversation;
    const imageConversation = await createConversation(settings, storePath, undefined, undefined, storageProvider);
    ...
    }, name, rootPath, false, imageConversation);
    return cm;
}
async function setupImageConversationManager(cm, userProfile) {
 ...
 * @param emails
 */
export async function addImageToConversation(cm, images, maxCharsPerChunk, extractor) {
    const messages = [];
 ...
 * @returns
 */
export async function imageToMessage(image, extractor) {
    const kr = await extractor.extract(JSON.stringify(image));
...
export function getKnowledgeForImage(image, extractor) {
    const imageEntity = {
        name: image.fileName,
        type: ["file", "image"],
        ...
        for (let i = 0; i < image.nearbyPOI.length; i++) {
            const poiEntity = {
                ...
                facets: [
                    ...
                    {
                        name: "position",
                        value: JSON.stringify(image.nearbyPOI[i].position) ?? "",
                    },
                    {
                        name: "longitude",
                        value: image.nearbyPOI[i].position?.longitude?.toString() ?? "",
                    },
                    {
                        name: "latitude",
                        value: image.nearbyPOI[i].position?.latitude?.toString() ??
                            "",
                    },
                ],
            };
    ...
    if (image.reverseGeocode) {
        for (let i = 0; i < image.reverseGeocode.length; i++) {
            ...
            if ((i == 0 || image.reverseGeocode[i].confidence == "High") &&
                image.reverseGeocode[i].address !== undefined) {
                const addrOutput = image.reverseGeocode[i].address;
                const addrEntity = {
 ...
 */
export async function loadImage(fileName, model, loadCachedDetails = true, cacheFolder = undefined) {
    if (cacheFolder === undefined) {
        cacheFolder = path.dirname(fileName);
    }
    const cachedFileName = path.join(cacheFolder, fileName + ".eat");
    ...
    const loadedImage = new CachedImageWithDetails(tags, fileName, `data:image/${mimeType};base64,${buffer.toString("base64")}`);
    // create a caption for the image
    const caption = createTypeChat(model, imageCaptionGeneratingSchema, "generateCaption", `You are photography expert.`, [], 4096, 30);
    try {
        const prompt = [];
        const content = await addImagePromptContent("user", loadedImage, true, false, false, true, true);
        ...
        const chatResponse = await caption.translate("Caption supplied images in no less than 200 words without making any assumptions, remain factual.", prompt);
 ...
 * @returns The described image.
 */
export async function loadImageWithKnowledge(fileName, cachePath, model, loadCachedDetails = true) {
    const cachedFileName = path.join(cachePath, path.basename(fileName) + ".kr.json");
    if (loadCachedDetails && fs.existsSync(cachedFileName)) {
        return JSON.parse(fs.readFileSync(cachedFileName, "utf8"));
    }
    const buffer = fs.readFileSync(fileName);
    ...
    const mimeType = getMimeType(path.extname(fileName));
    const loadedImage = new CachedImageWithDetails(tags, fileName, `data:image/${mimeType};base64,${buffer.toString("base64")}`);
    const validator = createTypeScriptJsonValidator(imageDetailExtractionSchema, "imageDetailExtractionSchema");
    const translator = createJsonTranslator(model, validator);
    translator.createRequestPrompt = createRequestPrompt;
    function createRequestPrompt(request) {
        return (`You are a service that translates images into JSON objects of type "imageDetailExtractionSchema" according to the following TypeScript definitions::\n` +
    ...
    // );
    try {
        const content = await addImagePromptContent("user", loadedImage, false, false, false, false, false);
        //prompt.push(promptLib.dateTimePromptSection()); // Always include the current date and time. Makes the bot much smarter
        //prompt.push(content.promptSection!);
 ...
 */
export function buildImageCountHistogram(filePath, recursive = true, bucketSizeInSeconds = 300) {
    // paramter checking
    ...
    // now bucketize
    const histogram = [];
    const startDate = allImageFiles[0].dateTaken.getTime() / 1000;
    const endDate = allImageFiles[allImageFiles.length - 1].dateTaken.getTime() / 1000;
    // how many buckets do we need?
    const buckets = Math.ceil((endDate - startDate) / bucketSizeInSeconds);
