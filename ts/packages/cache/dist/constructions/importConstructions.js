...
import { fromJsonActions, getTranslationNamesForActions, RequestAction, } from "../explanation/requestAction.js";
...
function createConstructions(data, explainer, schemaInfoProvider, createConstruction, ignoreSourceHash) {
    if (schemaInfoProvider !== undefined) {
        for (let i = 0; i < data.schemaNames.length; i++) {
            ...
            if (schemaInfoProvider.getActionSchemaFileHash(schemaName) !==
                sourceHash) {
    ...
    for (const entry of data.entries) {
        const requestAction = new RequestAction(entry.request, fromJsonActions(entry.action));
        try {
            const actions = requestAction.actions;
            for (const { action } of actions) {
                if (!dataSchemaNames.has(action.translatorName)) {
                    throw new Error(`Schema name '${action.translatorName}' not found in data header`);
                }
            }
            const explanation = entry.explanation;
            const namespaceKeys = getSchemaNamespaceKeys(getTranslationNamesForActions(actions), schemaInfoProvider);
            const construction = createConstruction(requestAction, explanation, {
            ...
            if (debugConstCreate.enabled) {
                ...
                debugConstCreate(`   Explanation: ${chalk.greenBright(JSON.stringify(explanation))}`);
                printTransformNamespaces(construction.transformNamespaces, debugConstCreate, "  ");
            }
            ...
            if (explainer.validate !== undefined) {
                ...
                if (error !== undefined) {
                    throw new Error(`${matched === undefined ? "Unmatched" : "Matched"} Explanation Validation failed: ${Array.isArray(error)
                        ? `\n${error.join("\n")}`
                        : error}`);
                }
            }
            else if (matched === undefined) {
                throw new Error(`Missing explanation validation failure for unmatched construction.\n  Construction: ${construction}`);
            }
            if (debugConstCreate.enabled) {
...
export function importConstructions(explanationData, constructionStore, getExplainerForTranslator, mergeMatchSets, cacheConflicts, schemaInfoProvider, ignoreSourceHash = false) {
    if (explanationData.some((data) => data.explainerName !== constructionStore.explainerName)) {
        throw new Error(`Unable to import constructions from different explainer than the store`);
    }
    ...
    for (const data of explanationData) {
        ...
        if (createConstruction === undefined) {
            const fileName = data.fileName !== undefined ? `in file ${data.fileName}` : "";
            throw new Error(`Explainer ${constructionStore.explainerName} does not support construction creation with schema ${data.schemaNames.join(",")}${fileName}.`);
        }
        const constructions = createConstructions(data, explainer, schemaInfoProvider, createConstruction, ignoreSourceHash);
    ...
    for (const { namespaceKeys, construction } of constructionData) {
        const result = constructionStore.addConstruction(namespaceKeys, construction, mergeMatchSets, cacheConflicts);
...
//# sourceMappingURL=importConstructions.js.map