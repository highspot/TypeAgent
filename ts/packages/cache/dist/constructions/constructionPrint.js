...
import { createActionProps, } from "./constructionValue.js";
import { normalizeParamString } from "../explanation/requestAction.js";
export function getMatchPartNames(parts, verbose) {
    const counts = new Map();
    const needFullName = new Map();
    if (!verbose) {
        parts.forEach((p) => {
            if (isMatchPart(p)) {
...
const printMatchedValueTranslator = {
    transform: (transformInfo, matchedText, history) => `map(${matchedText.join(",")})`,
    parse: (parsePart, match) => `convert(${match})`,
};
function filterConstruction(construction, options) {
    const normalizedMatches = options.match?.map((m) => normalizeParamString(m));
    if (normalizedMatches && normalizedMatches.length > 0) {
        if (!normalizedMatches.every((m) => construction.parts.some((p) => {
            if (isMatchPart(p)) {
    ...
    if (options.part) {
        if (!options.part.every((p) => construction.parts.some((c) => {
            if (isMatchPart(c)) {
...
export function printConstructionCache(cache, options) {
    ...
    for (const name of cache.getConstructionNamespaces()) {
        ...
        const filteredConstructions = constructions.filter((c) => filterConstruction(c, options));
        console.log(`Namespace: ${chalk.yellowBright(name)} (${filteredConstructions.length} ${filteredConstructions.length !== constructions.length ? "filtered " : ""}constructions)`);
        for (const construction of filteredConstructions) {
            ...
            console.log(`${construction.id.toString().padStart(3)}: ${chalk.green(construction.toString(verbose))}`);
            const symbolicValues = [];
            const names = getMatchPartNames(construction.parts, verbose);
            const columns = construction.parts.map((p) => {
                ...
                if (isMatchPart(p)) {
                    ...
                    if (!verbose && !all && matches.length > 5) {
                        matches.splice(4, matches.length - 4, `<...> (${matches.length - 4} more)`);
                    }
                    if (p.capture) {
            ...
            const maxLines = columns.reduce((accum, c) => Math.max(accum, c.length), 0);
            const widths = columns.map((c) => c.reduce((accum, l) => Math.max(accum, l.length), 0));
            const lines = [];
            for (let i = 0; i < maxLines; i++) {
                lines.push(columns.map((c, col) => (c[i] || "").padEnd(widths[col])));
                if (i === 0) {
                    lines.push(widths.map((w) => "-".repeat(w)));
                }
            }
            console.log(chalk.cyanBright(lines.map((l) => `  ${l.join(" | ")}`).join("\n")));
            // Print the resulting JSON with symbolic values.
            const result = construction.getMatchedValues(symbolicValues, {
                enableWildcard: false,
                rejectReferences: false,
            }, printMatchedValueTranslator);
            const value = result === undefined
                ? chalk.red("    Value: Internal error")
                : `    Value:\n${JSON.stringify(createActionProps(result.values), undefined, 2).replace(/^/gm, " ".repeat(6))}`;
            console.log(value);
        }
    }
    if (verbose) {
...
//# sourceMappingURL=constructionPrint.js.map