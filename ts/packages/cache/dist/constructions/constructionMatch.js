...
import { isSpaceOrPunctuation, isSpaceOrPunctuationRange, isWordBoundary, spaceAndPunctuationRegexStr, } from "../utils/regexp.js";
import { matchedValues, } from "./constructionValue.js";
const wildcardRegex = new RegExp(`^${spaceAndPunctuationRegexStr}*([^\\s].*?)${spaceAndPunctuationRegexStr}*$`);
export function matchParts(request, parts, config, matchValueTranslator) {
    const state = {
        capture: [],
        matchedStart: [],
        matchedEnd: [],
    ...
    do {
        if (finishMatchParts(state, request, parts, config)) {
            const values = matchedValues(parts, state.capture, config, matchValueTranslator);
            if (values !== undefined) {
                return values;
            }
        }
...
function finishMatchParts(state, request, parts, config) {
    ...
    if (state.pendingWildcard === -1) {
        ...
        return (state.matchedCurrent === request.length ||
            isSpaceOrPunctuationRange(request, state.matchedCurrent, request.length));
    }
    // End with wildcard
...
export function createMatchPartsCache(cachedString) {
    return {
        cachedString,
        cache: new Map(),
...
export function getMatchPartsCacheStats(matchPartsCache) {
