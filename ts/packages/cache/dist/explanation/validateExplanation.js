...
import { spaceAndPunctuationRegexStr, escapeMatch, wordBoundaryRegexStr, } from "../utils/regexp.js";
import { getExactStringRequirementMessage } from "./explainer.js";
export function validateSubPhraseText(requestAction, subPhrases) {
    ...
    for (const phrase of subPhrases) {
        ...
        const matchNext = new RegExp(`^${expectedMatch}${wordBoundaryRegexStr}(.*)`, "i");
        const result = matchNext.exec(current);
        if (result === null) {
            // doesn't match, detect different errors
            const matchBoundary = new RegExp(`^${expectedMatch}(?<=\\w)(\\w+)`, "i");
            ...
            const matchNext = new RegExp(`^${spaceAndPunctuationRegexStr}*(.*?)${expectedMatch}`, "i");
...
export function getActionProperty(actionProps, propertyName) {
    const nameParts = propertyName.split(".");
    let curr = actionProps; // TODO: Is there a better typing
...
export function checkActionPropertyValue(actionProps, propertyName, implicit) {
    const parameterStr = implicit ? "implicit parameter" : "parameter";
    const curr = getActionProperty(actionProps, propertyName);
    if (curr === undefined) {
        throw new Error(`Extraneous ${parameterStr}: '${propertyName}' is not a parameter in the action`);
    }
    if (Array.isArray(curr) || typeof curr === "object") {
        // This shouldn't happen since the schema should have caught it
        throw new Error(`Invalid ${parameterStr}: '${propertyName}' is not a leaf with a primitive value in the action`);
    }
    return curr;
}
// make sure the parameter value in the explanation is the same as the parameter value in the action
export function checkActionProperty(actionProps, // work on action or action.parameters, based on "paramName" format matches
param, implicit) {
    const curr = checkActionPropertyValue(actionProps, param.paramName, implicit);
    let isEqual = false;
    ...
    if (!isEqual) {
        throw new Error(`Mismatch parameter value: '${param.paramValue}' in the explanation is not the value of the parameter '${param.paramName}' in the action`);
    }
}
...
function ensureProperty(explanationParamNameSet, propertyName, value) {
    const corrections = [];
    if (Array.isArray(value)) {
        for (let i = 0; i < value.length; i++) {
            corrections.push(...ensureProperty(explanationParamNameSet, `${propertyName}.${i}`, value[i]));
        }
    }
    else if (typeof value === "object") {
        for (const key of Object.keys(value)) {
            corrections.push(...ensureProperty(explanationParamNameSet, `${propertyName}.${key}`, value[key]));
        }
    }
    else {
        if (!explanationParamNameSet.has(propertyName)) {
            corrections.push(`Missing parameter: parameter '${propertyName}' in the action is missing from explanation`);
        }
    }
    return corrections;
}
// Make sure the explanationParamNameSet has all property names
export function ensureProperties(explanationParamNameSet, actionProps) {
    const corrections = [];
    for (const key of Object.keys(actionProps)) {
        corrections.push(...ensureProperty(explanationParamNameSet, key, actionProps[key]));
    }
    return corrections;
}
//# sourceMappingURL=validateExplanation.js.map