...
import { getActionDescription, getExactStringRequirementMessage, } from "../explainer.js";
import { checkActionProperty, ensureProperties, } from "../validateExplanation.js";
import { form } from "./explanationV5.js";
export function createPropertyExplainer(enableContext, model) {
    const substringRequirement = getExactStringRequirementMessage(false);
    return new TypeChatAgent("property explanation", () => {
        return createJsonTranslatorFromFile("PropertyExplanation", getPackageFilePath(enableContext
            ? "./src/explanation/v5/propertyExplanationSchemaV5WithContext.ts"
            : "./src/explanation/v5/propertyExplanationSchemaV5.ts"), { model });
    }, (requestAction) => {
        return (`${form} with the following value:\n${requestAction.toPromptString()}\n` +
            (enableContext
    ...
    }, (requestAction) => requestAction.toPromptString(), validatePropertyExplanation);
}
export function isImplicitParameter(parameter) {
    return parameter.hasOwnProperty("isImplicit");
}
export function isEntityParameter(parameter) {
    return parameter.hasOwnProperty("entityIndex");
}
// REVIEW: disable entity constructions.
const enableEntityConstructions = false;
function validatePropertyExplanation(requestAction, actionExplanation, config) {
    const corrections = [];
    ...
    for (const prop of actionExplanation.properties) {
        if (propertyNameSet.has(prop.name)) {
            corrections.push(`Multiple properties with property name '${prop.name}' found`);
        ...
        if (!isImplicitParameter(prop)) {
            if (isEntityParameter(prop) && prop.entityIndex !== undefined) {
                // TODO: fuzzy match
                if (normalizeParamString(prop.substrings.join(" ")) ===
                    normalizeParamString(prop.value.toString())) {
                    corrections.push(`'${prop.name}' has value '${prop.value}' from a substring in the request. Should not have an entity index.`);
                }
                else {
                    if (enableEntityConstructions === false) {
                        throw new Error("Request has references to entities in the context");
                    }
                    const entities = requestAction.history?.entities;
                    if (entities === undefined ||
                        prop.entityIndex < 0 ||
                        entities.length <= prop.entityIndex) {
                        corrections.push(`Entity index ${prop.entityIndex} for '${prop.name}' is out of range`);
                    }
                    else if (entities[prop.entityIndex].name !== prop.value) {
                        corrections.push(`Entity at index ${prop.entityIndex} in the context doesn't match the value for property '${prop.name}'`);
                    }
                }
            }
            const normalizedRequest = normalizeParamString(requestAction.request);
            for (const substring of prop.substrings) {
                if (!normalizedRequest.includes(normalizeParamString(substring))) {
                    corrections.push(`Substring '${substring}' for property '${prop.name}' not found in the request string. ${getExactStringRequirementMessage(false)}`);
                }
            }
            ...
            if (typeof prop.value === "number" &&
                ...
                if (value !== null) {
                    if (parseInt(value[0]) !== prop.value) {
                        corrections.push(`Value ${prop.value} for property '${prop.name}' doesn't match the number ${value} in the substring '${prop.substrings[0]}'. Missing other substring to explain the value.`);
                    }
                }
            }
        }
...
//# sourceMappingURL=propertyExplainationV5.js.map