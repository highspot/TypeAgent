...
import { normalizeParamString, } from "../requestAction.js";
import { Construction, } from "../../constructions/constructions.js";
import { getParamSpec, getParamRange, getNamespaceForCache, } from "../schemaInfoProvider.js";
import { openai } from "aiclient";
import { createPropertyExplainer, isEntityParameter, } from "./propertyExplainationV5.js";
import { createSubPhraseExplainer, hasPropertyNames, isPropertySubPhrase, } from "./subPhraseExplanationV5.js";
import { createAlternativesExplainer, } from "./alternativesExplanationV5.js";
import { getPropertyParser, } from "../../constructions/propertyParser.js";
import { createParsePart } from "../../constructions/parsePart.js";
import { createMatchPart, isMatchPart, } from "../../constructions/matchPart.js";
import { Transforms } from "../../constructions/transforms.js";
import { getLanguageTools } from "../../utils/language.js";
import { createPolitenessGeneralizer, } from "./politenessGeneralizationV5.js";
// Parameter Explanation
export const form = "The user request is a JSON object containing a request string and the translated action";
class ExplanationV5TypeChatAgent {
    constructor(model) {
        this.propertyExplainerWithoutContext = createPropertyExplainer(false, model);
        this.propertyExplainerWithContext = createPropertyExplainer(true, model);
    ...
    async run(input, config) {
        ...
        const result2 = await this.subPhrasesExplainer.run([input, result1.data], config);
        ...
        const result3 = await this.alternativesExplainer.run([input, result1.data, result2.data], config);
    ...
    validate(input, result, config) {
        ...
        const result2 = this.subPhrasesExplainer.validate?.([input, result], result, config);
        const result3 = this.alternativesExplainer.validate?.([input, result, result], result, config);
        const corrections = [];
        if (result1) {
...
function getPropertyInfo(propertyName, actions) {
    ...
    if (actions.length > 1) {
        ...
        if (action === undefined) {
            throw new Error(`Invalid index '${firstPart}' in property name '${propertyName}'`);
        }
...
function getPropertySpec(propertyName, actions, schemaInfoProvider) {
...
function getPropertyTransformInfo(propertyName, actions, schemaInfoProvider) {
    const { action, parameterName, actionIndex } = getPropertyInfo(propertyName, actions);
    ...
    const namespace = parameterName
        ? getNamespaceForCache(translatorName, action.actionName, schemaInfoProvider)
...
async function augmentExplanation(explanation, requestAction, constructionCreationConfig) {
    ...
    for (const param of explanation.propertyAlternatives) {
        ...
        const paramSpec = getPropertySpec(param.propertyName, actions, schemaInfoProvider);
        if (paramSpec) {
...
`;
                    ...
                    if (result.success) {
                        const generatedAlternatives = JSON.parse(result.data);
                        const additionalEntries = generatedAlternatives.variations.map((v) => ({
...
export function createExplainerV5(model) {
    const agent = new ExplanationV5TypeChatAgent(model);
    return new Explainer(agent, createConstructionV5, toPrettyString, augmentExplanation);
}
function collectAltParamMatches(matches, phrase, paramInfo) {
    const index = paramInfo.propertySubPhrases.indexOf(phrase.text);
    if (index === -1) {
        // This shouldn't happen validate should have caught this
        throw new Error(`Property sub-phrase '${phrase.text}' not found in property '${paramInfo.propertyName}'`);
    }
    if (paramInfo.alternatives !== undefined) {
        paramInfo.alternatives.forEach((alt) => {
            ...
            if (alt.propertySubPhrases.length < index) {
                // This shouldn't happen validate should have caught this
                throw new Error(`Alternate sub-phrase for '${paramInfo.propertyName} not found at index ${index}'`);
            }
            matches.push(alt.propertySubPhrases[index]);
        });
    }
}
function getParserForPropertyValue(propertyValue, actions, schemaInfoProvider) {
    if (propertyValue.propertySubPhrases.length !== 1) {
        return undefined;
    }
    const paramSpec = getPropertySpec(propertyValue.propertyName, actions, schemaInfoProvider);
...
function canBeMergedNonPropertySubPhrase(phrase) {
    return langTool?.hasClosedClass(phrase.text, phrase.isOptional) !== true;
}
function useSynonymsForNonPropertySubPhrase(phrase) {
    return langTool?.hasClosedClass(phrase.text, phrase.isOptional) !== true;
}
function addPolitePrefixParts(parts, explanation) {
    if (explanation.politePrefixes === undefined ||
    ...
    if (!hasPolitePrefix && explanation.politePrefixes.length !== 0) {
        parts.unshift(createMatchPart(explanation.politePrefixes, "politeness", {
            optional: true,
        }));
    }
    if (!hasPoliteSuffix && explanation.politeSuffixes.length !== 0) {
        parts.push(createMatchPart(explanation.politeSuffixes, "politeness", {
            optional: true,
        }));
    }
}
export function createConstructionV5(requestAction, explanation, constructionCreationConfig) {
    const actions = requestAction.actions;
    const schemaInfoProvider = constructionCreationConfig.schemaInfoProvider;
    const entityParameters = explanation.properties.filter((param) => isEntityParameter(param) && param.entityIndex !== undefined);
    const entityParamMap = new Map(entityParameters.map((param) => [param.name, param.entityIndex]));
    // Collect the subphrases that we will ignore synonyms for.
    const disableSynonymsSubPhrases = new Set();
    // Collect the property names that we will ignore alternates for.
    const disableAlternateParamValues = new Set();
    for (const phrase of explanation.subPhrases) {
        ...
        if (phrase.propertyNames.some((propertyName) => getPropertySpec(propertyName, actions, schemaInfoProvider) === "literal")) {
            phrase.propertyNames.forEach((propertyName) => disableAlternateParamValues.add(propertyName));
        }
    }
    const transformNamespaces = new Map();
    const getTransforms = (namespace) => {
    ...
    const propertyParserMap = new Map();
    ...
    for (const param of explanation.propertyAlternatives) {
        ...
        const propertyParser = getParserForPropertyValue(param, actions, schemaInfoProvider);
        if (propertyParser !== undefined) {
            propertyParserMap.set(propertyName, propertyParser);
            continue;
        }
        // Add the transforms for the property
        const transformInfo = getPropertyTransformInfo(propertyName, actions, schemaInfoProvider);
        ...
        if (entityIndex !== undefined) {
            ...
            transforms.addEntity(transformInfo.transformName, param.propertySubPhrases.join("|"), requestAction.history.entities[entityIndex].type);
        ...
        transforms.add(transformInfo.transformName, param.propertySubPhrases.join("|"), param.propertyValue, true);
        ...
        if (!disableAlternateParamValues.has(param.propertyName)) {
            param.alternatives.forEach((alt) => {
                ...
                transforms.add(transformInfo.transformName, alt.propertySubPhrases.join("|"), alt.propertyValue, false);
            });
        }
    }
    ...
    const shouldValueBeCopied = (phrase, paramInfo) => {
    ...
    const updateWildcardMode = (wildcardMode, phrase, paramInfo) => {
        if (wildcardMode === 0 /* WildcardMode.Disabled */ ||
            !shouldValueBeCopied(phrase, paramInfo)) {
            return 0 /* WildcardMode.Disabled */;
        }
        const spec = getPropertySpec(paramInfo.propertyName, actions, schemaInfoProvider);
        return spec === "wildcard"
            ? 1 /* WildcardMode.Enabled */
            : spec === "checked_wildcard"
                ? wildcardMode
                : 0 /* WildcardMode.Disabled */;
    };
    const parts = explanation.subPhrases.map((phrase) => {
        try {
            ...
            let wildcardMode = 2 /* WildcardMode.Checked */;
            ...
            for (const propertyName of phrase.propertyNames) {
                ...
                if (entityParamMap.has(propertyName)) {
                    wildcardMode = 0 /* WildcardMode.Disabled */; // not a wildcard mapping
                ...
                wildcardMode = updateWildcardMode(wildcardMode, phrase, paramInfo);
            ...
            throw new Error(`Exception while processing the phrase '${phrase.text}': ${e.message}`);
        }
    });
    const implicitProperties = explanation.properties.filter((param) => !explicitPropertyNames.has(param.name));
    addPolitePrefixParts(parts, explanation);
    return Construction.create(parts, transformNamespaces, implicitProperties.map((e) => {
        return {
            paramName: e.name,
            paramValue: e.value,
        };
...
function toPrettyString(explanation) {
    const categoryStr = (phrase) => {
    ...
    const widths = explanation.subPhrases.map((phrase) => Math.max(phrase.text.length, phrase.category.length + 2, ...(isPropertySubPhrase(phrase)
        ? []
        : phrase.synonyms.map((s) => s.length)), ...categories.map((c) => c.length)));
    const nameWidths = 10;
    const createLine = (name, entries) => {
        return `${name.padStart(nameWidths)} | ${entries
            .map((e, i) => e.padEnd(widths[i]))
            .join(" | ")} |`;
    };
    const horizontalLine = createLine("", widths.map((w) => "-".repeat(w)));
    const lines = [];
    // Category
    lines.push(createLine("Category", categories));
    lines.push(horizontalLine);
    // Original
    lines.push(createLine("Original", explanation.subPhrases.map((phrase) => phrase.text)));
    lines.push(horizontalLine);
    // Synonyms
    const maxSynonyms = Math.max(...explanation.subPhrases.map((phrase) => isPropertySubPhrase(phrase) ? 0 : phrase.synonyms.length));
    for (let i = 0; i < maxSynonyms; i++) {
        lines.push(createLine(i === 0 ? "Synonyms" : "", explanation.subPhrases.map((phrase) => isPropertySubPhrase(phrase) ? "" : phrase.synonyms[i] ?? "")));
    }
    return lines.join("\n");
}
//# sourceMappingURL=explanationV5.js.map