...
import { getInstanceSessionNames, getInstanceSessionsDirPath, } from "agent-dispatcher/explorer";
...
export class VisualizationNotifier {
    constructor() {
        this.knowledgeFileDebounce = 0;
        this.listFileDebounce = 0;
        this.onListChanged = null;
        this.onKnowledgeUpdated = null;
        this.onHierarchyUpdated = null;
        this.onWordsUpdated = null;
        // file watchers
        fs.watch(getInstanceSessionsDirPath(), { recursive: true }, async (_, fileName) => {
        ...
        const kDir = path.join("conversation", "knowledge");
        fs.watch(getInstanceSessionsDirPath(), { recursive: true }, async (_, fileName) => {
    ...
    async enumerateLists() {
        let retValue = new TypeAgentList("sessions", new Array());
        ...
        sessions.map((n) => {
            ...
            let newList = new TypeAgentList(n, new Array());
            retValue.items.push(newList);
            // get the lists for all sessions
            try {
                const listsRaw = fs.readFileSync(path.join(getInstanceSessionsDirPath(), n, "list", "lists.json"));
                const lists = JSON.parse(listsRaw.toString());
                lists.map((z) => {
                    newList.items.push(z);
                });
            }
            catch (e) {
                console.log(e);
    ...
    async enumerateKnowledge() {
        let retValue = new Array();
        // create levels
        for (let i = 0; i < 6; i++) {
            retValue.push(new Array());
        }
        const sessions = await getInstanceSessionNames();
        ...
        const knowledgeDir = path.join(getInstanceSessionsDirPath(), lastSession, "conversation", "knowledge");
        ...
        files.map((f) => {
            ...
            const s = fs.readFileSync(path.join(knowledgeDir, f));
            const kk = JSON.parse(s.toString()); //todo: finish
            knowledgeMap.set(f, kk);
        });
        // level 2 - categories
        const entities = new KnowledgeGraph("entities", []);
        const topics = new KnowledgeGraph("topics", []);
        const actions = new KnowledgeGraph("actions", []);
        ...
        knowledgeMap.forEach((k, s) => {
            ...
            if (k.entities?.length > 0) {
                ...
                k.entities.map((n) => {
                    n.value.type.map((t) => {
                        if (!level3.get(t)) {
                            level3.set(t, new Set().add("entities"));
                        }
                        else {
                    ...
                    if (n.value.facets) {
                        n.value.facets.map((f) => {
                            if (!level4.get(f.value)) {
                                if (typeof f.value === "string") {
                                    level4.set(f.value, new Set().add(n.value.name));
                                }
                            }
                            else {
                                if (typeof f.value === "string") {
            ...
            if (k.topics?.length > 0) {
                ...
                k.topics.map((n) => {
                    if (!level3.get(n.value)) {
                        level3.set(n.value, new Set().add("topics"));
                    }
                    else {
            ...
            if (k.actions?.length > 0) {
                ...
                k.actions.map((a) => {
                    a.value.verbs.map((v) => {
                        if (!level3.get(v)) {
                            level3.set(v, new Set().add("actions"));
                        }
                        else {
        ...
        leve3_sorted.forEach((value, key) => {
    ...
    async enumerateKnowledgeForHierarchy() {
        let retValue = new Array();
        ...
        files.map((f) => {
            ...
            let hh = {
                ...
                imports: new Array("knowledge.message"),
            };
            retValue.push(hh);
        });
        return retValue;
    }
    async enumerateKnowledgeForWordCloud() {
        let retValue = new Array();
        const sessions = await getInstanceSessionNames();
        const lastSession = sessions[sessions.length - 1];
        // get the knowledge for this session
...
VisualizationNotifier.getinstance = () => {
...
//# sourceMappingURL=visualizationNotifier.js.map