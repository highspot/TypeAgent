...
export class TangledTree {
    constructor(data) {
        this.color = scaleOrdinal(schemeDark2);
        this.background_color = "white";
        this.update(data);
    }
    constructTangleLayout(levels, options = { c: 0, bigc: 0 }) {
        ...
        levels.forEach((l) => {
            y_negative_offset +=
                -min_family_height +
                    min(l.bundles, (b) => min(b.links, (link) => link.ys - 2 * c - (link.yt + c))) || 0;
    ...
    renderChart(data2, options = { c: 0, bigc: 0 }) {
        const tangleLayout = this.constructTangleLayout(data2, options);
        return `<svg width="${tangleLayout.layout.width}" height="${tangleLayout.layout.height}" style="background-color: ${this.background_color}">
        <style>
          text {
        ...
        ${tangleLayout.nodes.map((n) => `
          ...
          <text class="selectable" data-id="${n.id}" x="${n.x + 4}" y="${n.y - n.height / 2 - 4}" stroke="${this.background_color}" stroke-width="2">${n.id}</text>
...
//# sourceMappingURL=tangledTree.js.map