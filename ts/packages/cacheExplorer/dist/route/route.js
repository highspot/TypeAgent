...
import { getInstanceSessionNames, getSessionConstructionDirPaths, getSessionConstructionDirPath, getInstanceSessionDirPath, } from "agent-dispatcher/explorer";
...
export function setupMiddlewares(middlewares, devServer) {
    ...
    app.get("/session/:session/cache/:cache", async (req, res) => {
        ...
        res.json(JSON.parse(await fs.promises.readFile(path.join(cacheDir, req.params.cache), "utf-8")));
    });
    return middlewares;
}
