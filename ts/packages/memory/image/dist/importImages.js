...
import { ConversationIndex, createKnowledgeModel, createConversationSettings, addMetadataToIndex, buildSecondaryIndexes, ConversationSecondaryIndexes, writeConversationDataToFile, readConversationDataFromFile, } from "knowpro";
import { image } from "knowledge-processor";
...
export class ImageMeta {
    ...
    getKnowledge() {
        ...
        for (let i = 0; i < this.img?.exifData.length; i++) {
            if (this.img?.exifData[i] !== undefined &&
        ...
        if (this.img.nearbyPOI) {
            for (let i = 0; i < this.img.nearbyPOI.length; i++) {
                ...
                if (this.img.nearbyPOI[i].position != undefined &&
                    ...
                    poiEntity.facets?.push({
                        name: "longitude",
                        value: this.img.nearbyPOI[i].position.longitude.toString(),
                    });
                    poiEntity.facets?.push({
                        name: "latitude",
                        value: this.img.nearbyPOI[i].position.latitude.toString(),
                    });
                }
                if (this.img.nearbyPOI[i].categories !== undefined &&
        ...
        if (this.img.reverseGeocode) {
            for (let i = 0; i < this.img.reverseGeocode.length; i++) {
                ...
                if ((i == 0 ||
                    ...
                    const addrOutput = this.img.reverseGeocode[i].address;
                    const addrEntity = {
                        name: this.img.reverseGeocode[i].address
                            .formattedAddress ?? "",
                        type: ["address"],
                        facets: [],
                    };
                    ...
                    if (addrOutput.countryRegion) {
                        if (addrOutput.countryRegion.name !== undefined &&
                        ...
                        if (addrOutput.countryRegion.ISO !== undefined &&
...
export class ImageCollection {
    constructor(nameTag = "", messages = [], tags = [], semanticRefs = []) {
        ...
        this.secondaryIndexes = new ConversationSecondaryIndexes(this.settings.relatedTermIndexSettings);
    }
    addMetadataToIndex() {
        if (this.semanticRefIndex) {
            addMetadataToIndex(this.messages, this.semanticRefs, this.semanticRefIndex, (type, knowledge) => {
                if (type === "entity") {
                    return !isDuplicateEntity(knowledge, this.semanticRefs);
                }
                return true;
            });
        }
    ...
    async deserialize(data) {
        ...
        if (data.semanticIndexData) {
            this.semanticRefIndex = new ConversationIndex(data.semanticIndexData);
        }
        if (data.relatedTermsIndexData) {
            this.secondaryIndexes.termToRelatedTermsIndex.deserialize(data.relatedTermsIndexData);
        }
    ...
    static async readFromFile(dirPath, baseFileName) {
        ...
        const data = await readConversationDataFromFile(dirPath, baseFileName, imageCollection.settings.relatedTermIndexSettings
 ...
 */
export async function importImages(imagePath, cachePath, recursive = true, callback) {
 ...
 */
async function indexImages(sourcePath, cachePath, recursive, chatModel, callback) {
 ...
 */
async function indexImage(fileName, cachePath, chatModel) {
    ...
    const img = await image.loadImageWithKnowledge(fileName, cachePath, chatModel);
    if (img !== undefined) {
 ...
 */
function isDuplicateEntity(entity, semanticRefs) {
    for (let i = 0; i < semanticRefs.length; i++) {
        if (semanticRefs[i].knowledgeType == "entity" &&
            entity.name ==
                semanticRefs[i].knowledge.name) {
...
//# sourceMappingURL=importImages.js.map