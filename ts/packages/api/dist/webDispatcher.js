...
export async function createWebDispatcher() {
    let ws = null;
    const clientIOChannel = createGenericChannel((message) => ws?.send(JSON.stringify({
    ...
    let settingSummary = "";
    const updateSettingSummary = (force = false) => {
    ...
    async function processShellRequest(text, id, images) {
    ...
    const dispatcherChannel = createGenericChannel((message) => ws?.send(JSON.stringify({
...
//# sourceMappingURL=webDispatcher.js.map