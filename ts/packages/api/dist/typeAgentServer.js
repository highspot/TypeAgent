...
import { TypeAgentAPIWebServer, } from "./webServer.js";
...
import { createWebDispatcher } from "./webDispatcher.js";
export class TypeAgentServer {
    constructor(envPath) {
        this.envPath = envPath;
        this.fileWriteDebouncer = new Map();
        // typeAgent config
    ...
    async start() {
        ...
        // websocket server
        this.webSocketServer = new TypeAgentAPIWebSocketServer(this.webServer.server, this.webDispatcher.connect);
    }
    stop() {
        this.webServer?.stop();
     ...
     */
    async findBlobs(containerClient) {
    ...
    startLocalStorageBackup() {
        fs.watch(getUserDataDir(), { recursive: true, persistent: false }, async (_, fileName) => {
            ...
            // increase refcount
            let refCount = this.fileWriteDebouncer.get(fileName);
     ...
     */
    debounceFileThenUpload(fileName) {
        setTimeout(async () => {
            if (!this.fileWriteDebouncer.has(fileName)) {
                return;
            }
            ...
            const debounceCount = this.fileWriteDebouncer.get(fileName);
            if (debounceCount == 0) {
                try {
                    const localPath = path.join(getUserDataDir(), fileName);
                    if (!this.storageProvider) {
...
//# sourceMappingURL=typeAgentServer.js.map