...
export class TypeAgentAPIWebServer {
    constructor(config) {
        ...
        // secure webserver
        if (existsSync(".cert/localhost+2-key.pem") &&
    ...
    serve(config, request, response) {
        // serve up the requested file if we have it
        let root = path.resolve(config.wwwroot);
        let requestedFile = request.url == "/" || request.url === undefined
            ? "index.html"
            : request.url;
