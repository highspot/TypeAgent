...
function collectTypes(definitions, type) {
    switch (type.type) {
        case "object":
 ...
 * @returns
 */
export function toJSONActionSchemaFile(actionSchemaFile) {
    const definitions = {};
...
function resolveTypes(definitions, type) {
    switch (type.type) {
        case "object":
            for (const field of Object.values(type.fields)) {
        ...
        case "type-reference":
            if (type.definition !== undefined) {
                throw new Error("Internal error: type reference already have a definition");
            }
 ...
 * @returns
 */
export function fromJSONActionSchemaFile(json) {
    ...
    const schemaConfig = json.actionNamespace
        ...
        : undefined;
    return createActionSchemaFile(json.schemaName, json.sourceHash, entry, order, true, schemaConfig);
}
