...
import { toJSONActionSchemaFile, fromJSONActionSchemaFile, } from "../src/serialize.js";
import { fileURLToPath } from "node:url";
import { generateActionSchema } from "../src/generator.js";
import prettier from "prettier";
const defaultAgentProviderPath = fileURLToPath(new URL("../../../defaultAgentProvider", import.meta.url));
const configPath = path.resolve(defaultAgentProviderPath, "./data/config.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf-8"));
const tests = [];
function loadSchemaConfig(fileName) {
    try {
        const parsedFileName = path.parse(fileName);
        const schemaConfigFileName = path.join(parsedFileName.dir, parsedFileName.name + ".json");
        return fs.existsSync(schemaConfigFileName)
            ? JSON.parse(fs.readFileSync(schemaConfigFileName, "utf-8"))
            : undefined;
    }
    catch (e) {
        throw new Error(`Failed to load schema config file: ${fileName}\n${e.message}`);
    }
}
function addTest(schemaName, config, dir) {
    ...
    if (config.subActionManifests) {
        for (const [subname, subConfig] of Object.entries(config.subActionManifests)) {
            addTest(`${schemaName}.${subname}`, subConfig, dir);
        }
    }
}
const defaultAgentProviderRequire = createRequire(`${defaultAgentProviderPath}/src`);
for (const [name, entry] of Object.entries(config.agents)) {
    const manifestModulePath = `${entry.name}/agent/manifest`;
    const manifestPath = defaultAgentProviderRequire.resolve(manifestModulePath);
    const manifest = defaultAgentProviderRequire(manifestPath);
    const manifestDir = path.dirname(manifestPath);
    addTest(name, manifest, manifestDir);
}
async function compare(original, regenerated) {
...
describe("Action Schema Regeneration", () => {
    ...
    //
    it.skip.each(tests)("should regenerate $schemaName", async ({ source, schemaName, fileName, typeName, schemaConfig }) => {
        const actionSchemaFile = parseActionSchemaSource(source, schemaName, "testHash", // Don't care about source hash in tests
        typeName, fileName, schemaConfig, true);
        const regenerated = await generateActionSchema(actionSchemaFile, {
            exact: true,
        });
        await compare(source, regenerated);
    });
    it.each(tests)("should roundtrip regenerated - $schemaName", async ({ source, schemaName, fileName, typeName }) => {
        const actionSchemaFile = parseActionSchemaSource(source, schemaName, "testHash", // Don't care about source hash in tests
        typeName, fileName);
        const regenerated = await generateActionSchema(actionSchemaFile);
        const roundtrip = parseActionSchemaSource(regenerated, schemaName, "testHash", // Don't care about source hash in tests
        typeName);
...
describe("Action Schema Serialization", () => {
    it.each(tests)("roundtrip $schemaName", async ({ source, schemaName, fileName, typeName }) => {
        const actionSchemaFile = parseActionSchemaSource(source, schemaName, "testHash", // Don't care about source hash in tests
        typeName, fileName);
        const serialized = toJSONActionSchemaFile(actionSchemaFile);
        const deserialized = fromJSONActionSchemaFile(structuredClone(serialized));
        ...
        const deserialized2 = fromJSONActionSchemaFile(structuredClone(serialized));
        expect(deserialized2).toEqual(deserialized);
    });
});
//# sourceMappingURL=regen.spec.js.map