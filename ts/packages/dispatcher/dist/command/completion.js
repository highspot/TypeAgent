...
import { getDefaultSubCommandDescriptor, resolveCommand, } from "./command.js";
...
function getPartialCompletedCommand(input) {
...
function getPendingFlag(params, flags) {
...
async function getCommandParameterCompletion(descriptor, context, result) {
    const completions = [];
    ...
    const sessionContext = context.agents.getSessionContext(result.actualAppAgentName);
    ...
    if (agent.getCommandCompletion) {
        completions.push(...(await agent.getCommandCompletion(result.commands, params, pendingCompletions, sessionContext)));
    }
    return completions;
}
export async function getCommandCompletion(input, context) {
    ...
    if (descriptor !== undefined) {
        ...
        const parameterCompletions = await getCommandParameterCompletion(descriptor, context, result);
        ...
        if (result.parsedAppAgentName === undefined &&
            ...
            completions.push(...context.agents
                .getAppAgentNames()
