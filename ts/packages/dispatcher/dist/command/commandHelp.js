...
import { getFlagType, isCommandDescriptorTable, } from "@typeagent/agent-sdk/helpers/command";
import { getDefaultSubCommandDescriptor } from "./command.js";
import { displayResult } from "@typeagent/agent-sdk/helpers/display";
export function getUsage(command, descriptor) {
    if (descriptor.help) {
        return descriptor.help;
    }
    const paramUsage = [];
    const paramUsageFull = [];
    if (descriptor.parameters !== undefined) {
        if (descriptor.parameters.args) {
            ...
            if (args.length !== 0) {
                ...
                const maxNameLength = Math.max(...args.map(([name]) => name.length));
                for (const [name, def] of args) {
                    const usage = `<${name}>${def.multiple === true ? "..." : ""}`;
                    paramUsage.push(def.optional ? `[${usage}]` : usage);
                    paramUsageFull.push(`  ${`<${name}>`.padStart(maxNameLength)} - ${def.optional ? "(optional) " : ""}${def.description} (type: ${def.type ?? "string"})`);
                }
            }
        }
        if (descriptor.parameters.flags) {
            ...
            if (flags.length !== 0) {
                ...
                const maxNameLength = Math.max(...flags.map(([name]) => name.length));
                for (const [name, def] of flags) {
                    ...
                    paramUsageFull.push(`  ${`--${name}`.padStart(maxNameLength)} ${def.char !== undefined ? `-${def.char}` : "  "}${typeStr.padEnd(8)} : ${def.description}${def.default !== undefined ? ` (default: ${def.default})` : ""}`);
                }
            }
        }
    }
...
export function getHandlerTableUsage(table, command, systemContext) {
    const output = [];
    if (command) {
        ...
        output.push(`${chalk.bold(chalk.underline(`Subcommands: ${table.description}`))}`);
        output.push("");
        ...
        for (const name of systemContext.agents.getAppAgentNames()) {
            if (systemContext.agents.isCommandEnabled(name)) {
                output.push(`  ${name.padEnd(maxNameLength)} : ${systemContext.agents.getAppAgentDescription(name)}`);
            }
        }
        output.push("");
...
export function printStructuredHandlerTableUsage(table, command, context) {
    let index = 0;
    const commands = [];
    if (command) {
        const defaultSubCommand = getDefaultSubCommandDescriptor(table);
