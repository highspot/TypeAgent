...
import { ensureCommandResult, getCommandResult, } from "../context/commandHandlerContext.js";
...
export function getDefaultSubCommandDescriptor(table) {
...
export async function resolveCommand(input, context) {
    let prev = undefined;
    let curr = input;
    const nextToken = () => {
    ...
    const appAgentName = first !== undefined && context.agents.isAppAgentName(first)
        ? first
        : undefined;
    let actualAppAgentName;
    const useParsedAppAgentName = appAgentName !== undefined &&
    ...
    const parsedAppAgentName = useParsedAppAgentName || commands.length === 0
        ? appAgentName
        : undefined;
    const result = {
        parsedAppAgentName,
...
async function parseCommand(originalInput, context) {
    ...
    if (result.descriptor !== undefined) {
        ...
        try {
            if (result.suffix !== "" &&
                ...
                throw new Error(`Command '@${getParsedCommand(result)}' does not accept parameters.`);
            }
    ...
    const command = getParsedCommand(result);
    if (result.parsedAppAgentName !== undefined &&
        !context.agents.isCommandEnabled(result.parsedAppAgentName)) {
        throw new Error(`Command for '${result.parsedAppAgentName}' is disabled.`);
    }
    if (result.table === undefined) {
        throw new Error(`Unknown command '${input}'`);
    }
    const message = command.length === 0
...
export function getParsedCommand(result) {
...
export async function processCommandNoLock(originalInput, context, attachments) {
    try {
        const result = await parseCommand(originalInput, context);
        return await executeCommand(result.command, result.params, result.appAgentName, context, attachments);
    }
    catch (e) {
        context.clientIO.appendDisplay(makeClientIOMessage(context, {
            type: "text",
            content: `ERROR: ${e.message}`,
            kind: "error",
        }, context.requestId, DispatcherName, undefined), "block");
...
function beginProcessCommand(requestId, context) {
...
function endProcessCommand(requestId, context) {
...
export async function processCommand(originalInput, context, requestId, attachments) {
...
export function getSettingSummary(context) {
    ...
    const prompt = ["\uD83E\uDD16" /* unicodeChar.robotFace */];
    const names = context.agents.getActiveSchemas();
    const ordered = names.filter((name) => name !== context.lastActionSchemaName);
    if (ordered.length !== names.length) {
        ordered.unshift(context.lastActionSchemaName);
    }
    const translators = Array.from(new Set(ordered.map((name) => context.agents.getActionConfig(name).emojiChar)).values());
...
export function getTranslatorNameToEmojiMap(context) {
    return new Map(Object.entries(context.agents.getEmojis()));
}
export function getPrompt(context) {
