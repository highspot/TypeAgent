...
import { getFlagMultiple, getFlagType, resolveFlag, } from "@typeagent/agent-sdk/helpers/command";
import { setObjectProperty } from "common-utils";
function parseIntParameter(valueStr, kind, name) {
    const value = parseInt(valueStr);
    if (value.toString() !== valueStr) {
        throw new Error(`Invalid number value '${valueStr}' for ${kind} '${name}'`);
    }
    return value;
}
function parseJsonParameter(valueStr, kind, name) {
    try {
        ...
        throw new Error(`Invalid JSON value for ${kind} '${name}': ${e.message}`);
    }
}
function stripQuoteFromToken(term) {
...
function invalidValueToken(valueToken) {
...
function parseValueToken(flagToken, flag, parsedFlags, valueType, valueToken) {
    ...
    if (Array.isArray(existing)) {
        throw new Error(`Invalid flag '${flag}': multiple json value cannot use property syntax`);
    }
...
export function parseParams(parameters, paramDefs, partial = false) {
    ...
    const argDefs = paramDefs.args !== undefined
        ...
        try {
            ...
            if (flagInfo !== undefined) {
                ...
                if (valueType === "boolean") {
                    ...
                    try {
                        value = parseValueToken(next, name, parsedFlags, valueType, valueToken);
                    }
                    catch (e) {
                ...
                const argValue = argDef.type === "number"
