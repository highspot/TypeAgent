...
import { toExecutableActions, RequestAction, } from "agent-cache";
...
export async function confirmTranslation(elapsedMs, source, requestAction, context) {
    const actions = requestAction.actions;
    const systemContext = context.sessionContext.agentContext;
    if (!systemContext.developerMode || systemContext.batchMode) {
        const messages = [];
        messages.push(`${source}: ${chalk.blueBright(` ${requestAction.toString()}`)} ${getColorElapsedString(elapsedMs)}`);
        messages.push();
    ...
    const preface = "Use the buttons to run or cancel the following action(s). You can also press [Enter] to run it, [Del] to edit it, or [Escape] to cancel it.";
    ...
    const templateSequence = getActionTemplateEditConfig(systemContext, actions, preface, editPreface);
    const newActions = await systemContext.clientIO.proposeAction(templateSequence, systemContext.requestId, DispatcherName);
    return validateReplaceActions(newActions, systemContext)
        ? {
            requestAction: new RequestAction(requestAction.request, toExecutableActions(newActions), requestAction.history),
            replacedAction: actions,
        }
        : { requestAction };
}
//# sourceMappingURL=confirmTranslation.js.map