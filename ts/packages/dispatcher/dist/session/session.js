...
export function getSessionDirPath(dir) {
    return path.join(getSessionsDirPath(), dir);
}
function getSessionDataFilePath(dir) {
    return path.join(getSessionDirPath(dir), "data.json");
}
export function getSessionCacheDirPath(dir) {
...
async function readSessionData(dir) {
...
export class Session {
    static async create(config = defaultSessionConfig, persist) {
    ...
    static async restoreLastSession() {
    ...
    setCacheDataFilePath(filePath, explainerName) {
        const filePathRel =
            filePath &&
            this.dir &&
    ...
    async storeUserSuppliedFile(file) {
        ...
        const fileExtension = this.getFileExtensionForMimeType(
 ...
 */
export async function setupAgentCache(session, agentCache) {
...
export async function setupBuiltInCache(session, agentCache, enable) {
...
export async function deleteSession(dir) {
    if (dir === "") {
        return;
    }
...
export async function getSessionCaches(dir) {
