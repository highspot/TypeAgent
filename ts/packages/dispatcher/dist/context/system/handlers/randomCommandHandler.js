...
class RandomOfflineCommandHandler {
    constructor() {
        this.description = "Issues a random request from a dataset of pre-generated requests.";
    }
    async run(context) {
        ...
        systemContext.clientIO.notify("randomCommandSelected", systemContext.requestId, {
            message: randomRequest,
        }, DispatcherName);
        systemContext.clientIO.notify(AppAgentEvent.Info, systemContext.requestId, randomRequest, DispatcherName);
...
class RandomOnlineCommandHandler {
    ...
    async run(context) {
        ...
        let chatHistory = [];
        const chat = createTypeChat(chatModel, UserRequestSchema, "UserRequestList", this.instructions, chatHistory, maxContextLength, maxWindowLength, (data) => data.messages.toString());
    ...
    createModel() {
        ...
        const chatModel = openai.createChatModel(apiSettings, completionSettings, undefined, ["randomCommandHandler"]);
        return chatModel;
    }
}
