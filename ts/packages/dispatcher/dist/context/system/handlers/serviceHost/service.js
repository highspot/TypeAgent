...
try {
    ...
    wss.on("connection", (ws, req) => {
        ...
        if (clientId) {
            for (var socket of wss.clients) {
                if (socket.clientId == clientId && socket !== ws) {
                    debug("Closing duplicate socket instance for id " + clientId);
        ...
        ws.on("message", (message) => {
            try {
                ...
                if (!foundAtLeastOneTarget) {
                    const errorMessage = client.role === "client"
    ...
    console.error(`WebSocket server could not be started at ${hostEndpoint}: Error ${e.message}`);
    process.send?.("Failure");
}
