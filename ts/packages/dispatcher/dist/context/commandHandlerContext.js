...
import { ChildLogger, MultiSinkLogger, createDebugLoggerSink, createMongoDBLoggerSink, } from "telemetry";
import { randomUUID } from "crypto";
import { getSessionName, Session, setupAgentCache, setupBuiltInCache, } from "./session.js";
...
import { ensureCacheDir, ensureDirectory, lockInstanceDir, } from "../utils/fsUtils.js";
import { AppAgentEvent } from "@typeagent/agent-sdk";
import { conversation as Conversation } from "knowledge-processor";
import { AppAgentManager, getAppAgentStateSettings, } from "./appAgentManager.js";
...
import { readEmbeddingCache, writeEmbeddingCache, } from "../translation/actionSchemaSemanticMap.js";
...
export function getCommandResult(context) {
    if (context.collectCommandResult) {
        return ensureCommandResult(context);
    }
    return undefined;
}
export function ensureCommandResult(context) {
    if (context.commandResult === undefined) {
        context.commandResult = {};
    }
    return context.commandResult;
}
async function getAgentCache(session, provider, constructionProvider, logger) {
    ...
    const agentCache = cacheFactory.create(explainerName, actionSchemaProvider, session.cacheConfig, logger);
    try {
...
async function getSession(instanceDir) {
    let session;
...
function getLoggerSink(isDbEnabled, clientIO) {
    ...
    let dbLoggerSink;
    try {
        dbLoggerSink = createMongoDBLoggerSink("telemetrydb", "dispatcherlogs", isDbEnabled, (e) => {
            clientIO.notify(AppAgentEvent.Warning, undefined, e, DispatcherName);
        });
    }
    catch (e) {
        clientIO.notify(AppAgentEvent.Warning, undefined, `DB logging disabled. ${e}`, DispatcherName);
    }
    return new MultiSinkLogger(dbLoggerSink === undefined
        ? [debugLoggerSink]
        : [debugLoggerSink, dbLoggerSink]);
}
async function lockEmbeddingCacheDir(context) {
...
async function addAppAgentProviders(context, appAgentProviders) {
    ...
    try {
        if (embeddingCachePath) {
            try {
                ...
                debug(`Action Schema Embedding cache loaded: ${embeddingCachePath}`);
            }
            catch {
                // Ignore error
            }
...
function getEmbeddingCachePath(context) {
...
async function saveActionEmbeddings(context, embeddingCachePath) {
    try {
...
export async function installAppProvider(context, provider) {
...
export async function initializeCommandHandlerContext(hostName, options) {
    ...
    const explanationAsynchronousMode = options?.explanationAsynchronousMode ?? false;
    const persistSession = options?.persistSession ?? false;
    const persistDir = options?.persistDir;
    if (persistDir === undefined && persistSession) {
        throw new Error("Persist session requires persistDir to be set in options.");
    }
    const instanceDirLock = persistDir
        ? await lockInstanceDir(persistDir)
        : undefined;
    try {
        const session = await getSession(persistSession ? persistDir : undefined);
        ...
        const conversationManager = sessionDirPath
            ? await Conversation.createConversationManager({}, "conversation", sessionDirPath, false)
            : undefined;
        ...
        const logger = new ChildLogger(loggerSink, DispatcherName, {
            ...
            sessionId: () => context.session.sessionDirPath
        ...
        const context = {
            ...
            agentCache: await getAgentCache(session, agents, constructionProvider, logger),
            lastActionSchemaName: DispatcherName,
            translatorCache: new Map(),
            currentScriptDir: process.cwd(),
            chatHistory: createChatHistory(session.getConfig().execution.history),
            logger,
            serviceHost,
        ...
        const appAgentStateSettings = getAppAgentStateSettings(options?.agents, agents);
...
async function setAppAgentStates(context) {
    const result = await context.agents.setState(context, context.session.getConfig());
    ...
    const rollback = processSetAppAgentStateResult(result, context, (message) => context.clientIO.notify(AppAgentEvent.Error, undefined, message, DispatcherName));
    if (rollback) {
        context.session.updateConfig(rollback);
    }
}
async function updateAppAgentStates(context) {
    const systemContext = context.sessionContext.agentContext;
    const result = await systemContext.agents.setState(systemContext, systemContext.session.getConfig());
    const rollback = processSetAppAgentStateResult(result, systemContext, (message) => displayError(message, context));
    if (rollback) {
...
function processSetAppAgentStateResult(result, systemContext, cbError) {
    ...
    for (const [stateName, failed] of Object.entries(result.failed)) {
        for (const [translatorName, enable, e] of failed) {
            hasFailed = true;
            const prefix = stateName === "commands"
                ? systemContext.agents.getEmojis()[translatorName]
                : getSchemaNamePrefix(translatorName, systemContext);
            debugError(e);
            cbError(`${prefix}: Failed to ${enable ? "enable" : "disable"} ${stateName}: ${e.message}`);
...
export async function setSessionOnCommandHandlerContext(context, session) {
    ...
    await context.agents.close();
    context.agentCache = await getAgentCache(context.session, context.agents, context.constructionProvider, context.logger);
    await setAppAgentStates(context);
...
export async function changeContextConfig(options, context) {
    ...
    if (changed.explainer?.name !== undefined) {
        try {
            systemContext.agentCache = await getAgentCache(session, systemContext.agents, systemContext.constructionProvider, systemContext.logger);
        }
        catch (e) {
    ...
    if (changed.cache?.enabled !== undefined) {
        ...
        await setupAgentCache(session, agentCache, systemContext.constructionProvider);
    }
    else {
        ...
        if (autoSave !== undefined) {
            ...
            if (autoSave) {
                if (session.getConfig().cache) {
                    const cacheDataFilePath = await session.ensureCacheDataFilePath();
        ...
        if (builtInCache !== undefined) {
            await setupBuiltInCache(session, agentCache, builtInCache, systemContext.constructionProvider);
        }
    }
    return changed;
}
