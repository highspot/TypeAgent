...
function validateReplaceActions(actions, systemContext) {
    if (actions === null) {
...
async function confirmTranslation(elapsedMs, source, requestAction, context) {
    const actions = requestAction.actions;
    const systemContext = context.sessionContext.agentContext;
    if (!systemContext.developerMode || systemContext.batchMode) {
        const messages = [];
        messages.push(
            `${source}: ${chalk.blueBright(` ${requestAction.toString()}`)} ${getColorElapsedString(elapsedMs)}`,
        );
        messages.push();
...
async function matchRequest(request, context, history) {
    const systemContext = context.sessionContext.agentContext;
...
async function translateWithTranslator(
    ...
    const onProperty = config.translation.stream
        ? (prop, value, delta) => {
...
async function finalizeAction(action, translatorName, context, history) {
    let currentAction = action;
    let currentTranslatorName = translatorName;
    const systemContext = context.sessionContext.agentContext;
    while (true) {
...
function getChatHistoryForTranslation(context) {
...
export async function translateRequest(request, context, history, attachments) {
    const systemContext = context.sessionContext.agentContext;
    const config = systemContext.session.getConfig();
...
function canExecute(requestAction, context) {
    const actions = requestAction.actions;
    const systemContext = context.sessionContext.agentContext;
    const unknown = [];
...
async function requestExecute(requestAction, context) {
...
async function canTranslateWithoutContext(
    ...
    const oldActions = requestAction.actions.toFullActions();
    const newActions = [];
