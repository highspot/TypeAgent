...
import { closeCommandHandlerContext, initializeCommandHandlerContext, } from "../src/context/commandHandlerContext.js";
...
export const testCommandAgentProvider = {
    getAppAgentNames: () => {
        return ["test"];
    },
...
describe("Command", () => {
    describe("resolve", () => {
        ...
        it("resolves a command", async () => {
            const command = await resolveCommand("test test param param", context);
        ...
        it("resolves nested command", async () => {
            const command = await resolveCommand("test nested nested param param", context);
            expect(command).toBeDefined();
            expect(command.actualAppAgentName).toStrictEqual("test");
            expect(command.parsedAppAgentName).toStrictEqual("test");
            ...
            expect(command.descriptor).toBe(handlers.commands.nested.commands.nested);
            expect(command.matched).toBe(true);
        });
        it("does not resolve command", async () => {
            const command = await resolveCommand("test nested param param", context);
            expect(command).toBeDefined();
            expect(command.actualAppAgentName).toStrictEqual("test");
            expect(command.parsedAppAgentName).toStrictEqual("test");
        ...
        it("default to system", async () => {
            const command = await resolveCommand("agent nested param param", context);
            expect(command).toBeDefined();
    ...
    describe("parse", () => {
        ...
        it("resolves a command with extra param error", async () => {
            const result = await dispatcher.processCommand("@test test param param");
            expect(result).toBeDefined();
            expect(result.hasError).toBe(true);
            expect(result.exception).toContain("Command '@test test' does not accept parameters.");
        });
        it("resolves a default command with extra param error", async () => {
            const result = await dispatcher.processCommand("@test param param");
            expect(result).toBeDefined();
            expect(result.hasError).toBe(true);
            expect(result.exception).toContain("'param param' is not a subcommand for '@test");
        });
        it("resolves nested command with extra param error", async () => {
            const result = await dispatcher.processCommand("@test nested nested param param");
            expect(result).toBeDefined();
            expect(result.hasError).toBe(true);
            expect(result.exception).toContain("Command '@test nested nested' does not accept parameters.");
        });
        it("does not resolve command with extra param error", async () => {
            const result = await dispatcher.processCommand("@test nested param param");
            expect(result).toBeDefined();
            expect(result.hasError).toBe(true);
            expect(result.exception).toContain("'param param' is not a subcommand for '@test nested'.");
        });
        it("default to system with in valid command error", async () => {
            const result = await dispatcher.processCommand("@agent nested param param");
            expect(result).toBeDefined();
            expect(result.hasError).toBe(true);
            expect(result.exception).toContain("Command or agent name required.");
        });
        it("missing subcommand error", async () => {
            const result = await dispatcher.processCommand("@test nested");
            expect(result).toBeDefined();
            expect(result.hasError).toBe(true);
            expect(result.exception).toContain("'@test nested' requires a subcommand.");
        });
