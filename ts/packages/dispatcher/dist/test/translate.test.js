...
const inputs = await Promise.all(dataFiles.map((f) => readTestData(getPackageFilePath(f))));
...
describe("translation", () => {
    it.each(testInput)("translate [%s] '%s'", async (translatorName, request, action) => {
        const translator = loadAgentJsonTranslator(translatorName, getActionConfigProviderFromDefaultAppAgentProviders());
        ...
        if (result.success) {
            ...
            console.log(`Translation failed: ${request}\n${result.message}`);
        }
    }, 30000);
});
