...
const testAppAgentProvider = createNpmAppAgentProvider({
    test: {
        name: "test-agent",
        path: fileURLToPath(new URL("../../../agents/test", import.meta.url)),
    },
}, import.meta.url);
function createTestClientIO(data) {
    return {
        ...nullClientIO,
        setDisplay: (message) => data.push(message),
        appendDisplay: (message) => data.push(message),
    };
}
describe("dispatcher", () => {
    describe("Custom Provider", () => {
        describe("Command", () => {
            ...
            it("action command", async () => {
                await dispatcher.processCommand('@action test add --parameters \'{"a": 1, "b": 2}\'');
            ...
            it.each(errorCommands)("$name", async ({ command, match }) => {
                ...
                const content = output[0].message;
        ...
        it("Alternate request handler", async () => {
            ...
            await dispatcher.close();
            expect(output).toHaveLength(2);
            expect(output[0].message).toBe("Natural langue request handling agent is set to 'test'");
