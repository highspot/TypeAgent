...
describe("Argument parsing", () => {
    ...
    it("arguments", () => {
        ...
        const params = parseParams(`'${JSON.stringify(l2)}' 10 \t hello   world !`, parameters);
        ...
        const num = args.num;
        const str = args.str;
        const defStr = args.defStr;
        const optional = args.optional; // Use ! to make sure the type is correct
        const optional2 = args.optional2; // Use ! to make sure the type is correct
    ...
    it("arguments - optional multiple - no args", () => {
        ...
        expect(flags).toBe(undefined);
        const args = params.args;
        const single = args.single; // Use ! to make sure the type is correct
        const multiple = args.multiple; // Use ! to make sure the type is correct
    ...
    it("Invalid number value", () => {
        try {
            ...
            expect(e.message).toBe("Invalid number value 'abc' for argument 'num'");
        }
    });
    const invalidJsonTests = [10, [], null, true, false, "hello"];
    it.each(invalidJsonTests)("Invalid json value - %s", (value) => {
        try {
            parseParams(`'${JSON.stringify(value)}' 10 hello world`, parameters);
        }
        catch (e) {
            expect(e.message).toBe("Invalid JSON value for argument 'obj': Not an object");
        }
    });
});
