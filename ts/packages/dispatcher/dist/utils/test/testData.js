...
import { RequestAction, Action, } from "agent-cache";
...
export function getEmptyTestData(schemaName, sourceHash, explainerName) {
    return {
...
export async function readTestData(file) {
...
function getSafeTranslateFn(schemaName, provider, model) {
    const translator = loadAgentJsonTranslator(schemaName, provider, {}, false, false, true, model);
...
function getGenerateTestDataFn(schemaName, provider, explainerName, model) {
    ...
    return async (request, action, history, tags) => {
        ...
        if (action === undefined) {
            ...
            action = isMultipleAction(newActions)
                ? newActions.parameters.requests.map((e) => new Action(schemaName, e.action.actionName, e.action.parameters))
                : new Action(schemaName, newActions.actionName, newActions.parameters);
        }
...
export async function generateTestDataFiles(data, provider, incremental, concurrency = 1, model, overwrite = true) {
...
async function generateTestDataFile(data, provider, incremental, limit, done, model) {
    ...
    const processInput = async (pendingInput) => {
        const { success, elapsedMs, entry } = await generateTestData(pendingInput.request, pendingInput.action?.data, pendingInput.history, pendingInput.tags);
...
export function printTestDataStats(results, prefix = "") {
    // Report total
    let totalSuccess = 0;
    let totalFailed = 0;
    let totalAttempt = 0;
