...
export function readSchemaConfig(schemaFile) {
    const parseSchemaFile = path.parse(getPackageFilePath(schemaFile));
    const schemaConfigFile = path.join(parseSchemaFile.dir, parseSchemaFile.name + ".json");
    return fs.existsSync(schemaConfigFile)
        ? fs.readFileSync(schemaConfigFile, "utf8")
        : undefined;
}
//# sourceMappingURL=loadSchemaConfig.js.map