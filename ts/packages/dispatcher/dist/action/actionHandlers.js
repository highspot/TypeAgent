...
export function getTranslatorPrefix(translatorName, systemContext) {
    const config = systemContext.agents.getActionConfig(translatorName);
    return `[${config.emojiChar} ${translatorName}] `;
}
function getActionContext(appAgentName, systemContext, requestId, actionIndex) {
...
export function createSessionContext(name, agentContext, context) {
...
async function executeAction(action, context, actionIndex) {
...
export function startStreamPartialAction(translatorName, actionName, context) {
    const appAgentName = getAppAgentName(translatorName);
    const appAgent = context.agents.getAppAgent(appAgentName);
