...
import { getCacheFactory, convertTestDataToExplanationData, readExplanationTestData, createActionConfigProvider, createSchemaInfoProvider, } from "agent-dispatcher/internal";
import { RequestAction, getDefaultExplainerName, createActionProps, normalizeParamValue, normalizeParamString, fromJsonActions, toJsonActions, } from "agent-cache";
import { glob } from "glob";
import { fileURLToPath } from "node:url";
import { getDefaultAppAgentProviders } from "../src/defaultAgentProviders.js";
const schemaInfoProvider = createSchemaInfoProvider(await createActionConfigProvider(getDefaultAppAgentProviders(undefined)));
export async function getImportedCache(explainerName, merge = false) {
    const cache = getCacheFactory().create(explainerName, schemaInfoProvider, {
        mergeMatchSets: merge,
        cacheConflicts: true,
    });
    await cache.import(inputs
        .filter(([i]) => i.explainerName === explainerName)
        .map(([testData, fileName]) => convertTestDataToExplanationData(testData, fileName)), true);
    return cache;
}
...
const dataFiles = process.env.TEST_EXTENDED_DATA === "1"
...
const inputs = await Promise.all((await glob(dataFiles)).sort().map(async (f) => {
    return [await readExplanationTestData(f), f];
}));
const testInput = inputs.flatMap(([f]) => f.entries.map((data) => [
    f.schemaName,
    f.explainerName,
...
function getTestInputPart(testFileName) {
    ...
    return {
        testInputPart: selectedTestInput.slice(Math.floor(((index - 1) * selectedTestInput.length) / numParts), Math.floor((index * selectedTestInput.length) / numParts)),
        partString: ` (${index}/${numParts})`,
    };
}
function normalizeParams(action) {
    if (action.parameters !== undefined) {
        action.parameters = JSON.parse(normalizeParamString(JSON.stringify(action.parameters)));
    }
}
...
function expandActions(actions, conflictValues) {
    ...
    for (const [name, values] of conflictValues) {
        expandedActions.push(...expandedActions.flatMap((e) => values.map((v) => createActionProps([[name, normalizeParamValue(v)]], e))));
    }
    return expandedActions;
}
export function defineRoundtripTest(merge, testFileName) {
    ...
    describe("construction cache", () => {
        ...
        describe(`import ${merge ? "merge " : ""}roundtrip${partString}`, () => {
            ...
            it.each(testInputPart)("[%s %s] '%s'", async (translatorName, explainerName, requestAction, explanation, tags) => {
                if (cacheP === undefined) {
                    cacheP = getImportedCache(explainer, merge);
                }
                const cache = await cacheP;
                const matched = cache.constructionStore.match(requestAction.request, {
                    rejectReferences: false,
                    conflicts: true,
                });
                const matchedActions = matched.flatMap((m) => expandActions(m.match.actions, m.conflictValues));
                const expectedActions = normalizeActions(requestAction.actions);
                if (!tags.includes(failedTag)) {
                    ...
                    expect(matchedActions).not.toContainEqual(expectedActions);
                }
            });
        });
    });
}
//# sourceMappingURL=constructionCacheTestCommon.js.map