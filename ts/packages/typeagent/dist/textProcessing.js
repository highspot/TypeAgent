...
exports.stringSimilarity = exports.generateVariationsRecursive = exports.generateVariations = exports.generateList = exports.rewriteText = exports.summarizeWebPage = exports.summarize = exports.generateNotesForWebPage = exports.processTextFromWebPage = exports.htmlToText = exports.extractEntitiesFromLargeText = exports.extractEntities = exports.lookupAnswersOnWeb = exports.lookupAnswersOnWebPages = exports.lookupAnswerOnWebPage = exports.answerQueryFromLargeText = exports.answerQuery = exports.generateAnswerFromWebPages = exports.generateAnswerFromWebPage = exports.generateAnswer = exports.accumulateAnswer = exports.generateNotes = exports.jsonTranslateWebPage = exports.jsonTranslateLargeText = exports.getCompletionOnLargeText = exports.getTextChunks = exports.buildChunks = exports.joinFiles = exports.dedupeLineFile = exports.dedupeList = exports.distinctIterator = exports.setFromList = exports.splitSentenceIntoPhrases = exports.splitIntoParagraphs = void 0;
/**
...
const promptLib_1 = require("./promptLib");
const arrayAsync_1 = require("./arrayAsync");
const _1 = require(".");
...
function splitSentenceIntoPhrases(text) {
    return text.split(/(?<=[,:\-])\s+/);
}
exports.splitSentenceIntoPhrases = splitSentenceIntoPhrases;
/**
 ...
 * @returns
 */
function setFromList(items, caseSensitive = false) {
    return caseSensitive
        ? new Set(items)
        : new Set(items.map((v) => v.toLowerCase()));
}
exports.setFromList = setFromList;
/**
 * Yields a stream of distinct strings
 * @param strings
 * @param caseSensitive
 */
function* distinctIterator(strings, caseSensitive = false) {
    const seen = new Set();
 ...
 * @param caseSensitive
 */
function dedupeList(list, caseSensitive = false) {
 ...
 */
async function dedupeLineFile(filePath, outputFilePath, caseSensitive = false) {
    let lines = await (0, objStream_1.readAllLines)(filePath);
    lines = dedupeList(lines, caseSensitive);
    await (0, objStream_1.writeAllLines)(lines, outputFilePath ?? filePath);
}
exports.dedupeLineFile = dedupeLineFile;
/**
 ...
 * @param separator
 */
async function joinFiles(sourceFiles, outputFilePath, separator = "\n") {
    for (let i = 0; i < sourceFiles.length; ++i) {
        const filePath = sourceFiles[i];
        let text = await (0, objStream_1.readAllText)(filePath);
        if (i > 0) {
            text = separator + text;
        }
        await fs_1.default.promises.appendFile(outputFilePath, text);
    }
}
 ...
 */
function* buildChunks(strings, maxCharsPerChunk, separator = " ") {
    let chunk = "";
 ...
 */
function getTextChunks(text, maxCharsPerChunk) {
 ...
 * @returns
 */
async function getCompletionOnLargeText(text, maxCharsPerChunk, model, instruction, progress) {
    let completions = [];
    ...
    return (0, typechat_1.success)(completions);
    async function complete(request) {
        const prompt = typeof instruction === "string"
            ? instruction + request
            : instruction(request);
...
exports.getCompletionOnLargeText = getCompletionOnLargeText;
/**
 ...
 * @returns
 */
async function* jsonTranslateLargeText(translator, request, text, maxCharsPerChunk) {
...
exports.jsonTranslateLargeText = jsonTranslateLargeText;
/**
 ...
 * @returns
 */
async function* jsonTranslateWebPage(translator, request, webPageUrl, maxChunkSize, progress) {
    const htmlResponse = await (0, aiclient_1.getHtml)(webPageUrl);
    if (!htmlResponse.success) {
        return undefined;
    }
    const html = htmlResponse.data;
    const text = htmlToText(html);
    for await (const result of jsonTranslateLargeText(translator, request, text, maxChunkSize)) {
        yield result;
    }
}
exports.jsonTranslateWebPage = jsonTranslateWebPage;
const notesInstruction = "Generate very concise, bullet point notes for the following:\n";
/**
 ...
 * @returns
 */
function generateNotes(text, maxCharsPerChunk, model, progress) {
    return getCompletionOnLargeText(text, maxCharsPerChunk - notesInstruction.length, model, notesInstruction, progress);
}
exports.generateNotes = generateNotes;
function accumulateAnswer(answer, chunkResponse, url) {
...
exports.accumulateAnswer = accumulateAnswer;
/**
 ...
 * @returns
 */
function generateAnswer(request, text, maxCharsPerChunk, model, concurrency, progress) {
...
`;
    const charsPerchunk = maxCharsPerChunk - (preInstruction.length + postInstruction.length + 1);
    return incrCompletionOnLargeText(text, charsPerchunk, model, preInstruction, postInstruction, concurrency, progress);
    async function incrCompletionOnLargeText(text, maxCharsPerChunk, model, preInstruction, postInstruction, concurrency, progress) {
        let answer = {
            generatedText: "",
            entities: [],
        ...
        for (let i = 0; i < chunks.length; i += concurrency) {
            ...
            if (answer.answerStatus === "Answered") {
                return (0, typechat_1.success)(answer);
            }
        }
        return (0, typechat_1.success)(answer);
 ...
 * @returns
 */
async function generateAnswerFromWebPage(model, webPageUrl, request, maxChunkSize, maxTextLengthToSearch = Number.MAX_SAFE_INTEGER, concurrency, progress) {
    const htmlResponse = await (0, aiclient_1.getHtml)(webPageUrl);
    if (!htmlResponse.success) {
        return undefined;
    }
    ...
    const answer = await generateAnswer(request, text, maxChunkSize, model, concurrency, progress);
    if (answer.success) {
        return answer.data;
    }
    return undefined;
}
exports.generateAnswerFromWebPage = generateAnswerFromWebPage;
/**
 ...
 * @returns
 */
async function generateAnswerFromWebPages(optimizeFor, model, webPageUrls, request, options, concurrency, context, progress) {
    let answer = {
        generatedText: "",
        entities: [],
        urls: [],
    ...
    for (const webPageUrl of webPageUrls) {
        ...
        if (optimizeFor === "Quality") {
            const pageAnswer = await generateAnswerFromWebPage(model, webPageUrl, request, options.maxCharsPerChunk, options.maxTextLengthToSearch, concurrency, (_, answerChunk) => {
            ...
            const pageAnswer = await lookupAnswerOnWebPage(model, { query: request, webPageUrl }, options, concurrency, context, (lookup, i, answerResponse) => {
                answer = accumulateAnswer(answer, toChunkResponse(answerResponse), webPageUrl);
                if (progress) {
                    progress(webPageUrl, answer);
                }
            });
            answer = accumulateAnswer(answer, toChunkResponse(pageAnswer), webPageUrl);
        }
        if (answer.answerStatus === "Answered" ||
    ...
    if (options.rewriteForReadability &&
        optimizeFor === "Quality" &&
        answer.generatedText) {
        await rewritePartialAnswer(options.rewriteModel ?? model, request, answer, options.rewriteFocus);
    }
    return answer;
    function toChunkResponse(answer) {
        let response = {
    ...
    async function rewritePartialAnswer(rewriteModel, lookup, answer, focus) {
        if (answer.generatedText) {
            const rewritten = await rewriteText(rewriteModel, answer.generatedText, lookup, focus);
            if (rewritten) {
...
exports.generateAnswerFromWebPages = generateAnswerFromWebPages;
/**
 ...
 * @returns
 */
function answerQuery(model, query, text, context) {
    const answerSchema = `
    ...
    const request = `Answer the following question ACCURATELY using  ONLY HIGHLY RELEVANT information - if any - from the chat history, using information verbatim when suitable.` +
        `###\n${query}\n###\n`;
    const translator = (0, chat_1.createChatTranslator)(model, answerSchema, "AnswerResponse");
    if (context && context.length > 0) {
        return translator.translate(request, [
            ...context,
            { role: message_1.MessageSourceRole.user, content: text },
        ]);
    }
 ...
 * @param maxCharsPerChunk
 */
async function answerQueryFromLargeText(model, query, text, maxCharsPerChunk, concurrency = 2, rewriteForReadability, rewriteFocus, context, progress) {
    const chunks = getTextChunks(text, maxCharsPerChunk);
    console.log(`Anser Query processing ${chunks.length} text chunks.`);
    const chunkAnswers = await (0, arrayAsync_1.mapAsync)(chunks, concurrency, (chunk) => runChunk(model, query, chunk), chunkProgress);
    ...
    if (rewriteForReadability && answer.type !== "NoAnswer" && answer.answer) {
        const rewritten = await rewriteText(model, answer.answer, query, rewriteFocus);
        if (rewritten) {
    ...
    return answer;
    async function runChunk(model, query, chunk) {
    ...
    function chunkProgress(chunk, index, chunkResponse) {
        if (progress) {
            // Notify progress
    ...
    function accumulateAnswer(answer, latestAnswer, answerType) {
...
exports.answerQueryFromLargeText = answerQueryFromLargeText;
 ...
 * @returns
 */
async function lookupAnswerOnWebPage(model, lookup, options, concurrency = 2, context, progress) {
    const defaultAnswer = emptyAnswer();
    const htmlResponse = await (0, aiclient_1.getHtml)(lookup.webPageUrl);
    if (!htmlResponse.success) {
    ...
    return await answerQueryFromLargeText(model, lookup.query, text, options.maxCharsPerChunk, concurrency, options.rewriteForReadability ?? true, options.rewriteFocus, context, (chunk, index, answer) => {
        if (progress) {
...
exports.lookupAnswerOnWebPage = lookupAnswerOnWebPage;
/**
 ...
 * @returns
 */
async function lookupAnswersOnWebPages(model, query, webPageUrls, options, concurrency = 2, context, progress) {
    let partialAnswer = emptyWebLookupAnswer();
    for (const webPageUrl of webPageUrls) {
        let lookup = { query, webPageUrl };
        const answer = await lookupAnswerOnWebPage(model, lookup, options, concurrency, context, progress);
        if (answer.type === "FullAnswer") {
...
exports.lookupAnswersOnWebPages = lookupAnswersOnWebPages;
/**
 ...
 * @returns
 */
async function lookupAnswersOnWeb(model, query, maxSearchResults, options, concurrency = 2, context, progress) {
    const searchClientResult = await aiclient_1.bing.createBingSearch();
    ...
    const lookupAnswer = await lookupAnswersOnWebPages(model, query, urls, options, concurrency, context, progress);
    return lookupAnswer;
}
exports.lookupAnswersOnWeb = lookupAnswersOnWeb;
function emptyWebLookupAnswer() {
    return {
        webPageUrls: [],
        answer: emptyAnswer(),
    };
 ...
 * @returns
 */
async function extractEntities(model, text) {
...
`;
    const translator = (0, chat_1.createChatTranslator)(model, entitySchema, "EntityResponse");
    const result = await translator.translate(request, [
        (0, promptLib_1.textToProcessSection)(text),
    ]);
    if (!result.success) {
        return [];
    }
    const entities = result.data.type === "NoEntities" ? undefined : result.data.entities;
    return entities ?? [];
}
exports.extractEntities = extractEntities;
async function extractEntitiesFromLargeText(model, text, maxCharsPerChunk, concurrency) {
    const chunks = await getTextChunks(text, maxCharsPerChunk);
    const entityChunks = await (0, arrayAsync_1.mapAsync)(chunks, concurrency, (chunk) => extractEntities(model, chunk));
    const entities = [];
...
exports.extractEntitiesFromLargeText = extractEntitiesFromLargeText;
/**
 ...
 * @returns text
 */
function htmlToText(html, nodeQuery) {
 ...
 * @returns
 */
async function processTextFromWebPage(model, webPageUrl, instruction, maxChunkSize, progress) {
    const htmlResponse = await (0, aiclient_1.getHtml)(webPageUrl);
    if (!htmlResponse.success) {
        return undefined;
    }
    const html = htmlResponse.data;
    const text = htmlToText(html);
    const summary = await getCompletionOnLargeText(text, maxChunkSize, model, instruction, progress);
    if (summary.success) {
        return summary.data.join("\n");
    }
    return undefined;
}
exports.processTextFromWebPage = processTextFromWebPage;
/**
 ...
 * @param progress
 */
async function generateNotesForWebPage(model, webPageUrl, maxChunkSize, progress) {
    return processTextFromWebPage(model, webPageUrl, notesInstruction, maxChunkSize, progress);
}
exports.generateNotesForWebPage = generateNotesForWebPage;
 ...
 * @returns
 */
function summarize(model, text, maxCharsPerChunk, progress) {
    return getCompletionOnLargeText(text, maxCharsPerChunk - notesInstruction.length, model, summarizeInstruction, progress);
}
 ...
 * @returns
 */
async function summarizeWebPage(model, webPageUrl, maxChunkSize, progress) {
    return processTextFromWebPage(model, webPageUrl, summarizeInstruction, maxChunkSize, progress);
}
exports.summarizeWebPage = summarizeWebPage;
/**
 ...
 * @returns
 */
async function rewriteText(model, text, question, rewriteFocus) {
    let prompt = question
 ...
 */
async function generateList(model, listDefinition, context, itemCount) {
...
`;
    const request = itemCount && itemCount > 0
    ...
    const translator = (0, chat_1.createChatTranslator)(model, GenerateListResponseSchema, "GenerateListResponse");
    const result = await translator.translate(request, context);
    const response = (0, typechat_1.getData)(result);
 ...
 */
async function generateVariations(model, seedPhrase, settings) {
    // Process next phrase
    const preamble = [];
    let listDef;
    if (settings.schema) {
 ...
 * @returns A list of variations
 */
async function generateVariationsRecursive(model, seedPhrase, settings, depth = 1, progress) {
    if (depth <= 1) {
    ...
    while (newPhrases.length > 0) {
        ...
        while ((phrase = pendingPhrases.pop()) !== undefined) {
            const variations = await generateVariations(model, phrase, settings);
...
exports.generateVariationsRecursive = generateVariationsRecursive;
async function stringSimilarity(model, x, y) {
    if (x && y) {
        if (x === y) {
            return 1.0;
        }
        const embeddings = await (0, _1.generateTextEmbeddings)(model, [x, y]);
        return (0, _1.similarity)(embeddings[0], embeddings[1], _1.SimilarityType.Dot); // Embeddings are normalized
    }
