...
import { createActionConfigProvider, getSchemaNamesForActionConfigProvider, } from "agent-dispatcher/internal";
...
class TranslateCommand extends Command {
    async run() {
        ...
        await withConsoleClientIO(async (clientIO) => {
            ...
            try {
                await dispatcher.processCommand(`@dispatcher translate ${args.request}`);
            }
            finally {
...
TranslateCommand.args = {
    request: Args.string({
        description: "Request to translate and get and explanation of the translation",
        required: true,
    }),
};
...
TranslateCommand.description = "Translate a request into action";
TranslateCommand.example = [
