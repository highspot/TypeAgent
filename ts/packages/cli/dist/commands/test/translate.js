...
import { readExplanationTestData, getSchemaNamesForActionConfigProvider, createActionConfigProvider, } from "agent-dispatcher/internal";
...
function summarizeResult(result) {
    ...
    for (const entry of result.pass) {
        ...
        if (entry.actions.every((actions) => JSON.stringify(actions) === expected)) {
            exact++;
        }
    }
    let inconsistent = new Map();
    for (const entry of result.fail) {
        if (entry.actions === undefined) {
            continue;
        }
        const actionNames = entry.actions.map((actions) => actions
    ...
    console.log(`Passed: ${passed} (${((passed / total) * 100).toFixed(2)}%) [Exact: ${exact} (${((exact / total) * 100).toFixed(2)}%)]`);
    console.log(`Failed: ${failed} (${((failed / total) * 100).toFixed(2)}%)`);
    const inconsistentSorted = Array.from(inconsistent.entries()).sort((a, b) => b[1] - a[1]);
...
function getTokenUsageStr(usage, count = 1) {
...
class TestTranslateCommand extends Command {
    async run() {
        ...
        if (flags.summarize) {
            if (argv.length !== 0) {
                throw new Error("No files should be specified when summarizing result");
            }
            const result = JSON.parse(fs.readFileSync(flags.summarize, "utf-8"));
        ...
        if (flags.input) {
            if (argv.length !== 0) {
                throw new Error("No files should be specified when using --input flags");
            }
            const input = JSON.parse(fs.readFileSync(flags.input, "utf-8"));
            if (input.pass.length === 0 && input.fail.length === 0) {
                throw new Error("Result file is empty. No tests to rerun.");
            }
            const includeAll = !flags.succeeded && !flags.failed && !flags.skipped;
            if (includeAll) {
            ...
            if (includeAll || flags.succeeded) {
                requests = requests.concat(input.pass.map((entry) => entry.request));
            }
            else {
                output.pass = input.pass;
            }
            if (includeAll || flags.failed) {
                requests = requests.concat(input.fail.map((entry) => entry.request));
            }
            else {
            ...
            requests = inputs
                .flatMap((input) => input.data.entries.map((e) => ({
                request: e.request,
        ...
        if (flags.sample !== undefined) {
            ...
            while (flags.sample < requests.length) {
                output.skipped.push(...requests.splice(Math.floor(Math.random() * requests.length), 1));
            }
        ...
        function print(msg) {
            processed++;
            console.log(`${chalk.white(`[${processed.toString().padStart(totalStr.length)}/${totalStr}]`)} ${chalk.yellow(`[Fail: ${failedTotal.toString().padStart(totalStr.length)} (${((failedTotal / processed) * 100).toFixed(2).padStart(5)}%)]`)} ${msg}`);
        }
        const concurrency = flags.concurrency ?? 4;
        console.log(`Starting ${countStr} tests (concurrency: ${concurrency}, repeat: ${repeat})`);
        ...
        async function worker() {
            ...
            while (requests.length > 0) {
                ...
                for (let i = 0; i < repeat; i++) {
                    ...
                    const commandResult = await dispatcher.processCommand(request);
                    ...
                    if (tokenUsage) {
                        ...
                        if (tokenUsage.total_tokens > maxTokenUsage.total_tokens) {
                ...
                for (let i = 1; i < results.length; i++) {
                    ...
                    if (expected === undefined) {
                        ...
                        failedTotal++;
                        failed = true;
                        print(chalk.red(`Failed to consistently generate actions`));
                        break;
                    }
                    if (actual === undefined) {
                        failedTotal++;
                    ...
                    if (actual.length !== expected.length) {
                        failedTotal++;
                        failed = true;
                        print(chalk.red(`Failed (number of actions, actual: ${actual?.length}, expected: ${expected.length})`));
                        break;
                    }
                    for (let i = 0; i < actual.length; i++) {
                        if (actual[i].translatorName !==
                            expected[i].translatorName ||
                            actual[i].actionName !== expected[i].actionName) {
                            print(chalk.red(`Failed (${actual[i].translatorName}.${actual[i].actionName}) !== (${expected[i].translatorName}.${expected[i].actionName})`));
                            failedTotal++;
                            failed = true;
                            break;
                        }
                ...
                if (!failed) {
                    ...
                    const avgTokenStr = getTokenUsageStr(currentTokenUsage, repeat);
                    const maxTokenStr = getTokenUsageStr(maxTokenUsage);
                    const tokenStr = repeat === 1
                        ? `(Token: ${avgTokenStr})`
                        : avgTokenStr === maxTokenStr
                            ? `(Token Avg: ${avgTokenStr})`
                            : `(Token Avg: ${avgTokenStr} Max: ${maxTokenStr})`;
                    print(`${chalk.green(msg)} ${chalk.grey(timeStr)} ${tokenStr}`);
                }
            }
        ...
        const totalData = output.pass.length +
            output.fail.length +
            (output.skipped?.length ?? 0);
        const totalDataStr = totalData.toString();
        const numberLength = totalDataStr.length;
        function printPart(name, count, total) {
            if (count > 0) {
                console.log(`${name.padEnd(15)}: ${count.toString().padStart(numberLength)} (${((count / total) * 100).toFixed(2)}%)`);
            }
        }
        ...
        console.log(`Total          : ${processed.toString().padStart(numberLength)}`);
        ...
        console.log(`Elapsed Time: ${getElapsedString(elapsed)}, Avg: ${getElapsedString(elapsedPerRequest)} (${getElapsedString(elapsedPerCall)}/call)`);
        const executionTimePerRequest = totalExecTime / processed;
        const executionTimePerCall = executionTimePerRequest / repeat;
        console.log(`Execution Time: ${getElapsedString(totalExecTime)}, Avg: ${getElapsedString(executionTimePerRequest)} (${getElapsedString(executionTimePerCall)}/call) Max: ${getElapsedString(maxExecTime)}`);
        console.log(`Token Usage: ${getTokenUsageStr(totalTokenUsage)}, Avg per call: ${getTokenUsageStr(totalTokenUsage, processed * repeat)}`);
    }
}
TestTranslateCommand.args = {
    files: Args.string({
        files: Args.string({
            description: "List of test data files. Default to all test files in the config.json.",
        }),
    }),
};
TestTranslateCommand.flags = {
    schema: Flags.string({
    ...
    switchSearch: Flags.boolean({
        description: "Enable second chance full switch schema to find schema group",
        default: true, // follow DispatcherOptions default
        allowNo: true,
    }),
    ...
    succeeded: Flags.boolean({
        description: "Copy failed test data and rerun only successful tests from the test result file",
    }),
    failed: Flags.boolean({
        description: "Copy pass test data and rerun only failed tests from the test result file",
    }),
    skipped: Flags.boolean({
        description: "Copy skipped test data and rerun only skipped tests from the test result file",
    }),
    input: Flags.string({
        char: "i",
...
export default TestTranslateCommand;
