...
class Prompt extends Command {
    async run() {
        ...
        const model = openai.createChatModel(flags.model, flags.json
            ? {
        ...
        const request = (fs.existsSync(args.request)
        ...
        try {
            if (flags.stream) {
                ...
                console.log();
                console.log(`${number_chunks} chunks streamed in ${end - start}ms (first response in ${first - start}ms)`);
                console.log();
                time = end - start;
            }
            else {
                ...
                if (complete.success) {
                    ...
                    if (!flags.json) {
                        console.log(chalk.green(`Full GPT response: (${end - start}ms)`));
...
Prompt.description = "Send a prompt to GPT";
Prompt.args = {
    request: Args.string({
