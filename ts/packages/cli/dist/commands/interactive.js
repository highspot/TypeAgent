...
import { getDefaultAppAgentProviders, getDefaultConstructionProvider, getDefaultAppAgentInstaller, } from "default-agent-provider";
import inspector from "node:inspector";
import { getChatModelNames } from "aiclient";
import { processCommands, withConsoleClientIO, } from "agent-dispatcher/helpers/console";
    ...
    async run() {
        ...
        await withConsoleClientIO(async (clientIO) => {
            ...
            try {
                ...
                await processCommands(() => dispatcher.getPrompt(), (command, dispatcher) => dispatcher.processCommand(command), dispatcher);
            }
            finally {
                if (dispatcher) {
...
Interactive.description = "Interactive mode";
Interactive.flags = {
    agent: Flags.string({
...
Interactive.args = {
    input: Args.file({
        description: "A text input file containing one interactive command per line",
        exists: true,
    }),
};
