...
import { getEmptyExplanationTestData, readExplanationTestData, } from "agent-dispatcher/internal";
import fs from "node:fs";
import path from "node:path";
class ExplanationDataSplitCommmand extends Command {
    async run() {
        ...
        for (const file of files) {
            ...
            if (total <= flags.limit) {
                this.log(chalk.yellow(`${file}: ${total} entries found. No need to split.`));
                continue;
            }
            const splitData = [];
            let currEntry = 0;
            let currFailed = 0;
            while (currEntry < data.entries.length ||
                currFailed < failedCount) {
                const newData = getEmptyExplanationTestData(data.schemaName, data.sourceHash, data.explainerName);
            ...
            for (let i = 0; i < splitData.length; i++) {
                ...
                await fs.promises.writeFile(fileName, JSON.stringify(split, undefined, 2));
            }
            this.log(chalk.green(`${file}: ${total} entries found. Split into ${splitData.length} files.`));
        }
    }
}
ExplanationDataSplitCommmand.strict = false;
ExplanationDataSplitCommmand.args = {
    files: Args.string({
        description: "List of explanation data files.",
    }),
};
ExplanationDataSplitCommmand.flags = {
    limit: Flags.integer({
        char: "l",
...
export default ExplanationDataSplitCommmand;
