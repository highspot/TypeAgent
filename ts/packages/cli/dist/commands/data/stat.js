...
import { getCacheFactory, readExplanationTestData, getSchemaNamesForActionConfigProvider, createActionConfigProvider, } from "agent-dispatcher/internal";
import { getInstanceDir } from "agent-dispatcher/helpers/data";
import path from "node:path";
import { getDefaultAppAgentProviders, getDefaultConstructionProvider, } from "default-agent-provider";
function commonPathPrefix(s) {
    const paths = s.map((str) => str.split(path.sep));
    let index = 0;
    while (paths.every((p) => index < p.length && p[index] === paths[0][index])) {
        index++;
    }
...
function printStats(name, stats, options) {
    if (stats.size === 0) {
        return;
    }
    const keyWidth = Math.min(80, Math.max(...Array.from(stats.keys()).map((k) => k.length + 1)));
    const columns = Array.from(new Set(Array.from(stats.values()).flatMap((v) => Object.keys(v))).values());
    if (columns.length === 0) {
        return;
    }
    ...
    const fullData = data.map(([row, values]) => {
        const fullValues = {};
        let rowTotal = 0;
    ...
    const getValueStr = (row, value, total) => {
    ...
    const columnTotal = options?.columnTotal !== false ? {} : undefined;
    const printRow = (row, values) => {
        const items = [row.slice(0, keyWidth).padEnd(keyWidth)];
        for (const column of columns) {
            items.push(getValueStr(row, values[column], columnRatioTotal?.[column]));
            if (columnTotal) {
                columnTotal[column] =
...
const schemaNames = getSchemaNamesForActionConfigProvider(await createActionConfigProvider(getDefaultAppAgentProviders(getInstanceDir())));
class ExplanationDataStatCommmand extends Command {
    async run() {
        ...
        const collectOneStat = (statsMap, key, field, value = 1) => {
            const stats = statsMap.get(key) ?? {};
            stats[field] = (stats[field] ?? 0) + value;
            statsMap.set(key, stats);
        };
        const errorStats = new Map();
        const correctStats = new Map();
        const summaryStats = new Map();
        const collectOneCorrectionStats = (correction, succeeded) => {
            const normalize = correction
                .replace(/^(Parameter|Property) ['A-Za-z0-9.]+ has /, "$1 <string> has ")
                .replace(/ '.*', which/, " <string>, which")
                .replace(/ '.*', /, " <string>, ")
                .replace(/ '.*' for (parameter|property)/, " <string> for $1")
                .replace(/end of request '.*'$/, "end of request <string>")
                .replace(/explanation has '.*' not found/, "explanation has <string> not found")
                .replaceAll(/ '.*?'([ ,]|$)/g, " <string>$1")
                .replaceAll(/ -?\d+ /g, " <number> ")
                .replaceAll(/in the substring '.*'./g, "in the substring <string> ");
            collectOneStat(correctStats, normalize, succeeded ? "Succ" : "Failed");
        };
        const collectCorrectionStats = (corrections, succeeded) => {
            for (const { correction } of corrections) {
                if (Array.isArray(correction)) {
                    correction.forEach((c) => collectOneCorrectionStats(c, succeeded));
                }
                else {
        ...
        if (showFiles) {
            console.log(`${chalk.cyan(`File stats: (${path.relative(process.cwd(), basePath)})`.padEnd(70))} |  Total |   Succ | Failed (Rate)`);
            console.log("-".repeat(100));
        }
        const printFileStat = (file, entries, failed) => {
            const total = entries + failed;
            console.log(`${chalk.cyanBright(file.padEnd(70))} | ${total.toString().padStart(6)} | ${entries.toString().padStart(6)} | ${failed.toString().padStart(6)} (${((failed / total) * 100).toFixed(2).padStart(5)}%)`);
        };
        for (const file of files) {
            try {
                const data = await readExplanationTestData(file);
                if ((flags.schema && !flags.schema.includes(data.schemaName)) ||
                    (flags.explainer &&
                ...
                if (flags.succeeded || !flags.failed) {
                    for (const entry of data.entries) {
                        collectOneStat(summaryStats, "Retries", data.explainerName, (entry.corrections?.length ?? 0) + 1);
                ...
                if (flags.failed || !flags.succeeded) {
                    if (data.failed !== undefined) {
                        for (const entry of data.failed) {
                            collectOneStat(summaryStats, entry.action === undefined
                                ? "  Translation"
                                : "  Explanation", data.explainerName);
                            collectOneStat(summaryStats, "Retries", data.explainerName, (entry.corrections?.length ?? 0) + 1);
                            if (showCorrections && entry.corrections) {
                                collectCorrectionStats(entry.corrections, false);
                            }
                            if (showErrors) {
                                const message = entry.message
                                    ...
                                    .slice(0, 2)
                                    .join(":");
                                collectOneStat(errorStats, message, data.explainerName);
                            }
                        }
                    }
                }
                ...
                collectOneStat(summaryStats, "Entries", data.explainerName, entries + failed);
                collectOneStat(summaryStats, "Failed", data.explainerName, failed);
                fileCount++;
                if (showFiles) {
...
ExplanationDataStatCommmand.strict = false;
ExplanationDataStatCommmand.args = {
    files: Args.string({
        description: "List of explanation data files. Default to all test files in the config.json.",
    }),
};
ExplanationDataStatCommmand.flags = {
    schema: Flags.string({
...
export default ExplanationDataStatCommmand;
