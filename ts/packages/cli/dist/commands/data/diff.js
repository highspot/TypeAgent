...
class ExplanationDataDiffCommand extends Command {
    async run() {
        ...
        const p = [
            fs.promises.writeFile(prevFile, JSON.stringify(prevData, undefined, 2)),
            fs.promises.writeFile(currFile, JSON.stringify(currData, undefined, 2)),
        ];
        await Promise.all(p);
...
ExplanationDataDiffCommand.args = {
    file: Args.string({
...
ExplanationDataDiffCommand.description = "Regenerate the data in the test data file";
ExplanationDataDiffCommand.example = [
    `$ <%= config.bin %> <%= command.id %> data.json [...<files>]`,
];
export default ExplanationDataDiffCommand;
