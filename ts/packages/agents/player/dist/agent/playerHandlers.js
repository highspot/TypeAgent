...
import { getClientContext, handleCall, } from "music";
...
import { resolveArtists, searchArtists, toQueryString, } from "../search.js";
...
async function executePlayerAction(action, context) {
    if (context.sessionContext.agentContext.spotify) {
        return handleCall(action, context.sessionContext.agentContext.spotify, context.actionIO);
    }
    return createActionResultFromError("Action translated but not performed. Spotify integration is not enabled.");
}
async function updatePlayerContext(enable, context) {
    if (enable) {
        const user = await enableSpotify(context);
        context.notify(AppAgentEvent.Info, chalk.blue(`Spotify integration enabled. Logged in as ${user}.`));
    }
    else {
...
async function enableSpotify(context) {
...
async function validatePlayerWildcardMatch(action, context) {
    ...
    switch (action.actionName) {
        case "playTrack":
            return validateTrack(action.parameters.trackName, action.parameters.artists, action.parameters.albumName, clientContext);
        case "playAlbum":
            return await validateAlbum(action.parameters.albumName, action.parameters.artists, clientContext);
        case "playArtist":
            return await validateArtist(action.parameters.artist, clientContext);
        case "playGenre":
            return false;
    }
    return true;
}
async function validateTrack(trackName, artists, album, context) {
    ...
    if (tracks) {
        ...
        return tracks
            .getTracks()
            .some((track) => track.name.toLowerCase().includes(lowerCaseTrackName) &&
...
async function validateAlbum(albumName, artists, context) {
    const resolvedArtists = artists
        ? await resolveArtists(artists, context)
        : [];
    if (resolvedArtists === undefined) {
    ...
    return resolvedArtists.every((resolvedArtist) => album.artists.some((artists) => artists.id === resolvedArtist.id));
}
async function validateArtist(artistName, context) {
    ...
    if (data && data.artists && data.artists.items.length > 0) {
        ...
        return data.artists.items.some((artist) => artist.name.toLowerCase().includes(lowerCaseArtistName));
    }
    return false;
}
async function getPlayerDynamicDisplay(type, displayId, context) {
    ...
    if (displayId === "status") {
        ...
        return {
            content: type === "html" ? status.displayContent : status.literalText,
            nextRefreshMs: 1000,
        };
    }
    throw new Error(`Invalid displayId ${displayId}`);
}
async function getPlayerActionCompletion(action, propertyName, context) {
    const clientContext = context.agentContext.spotify;
    if (clientContext === undefined) {
        return [];
    }
