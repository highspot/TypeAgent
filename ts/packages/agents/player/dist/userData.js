...
import { getFavoriteAlbums, getFavoriteTracks, getFollowedArtists, getRecentlyPlayed, getTopUserArtists, getTopUserTracks, } from "./endpoints.js";
import registerDebug from "debug";
const debugData = registerDebug("typeagent:spotify:data");
function getUserDataFilePath() {
    return "userdata.json";
}
async function loadUserData(instanceStorage) {
...
export async function saveUserData(storage, userData) {
    const json = {
...
export function mergeUserDataKind(existing, newItems) {
    let added = 0;
...
function mergeUserData(userData, tracks, artists, albums) {
    return [
        tracks
            ? mergeUserDataKind(userData.tracks, tracks.map((t) => trackToJSON(t)))
            : 0,
        artists
            ? mergeUserDataKind(userData.artists, artists.map((a) => artistToJSON(a)))
            : 0,
        albums
            ? mergeUserDataKind(userData.albums, albums.map((a) => albumToJSON(a)))
            : 0,
    ];
}
function mergeTracksWithTimestamps(userData, tracks) {
    let added = 0;
...
export function addUserDataStrings(userData) {
...
async function updateUserData(storage, service, userData) {
    try {
        debugData("Updating user data");
        const [favoriteTracks, favoriteAlbums, followedArtists, topArtists, topTracks, recentlyPlayed,] = await Promise.all([
        ...
        if (favoriteTracks) {
            ...
            updates["saved tracks"] = [
                tracks.length,
                ...mergeUserData(userData, tracks, tracks.flatMap((t) => t.artists)),
            ];
        }
        if (favoriteAlbums) {
        ...
        if (debugData.enabled) {
            ...
            messages.push([undefined, userData.tracks, userData.artists, userData.albums]
...
export async function initializeUserData(instanceStorage, service) {
    debugData("Loading saved user data");
    const data = await loadUserData(instanceStorage);
    const result = { data };
    debugData(`Tracks: ${data.tracks.size}, Artists: ${data.artists.size}, Albums: ${data.albums.size}`);
    // Update once a day
    const update = async () => {
