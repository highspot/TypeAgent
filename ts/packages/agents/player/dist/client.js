...
import { AlbumTrackCollection, PlaylistTrackCollection, TrackCollection, } from "./trackCollections.js";
import { applyFilterExpr } from "./trackFilter.js";
import { play, getUserProfile, getDevices, search, setVolume, limitMax, getFavoriteTracks, createPlaylist, deletePlaylist, getPlaylists, getPlaybackState, getPlaylistTracks, pause, next, previous, shuffle, getQueue, getAlbum, } from "./endpoints.js";
import { htmlStatus, listAvailableDevices, printStatus, selectDevice, } from "./playback.js";
import { SpotifyService } from "./service.js";
import { fileURLToPath } from "node:url";
import { hostname } from "node:os";
import { initializeUserData, mergeUserDataKind, saveUserData, addUserDataStrings, } from "./userData.js";
import registerDebug from "debug";
import { createActionResultFromHtmlDisplay, createActionResultFromTextDisplay, } from "@typeagent/agent-sdk/helpers/action";
import { equivalentNames, findAlbums, findArtistTopTracks, findArtistTracksWithGenre, findTracks, findTracksWithGenre, getTrackFromEntity, } from "./search.js";
...
async function printTrackNames(trackCollection, context) {
...
export async function loadHistoryFile(instanceStorage, historyPath, context) {
    ...
    try {
        ...
        mergeUserDataKind(context.userData.data.tracks, data.map((r) => ({
            timestamps: [r.ts],
            freq: 1,
...
async function htmlTrackNames(trackCollection, headText = "Tracks") {
    const selectedTracks = trackCollection.getTracks();
    ...
    if (selectedTracks.length > 1) {
        ...
        for (const track of selectedTracks) {
            ...
            const artistsPrefix = track.artists.length > 1 ? "   Artists: " : "   Artist: ";
            const artists = artistsPrefix +
        ...
        const litArtists = litArtistsPrefix +
            track.artists.map((artist) => artist.name).join(", ");
...
async function updateTrackListAndPrint(collection, clientContext) {
...
export async function getClientContext(instanceStorage) {
    const service = new SpotifyService(await createTokenProvider(instanceStorage));
    await service.init();
...
function internTrackCollection(trackCollection, clientContext) {
...
export async function searchTracks(queryString, context) {
    const query = {
        q: queryString,
        type: "track",
        limit: 50,
...
export async function searchAlbum(albumName, context) {
    const query = {
        q: `album:"${albumName}"`,
        type: "album",
        limit: 50,
...
async function playTrackCollection(trackCollection, clientContext) {
    if (clientContext.deviceId) {
        ...
        await play(clientContext.service, clientContext.deviceId, uris, trackCollection.getContext());
        return actionResult;
    }
    else {
...
async function playRandomAction(clientContext, action) {
...
async function playTrackAction(clientContext, action) {
    if (action.parameters.albumName) {
        return playTrackWithAlbum(clientContext, action);
    }
    const tracks = await findTracks(clientContext, action.parameters.trackName, action.parameters.artists, action.entities?.trackName, action.entities?.artists, 3);
...
function isTrackMatch(track, albumName, artists) {
    ...
    return artists.every((artist) => track.album.artists.some((albumArtist) => equivalentNames(artist, albumArtist.name)));
}
async function playTrackWithAlbum(clientContext, action) {
    const albumName = action.parameters.albumName;
    const trackName = action.parameters.trackName;
    const trackFromEntity = await getTrackFromEntity(trackName, clientContext, action.entities?.trackName);
    if (trackFromEntity !== undefined) {
        if (isTrackMatch(trackFromEntity, albumName, action.parameters.artists)) {
            return playTrackCollection(new TrackCollection([trackFromEntity]), clientContext);
        }
    }
    const albums = await findAlbums(albumName, action.parameters.artists, clientContext, action.entities?.albumName, action.entities?.artists);
    if (trackFromEntity !== undefined) {
    ...
    for (const album of albums) {
        ...
        if (track !== undefined) {
            return playTrackCollection(new TrackCollection([toTrackObjectFull(track, album)]), clientContext);
        }
    }
    // Even though we search thru all the possible matched albums, just use the first one
    return createErrorActionResult(`Track ${trackName} not found in album ${albums[0].name}`);
}
async function playFromCurrentTrackListAction(clientContext, action) {
    ...
    if (trackList) {
        ...
        if (trackIndex < tracks.length) {
            return playTrackCollection(new TrackCollection([tracks[trackIndex]]), clientContext);
        }
        else {
            return createErrorActionResult(`Track number ${action.parameters.trackNumber} not found in current track list`);
        }
    }
    else {
...
async function playAlbumAction(clientContext, action) {
    const albums = await findAlbums(action.parameters.albumName, action.parameters.artists, clientContext, action.entities?.albumName, action.entities?.artists);
    const album = albums[0];
    if (action.parameters.trackNumber === undefined) {
        return playTrackCollection(new AlbumTrackCollection(album), clientContext);
    }
    const tracks = [];
    for (const trackNumber of action.parameters.trackNumber) {
        const track = album.tracks.items.find((track) => track.track_number === trackNumber);
        if (track === undefined) {
            return createErrorActionResult(`Track number ${trackNumber} not found in album ${album.name}`);
        }
        tracks.push(track);
    }
    return playTrackCollection(new TrackCollection(tracks.map((track) => toTrackObjectFull(track, album))), clientContext);
}
async function playArtistAction(clientContext, action) {
    const tracks = await (action.parameters.genre
        ? findArtistTracksWithGenre(action.parameters.artist, action.parameters.genre, clientContext, action.entities?.artist)
        : findArtistTopTracks(action.parameters.artist, clientContext, action.entities?.artist));
    const quantity = action.parameters.quantity ?? 0;
    if (quantity > 0) {
...
async function playGenreAction(clientContext, playAction) {
    const tracks = await findTracksWithGenre(clientContext, playAction.parameters.genre, playAction.parameters.quantity);
    const collection = new TrackCollection(tracks);
    return playTrackCollection(collection, clientContext);
}
function ensureClientId(state) {
    if (state.device.id !== null) {
        return state.device.id;
    }
    console.log(chalk.red(`Current device '${state.device.name}' is a restricted device and cannot be controlled.`));
    return undefined;
}
async function resumeActionCall(clientContext) {
...
async function pauseActionCall(clientContext) {
    const state = await getPlaybackState(clientContext.service);
    if (!state) {
...
async function nextActionCall(clientContext) {
    const state = await getPlaybackState(clientContext.service);
    if (!state) {
...
async function previousActionCall(clientContext) {
    const state = await getPlaybackState(clientContext.service);
    if (!state) {
        return createWarningActionResult("No active playback to move previous to");
    }
    const deviceId = ensureClientId(state);
    if (deviceId === undefined) {
        return createWarningActionResult("No device active");
...
async function shuffleActionCall(clientContext, on) {
    const state = await getPlaybackState(clientContext.service);
    if (!state) {
...
export function getUserDataStrings(clientContext) {
    return clientContext.userData
        ? Array.from(addUserDataStrings(clientContext.userData.data).keys())
        : [];
}
export async function handleCall(action, clientContext, actionIO) {
    switch (action.actionName) {
        ...
        case "getQueue": {
            ...
            if (currentQueue) {
                const filtered = currentQueue.queue.filter((item) => item.type === "track");
        ...
        case "shuffle": {
            const shuffleAction = action;
            return shuffleActionCall(clientContext, shuffleAction.parameters.on);
        }
        case "resume": {
        ...
        case "selectDevice": {
            const selectDeviceAction = action;
            ...
            if (result) {
                ...
                return createErrorActionResult("No devices found");
        }
        case "setVolume": {
            const setVolumeAction = action;
            let newVolumeLevel = setVolumeAction.parameters.newVolumeLevel;
            if (newVolumeLevel > 50) {
                newVolumeLevel = 50;
            }
            actionIO.setDisplay(chalk.yellowBright(`setting volume to ${newVolumeLevel} ...`));
            await setVolume(clientContext.service, newVolumeLevel);
            return htmlStatus(clientContext);
        }
        case "changeVolume": {
            const changeVolumeAction = action;
            const volumeChangeAmount = changeVolumeAction.parameters.volumeChangePercentage;
            ...
            if (playback && playback.device) {
                ...
                let nv = Math.floor((1.0 + volumeChangeAmount / 100.0) * volpct);
                if (nv > 50) {
                    nv = 50;
                }
                actionIO.setDisplay(chalk.yellowBright(`setting volume to ${nv} ...`));
        ...
        case "searchTracks": {
            const searchTracksAction = action;
        ...
        case "getPlaylist": {
            const getPlaylistAction = action;
            ...
            if (playlist) {
                const playlistResponse = await getPlaylistTracks(clientContext.service, playlist.id);
                if (playlistResponse) {
                    const collection = new PlaylistTrackCollection(playlist, playlistResponse.items.map((item) => item.track));
        ...
        case "getAlbum": {
            ...
            if (album !== undefined) {
                const fullAlbumRsponse = await getAlbum(clientContext.service, album.id);
                if (status !== undefined &&
                    status.is_playing &&
                    status.item !== null &&
                    status.item.type === "track") {
                    await play(clientContext.service, clientContext.deviceId, [], album.uri, status.item.track_number - 1, status.progress_ms ? status.progress_ms : 0);
                }
                if (fullAlbumRsponse) {
                    const collection = new AlbumTrackCollection(fullAlbumRsponse);
                    actionIO.setDisplay(chalk.magentaBright(`${album.name}:`));
                    await updateTrackListAndPrint(collection, clientContext);
                    return htmlTrackNames(collection);
                }
                return createNotFoundActionResult(`tracks from album ${album.name}`);
            }
            return createNotFoundActionResult("album");
        }
        case "getFavorites": {
            const getFavoritesAction = action;
        ...
        case "filterTracks": {
            ...
            if (trackListEntity) {
                console.log(`entity id: ${action.parameters.trackListEntityId}`);
                if (trackListEntity !== undefined &&
                    trackListEntity.type.includes("track-list") &&
                    trackListEntity.uniqueId) {
                    const trackList = clientContext.trackListMap.get(trackListEntity.uniqueId);
                    if (trackList) {
                        input = trackList;
                    }
                }
            ...
            if (input) {
                let filterType = action.parameters.filterType;
                ...
                if (parseResult.ast) {
                    ...
                    const tracks = await applyFilterExpr(clientContext, getTypeChatLanguageModel(), parseResult.ast, trackList, negate);
                    const collection = new TrackCollection(tracks);
        ...
        case "createPlaylist": {
            ...
            if (trackListEntity) {
                console.log(`entity id: ${action.parameters.trackListEntityId}`);
                if (trackListEntity.type.includes("track-list") &&
                    trackListEntity.uniqueId) {
                    const trackList = clientContext.trackListMap.get(trackListEntity.uniqueId);
                    if (trackList) {
                        input = trackList;
            ...
            if (input !== undefined) {
                ...
                await createPlaylist(clientContext.service, name, clientContext.service.retrieveUser().id, uris, name);
                ...
                let displayText = "";
                if (actionResult.displayContent &&
                ...
                return createActionResultFromHtmlDisplay(`<div>playlist ${name} created with tracks...</div>${displayText}`);
            }
            return createErrorActionResult("no input tracks for createPlaylist");
        }
        case "deletePlaylist": {
            const deletePlaylistAction = action;
            ...
            if (playlist !== undefined) {
                ...
                return createActionResultFromTextDisplay(chalk.magentaBright(`playlist ${playlist.name} deleted`));
            }
            return createErrorActionResult(`playlist ${playlistName} not found`);
        }
        default:
            return createErrorActionResult(`Action not supported: ${action.actionName}`);
    }
}
