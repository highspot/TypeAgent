...
const pauseSymbol = "⏸️";
const playSymbol = "▶️";
export function chalkStatus(status) {
    let result = "";
    if (status.item) {
...
function htmlPlaybackStatus(status, actionResult) {
    let displayHTML = "";
    if (status.item) {
        let timePart = msToElapsedMinSec(status.item.duration_ms);
        if (status.progress_ms) {
...
export async function htmlStatus(context) {
    ...
    if (devices && devices.devices.length > 0) {
        ...
        if (activeDevice) {
            ...
            for (const device of devices.devices) {
                console.log(chalk.magenta(`   Device ${device.name} of type ${device.type} is available`));
            }
        }
    }
}
export async function selectDevice(keyword, context) {
    const devices = await getDevices(context.service);
    if (devices && devices.devices.length > 0) {
        for (const device of devices.devices) {
            if (device.name.toLowerCase().includes(keyword.toLowerCase()) ||
                ...
                if (status) {
                    ...
                    await transferPlayback(context.service, device.id, status.is_playing);
                }
...
export async function listAvailableDevices(context) {
    const devices = await getDevices(context.service);
    if (devices && devices.devices.length > 0) {
