...
export async function searchCode(context, input) {
    resetEpoch();
    ...
    if (!chunks.length) {
        return createActionResultFromError("No chunks selected (server access problem?)");
    }
    ...
    if (!wrappedResult.success) {
        return createActionResultFromError(`Failed to get an answer: ${wrappedResult.message}`);
    }
    // 6. Extract answer from result.
    const result = wrappedResult.data;
    const answer = result.answer.trimEnd() + formatReferences(result.references);
    // 6a. Log the answer to a permanent place.
    // Wrong place in the hierarchy, but avoids accidental deletion
    const logFile = path.join(process.env.HOME ?? "", ".spelunker.log");
    ...
    return createActionResultFromMarkdownDisplay(answer, outputEntities, resultEntity);
}
...
function constructPrompt(input, chunks) {
...
async function queryOracle(context, prompt) {
    console_log(`[Step 5: Ask the oracle]`);
    return await context.queryContext.oracle.translate(prompt);
}
function produceEntitiesFromResult(result, allChunks, db) {
    console_log(`  [Success:]`);
    ...
    for (const ref of result.references) {
        ...
        if (!chunk)
            continue;
        const blob = db
            .prepare(`SELECT * FROM Blobs WHERE chunkId = ? ORDER BY start ASC LIMIT 1`)
            .get(ref);
        if (!blob)
            continue;
        const entity = {
...
function createResultEntity(input, answer) {
    return {
...
export async function selectChunks(context, allChunks, input) {
    ...
    const promises = [];
    const maxConcurrency = parseInt(process.env.AZURE_OPENAI_MAX_CONCURRENCY ?? "5") ?? 5;
    ...
    console_log(`  [${batches.length} batches, maxConcurrency ${maxConcurrency}]`);
    for (let i = 0; i < batches.length; i++) {
        const batch = batches[i];
        const p = limiter(() => selectRelevantChunks(context.queryContext.chunkSelector, batch, input, i));
        promises.push(p);
    }
    const allChunkDescs = [];
...
async function selectRelevantChunks(selector, chunks, input, batchIndex) {
    ...
    if (!result) {
        console_log(`  [Failed to select chunks for batch ${batchIndex + 1} with ${chunks.length} chunks]`);
        return [];
    }
    else {
...
export async function loadDatabase(context) {
    console_log(`[Step 1: Load database]`);
    if (!context.queryContext) {
        context.queryContext = createQueryContext();
    }
    const db = context.queryContext.database;
    const prepInsertFiles = db.prepare(`INSERT OR REPLACE INTO Files (fileName, mtime, size) VALUES (?, ?, ?)`);
    const prepSelectAllFiles = db.prepare(`SELECT fileName, mtime, size FROM Files`);
    const prepInsertChunks = db.prepare(`INSERT OR REPLACE INTO Chunks (chunkId, treeName, codeName, parentId, fileName, lineNo) VALUES (?, ?, ?, ?, ?, ?)`);
    const prepInsertBlobs = db.prepare(`INSERT INTO Blobs (chunkId, start, lines, breadcrumb) VALUES (?, ?, ?, ?)`);
    ...
    const filesToDo = [];
    const filesInDb = new Map();
    const fileRows = prepSelectAllFiles.all();
    ...
    const filesSet = new Set(files.map((file) => file.file));
    const filesToDelete = [...filesInDb.keys()].filter((file) => !filesSet.has(file));
    ...
    if (!filesToDo.length) {
        console_log(`  [No files to update out of ${files.length}, yay cache!]`);
        return;
    }
    // 1b. Chunkify all new files (without LLM help).
    // TODO: Make this into its own function.
    // TODO: Numbers may look weird when long files are split by pythonChunker.
    console_log(`[Step 1b: Chunking ${filesToDo.length} out of ${files.length} files]`);
    ...
    const allErrorItems = allItems.filter((item) => "error" in item);
    ...
    const allChunkedFiles = allItems.filter((item) => "chunks" in item);
    const allChunks = [];
    for (const chunkedFile of allChunkedFiles) {
        ...
        if (!file) {
            console_log(`  [*** File ${chunkedFile.fileName} is missing from filesToInsert]`);
            continue;
        }
        prepInsertFiles.run(file.file, file.mtime, file.size);
        for (const chunk of chunkedFile.chunks) {
            allChunks.push(chunk);
            prepInsertChunks.run(chunk.chunkId, chunk.treeName, chunk.codeName, chunk.parentId || null, chunk.fileName, chunk.lineNo);
            for (const blob of chunk.blobs) {
                prepInsertBlobs.run(chunk.chunkId, blob.start, blob.lines.map((line) => line.trimEnd()).join("\n"), blob.breadcrumb);
            }
        }
        db.exec(`COMMIT`);
    }
    console_log(`  [Chunked ${allChunkedFiles.length} files into ${allChunks.length} chunks]`);
    if (!allChunks.length) {
