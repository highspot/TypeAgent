...
export function makeEmbeddingModel() {
...
export async function loadEmbeddings(context, chunks) {
    ...
    const prepInsertEmbeddings = db.prepare(`INSERT OR REPLACE INTO ChunkEmbeddings (chunkId, embedding) VALUES (?, ?)`);
    ...
    for (const batch of batches) {
        const p = limiter(() => generateAndInsertEmbeddings(generateEmbeddingBatch, prepInsertEmbeddings, batch));
        promises.push(p);
    }
    await Promise.all(promises);
}
async function generateAndInsertEmbeddings(generateEmbeddingBatch, prepInsertEmbeddings, batch) {
    const t0 = new Date().getTime();
    const stringBatch = batch.map(blobText);
    const embeddings = await retryOn429(() => generateEmbeddingBatch(stringBatch));
    if (embeddings) {
        for (let i = 0; i < embeddings.length; i++) {
            const chunk = batch[i];
            const embedding = createNormalized(embeddings[i]);
            prepInsertEmbeddings.run(chunk.chunkId, Buffer.from(embedding));
        }
        const t1 = new Date().getTime();
        const dtms = t1 - t0;
        const dtStr = dtms < 1000 ? `${dtms}ms` : `${(dtms / 1000).toFixed(3)}s`;
        console_log(`  [Generated and inserted embedding batch of ${batch.length} in ${dtStr}]`);
    }
    else {
        const t1 = new Date().getTime();
        const dtms = t1 - t0;
...
export async function preSelectChunks(context, input, maxChunks = 1000) {
    ...
    console_log(`  [Embedding input of ${input.length} characters took ${((tb1 - tb0) / 1000).toFixed(3)} seconds${tail}]`);
    if (!queryEmbedding) {
    ...
    const db = context.queryContext.database;
    const prepAllEmbeddings = db.prepare(`SELECT chunkId, embedding FROM ChunkEmbeddings`);
    const allEmbeddingRows = prepAllEmbeddings.all();
    const ta1 = new Date().getTime();
    console_log(`  [Read ${allEmbeddingRows.length} embeddings in ${((ta1 - ta0) / 1000).toFixed(3)} seconds]`);
    ...
    const embeddings = allEmbeddingRows.map((row) => new Float32Array(Buffer.from(row.embedding)));
    ...
    console_log(`  [Found ${chunkIds.length} nearest neighbors in ${((tc1 - tc0) / 1000).toFixed(3)} seconds]`);
    return chunkIds;
}
async function getEmbedding(context, query) {
    const model = context.queryContext.embeddingModel;
    const generateEmbeddingBatch = model.generateEmbeddingBatch;
    ...
    const rawEmbeddings = await retryOn429(() => generateEmbeddingBatch([query]));
