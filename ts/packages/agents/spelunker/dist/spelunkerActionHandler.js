...
class RequestCommandHandler {
    constructor() {
        this.description = "Send a natural language request to the Spelunker";
        this.parameters = {
    ...
    async run(actionContext, params) {
        if (typeof params.args?.question === "string") {
            ...
            if (question.startsWith(".")) {
                ...
                result = await searchCode(actionContext.sessionContext.agentContext, question);
            }
...
export async function initializeSpelunkerContext() {
    return {
        focusFolders: [],
        queryContext: undefined,
    };
}
async function updateSpelunkerContext(enable, context, schemaName) {
    if (enable) {
...
async function saveContext(context) {
    const storage = context.sessionStorage;
    if (storage) {
        await storage.write(spelunkerStorageName, JSON.stringify({ focusFolders: context.agentContext.focusFolders }));
    }
}
async function executeSpelunkerAction(action, context) {
    const result = await handleSpelunkerAction(action, context.sessionContext);
    return result;
}
async function handleSpelunkerAction(action, context) {
    switch (action.actionName) {
        ...
        case "setFocus": {
            ...
            saveContext(context);
            return focusReport(context.agentContext, "Focus cleared", "Focus set to");
        }
        case "getFocus": {
            return focusReport(context.agentContext, "Focus is empty", "Focus is");
        }
        default:
            // Unreachable
...
function handleFocus(sessionContext, question) {
    question = question.trim();
    if (!question.startsWith(".")) {
        throw new Error("handleFocus requires a question starting with '.'");
    }
    const spelunkerContext = sessionContext.agentContext;
    ...
    if (words.length < 2) {
        return focusReport(sessionContext.agentContext, "Focus is empty", "Focus is");
    }
    ...
    return focusReport(sessionContext.agentContext, "Focus cleared", "Focus set to");
}
//# sourceMappingURL=spelunkerActionHandler.js.map