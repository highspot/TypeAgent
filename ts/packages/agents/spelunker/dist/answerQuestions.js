...
function createModelContext() {
    ...
    const miniModel = openai.createChatModel("GPT_4_0_MINI", undefined, undefined, ["spelunkerMini"]);
...
export async function answerQuestion(context, input) {
    ...
    const chunkDescs = await selectChunks(context, allChunks, input);
    ...
    const preppedChunks = chunkDescs.map((chunkDesc) => prepChunk(chunkDesc, allChunks));
    const prompt = `\
...
`;
    ...
    const entities = []; // TODO: Construct from references
    return createActionResultFromMarkdownDisplay(answer, entities);
}
function prepChunk(chunkDesc, allChunks) {
    const chunks = allChunks.filter((chunk) => chunk.id === chunkDesc.chunkid);
    if (chunks.length !== 1)
        return undefined;
    return chunks[0];
}
async function selectChunks(context, chunks, input) {
    ...
    const chunksPerJob = chunks.length / maxConcurrency < 10
        ? 10
    ...
    for (let i = 0; i < chunks.length; i += chunksPerJob) {
        ...
        const p = selectRelevantChunks(context.modelContext.chunkSelector, slice, input);
        promises.push(p);
    }
    const allChunks = [];
...
async function selectRelevantChunks(selector, chunks, input) {
    const prompt = `\
...
function createChunkSelector(model) {
 ...
 * (Written by ChatGPT)
 */
function getAllPyFilesSync(dir) {
    let results = [];
...
function createActionResultFromMarkdownDisplay(markdownText, entities) {
    return {
