...
export async function chunkifyTypeScriptFiles(fileNames) {
    // console_log("========================================================");
    const results = [];
    for (const fileName of fileNames) {
        // console_log(fileName);
        const sourceFile = await tsCode.loadSourceFile(fileName);
        ...
        function recursivelyChunkify(parentNode, parentChunk) {
            const chunks = [];
            for (const childNode of parentNode.getChildren(sourceFile)) {
                if (ts.isInterfaceDeclaration(childNode) ||
                    ...
                    const blobs = makeBlobs(sourceFile, childNode.getFullStart(), childNode.getEnd());
                    const lineNo = blobs.length ? blobs[0].start + 1 : 1;
                    const childChunk = {
                        chunkId: generate_id(),
                        treeName,
...
function spliceBlobs(parentChunk, childChunk) {
    ...
    assert(childBlob.start + childBlob.lines.length <=
        parentBlob.start + parentBlob.lines.length, "Child blob must end before parent blob");
    ...
    const sig = signature(childChunk);
...
function makeBlobs(sourceFile, startPos, endPos) {
...
//# sourceMappingURL=typescriptChunker.js.map