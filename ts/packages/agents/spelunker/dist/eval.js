...
import { loadDatabase, readAllChunksFromDatabase, selectChunks, } from "./searchCode.js";
import { initializeSpelunkerContext, } from "./spelunkerActionHandler.js";
...
function fillSpelunkerContext(context, config) {
...
function readQuestion(context, config) {
    const db = context.queryContext.database;
    const row = db
        .prepare("SELECT question FROM Questions WHERE questionId = ?")
        .get(CONFIG.questionId);
    if (!row) {
        throw new Error(`No question found for questionId ${CONFIG.questionId}`);
    }
    return row.question;
}
async function conductEval(context, config, question) {
    ...
    const prep = db.prepare("SELECT chunkHash FROM Scores WHERE score == 1 AND questionId == ?");
    ...
    const intersection = intersect(selectedHashes, correctHashes);
    ...
    console_log(`precision: ${precision.toFixed(3)}, recall: ${recall.toFixed(3)}, F1: ${F1.toFixed(3)}`);
}
function intersect(a, b) {
    return new Set([...a].filter((x) => b.has(x)));
}
function lookupHashFromChunkId(context, chunkId) {
    const db = context.queryContext.database;
    const row = db
        .prepare("SELECT chunkHash FROM Hashes WHERE chunkId = ?")
        .get(chunkId);
    if (!row) {
