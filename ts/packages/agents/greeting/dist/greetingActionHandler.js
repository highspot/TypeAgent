...
export class GreetingCommandHandler {
    constructor() {
        ...
        this.instructions = `You are a breezy greeting generator. Greetings should NOT end with questions.`;
    }
    async run(context) {
        //
        // Create Model
        ...
        let chatHistory = [];
        const chat = createTypeChat(chatModel, personalizedGreetingSchema, "PersonalizedGreetingAction", this.instructions, chatHistory, maxContextLength, maxWindowLength);
        const response = await this.getTypeChatResponse("Hi!", chat);
        if (response.success) {
            let result = handlePersonalizedGreetingAction(response.data);
    ...
    async getTypeChatResponse(userInput, chat) {
        const chatResponse = await chat.translate(userInput, promptLib.dateTimePrompt());
        return chatResponse;
    }
}
const handlers = {
...
async function executeGreetingAction(action, context) {
    let result = await handleGreetingAction(action, context);
    return result;
}
async function handleGreetingAction(action, context) {
    switch (action.actionName) {
        case "personalizedGreetingAction": {
            return handlePersonalizedGreetingAction(action);
        }
        default:
            throw new Error(`Unknown action: ${action.actionName}`);
    }
}
function handlePersonalizedGreetingAction(greetingAction) {
...
// }
//# sourceMappingURL=greetingActionHandler.js.map