...
import { displayError, displayResult, displayStatus, } from "@typeagent/agent-sdk/helpers/display";
 ...
 */
export class GreetingCommandHandler {
    constructor() {
        this.description = "Have the agent generate a personalized greeting.";
        this.instructions = `You are a breezy greeting generator.`;
    }
    /**
        ...
        if (response.success) {
            ...
            switch (action.actionName) {
                case "personalizedGreetingAction":
                    result = (await handlePersonalizedGreetingAction(action, context));
    ...
    createModel(fastModel = true) {
        let apiSettings;
        if (!apiSettings) {
            if (fastModel) {
                apiSettings = openai.localOpenAIApiSettingsFromEnv(openai.ModelType.Chat, undefined, "GPT_35_TURBO", ["greeting"]);
            }
            else {
        ...
        const chatModel = openai.createChatModel(apiSettings, completionSettings, undefined, ["greeting"]);
        return chatModel;
    }
    async getTypeChatResponse(context) {
        ...
        const chat = createTypeChat(chatModel, personalizedGreetingSchema, //loadSchema(["greetingActionSchema.ts"]),
        "GreetingAction", this.instructions, chatHistory, maxContextLength, maxWindowLength);
        // get chat history
        const history = await getRecentChatHistory(context);
        history.push("Hi!");
        history.push("###");
        history.push(`Current Date is ${new Date().toLocaleDateString("en-US")}. The time is ${new Date().toLocaleTimeString()}.`);
        // make the request
...
async function handlePersonalizedGreetingAction(greetingAction, context) {
    ...
    if (greetingAction.parameters !== undefined) {
        ...
        //     }
        // } else if (index == 0) {
        result = createActionResult(greetingAction.parameters.possibleGreetings[randomInt(0, count)]
            .generatedGreeting);
        // } else {
...
// }
async function getRecentChatHistory(context) {
    const conversationManager = context.sessionContext.conversationManager;
    const chatHistory = [];
    if (conversationManager !== undefined) {
        let searchResponse = await conversationManager.getSearchResponse("What were we talking about last?", [{ terms: ["last conversation"] }]);
        if (searchResponse && searchResponse.response?.hasHits()) {
            ...
            const matches = await conversationManager.generateAnswerForSearchResponse("What were we talking about last?", searchResponse);
            console.log(matches);
        }
    }
    return chatHistory;
}
//# sourceMappingURL=greetingCommandHandler.js.map