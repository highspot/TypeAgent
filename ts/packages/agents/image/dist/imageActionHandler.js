...
async function handlePhotoAction(action, photoContext) {
    ...
    switch (action.actionName) {
        case "findImageAction": {
            ...
            const searchResults = await bing.searchImages(findImageAction.parameters.searchTerm, findImageAction.parameters.numImages);
        ...
        case "createImageAction":
            ...
            const imageCount = createImageAction.parameters.numImages > 5
                ? 5
            ...
            if (images.length == 0) {
                result = createActionResult(`Failed to generate the requested image. ${lastError}`);
            }
            else {
                const urls = [];
                const captions = [];
...
function createCarouselForImages(images, captions) {
    let literal = `There are ${images.length} shown. `;
    const hash = randomBytes(4).readUInt32LE(0).toString();
    const jScript = `
    <script>
    ...
    return createActionResultFromHtmlDisplayWithScript(carousel_start + carousel + carousel_end + jScript, literal);
}
//# sourceMappingURL=imageActionHandler.js.map