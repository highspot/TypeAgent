...
async function executeHighspotContentAction(action, context) {
...
async function updateHighspotContentContext(enable, context) {
    if (enable) {
...
async function handleHighspotContentAction(action, context) {
    ...
    switch (action.actionName) {
        case "getCurrentUser": {
            const response = await getCurrentUser(context.highspot);
        ...
        case "getItem": {
            ...
            if (item_id === undefined) {
                const spotsResponse = await getAllSpots(context.highspot);
                ...
                await Promise.all(spotIds.map(async (spotId) => {
                    ...
                    const response = await getSpotItems(context.highspot, spotId);
                    if (response !== undefined) {
            ...
            const response = await getItem(context.highspot, item_id);
        ...
        case "createSpot": {
            ...
            const response = await createSpot(context.highspot, action.parameters.title);
        ...
        case "getSpotItems": {
            ...
            const response = await getSpotItems(context.highspot, spot_id);
        ...
        case "deleteSpot": {
            ...
            const response = await deleteSpot(context.highspot, spot_id);
        ...
        case "getAllSpots": {
            const response = await getAllSpots(context.highspot);
...
//# sourceMappingURL=highspotContentActionHandler.js.map