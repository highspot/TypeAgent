...
import { getDateRelativeToDayV2, getISODayStartTime, getISODayEndTime, parseFuzzyDateString, } from "common-utils";
export function generateEventReferenceCriteria(eventReference) {
    const criteria = [];
    if (eventReference.day) {
        criteria.push(`start/dateTime ge '${eventReference.day}T00:00:00Z' and start/dateTime lt '${eventReference.day}T23:59:59Z'`);
    }
    if (eventReference.dayRange) {
        let nlpCriteria = generateNaturalLanguageCriteria(eventReference.dayRange);
        if (nlpCriteria)
            criteria.push(nlpCriteria);
    }
    if (eventReference.timeRange) {
        criteria.push(`start/dateTime ge '${eventReference.timeRange.startTime}' and end/dateTime lt '${eventReference.timeRange.endTime}'`);
    }
    if (eventReference.description) {
        criteria.push(`contains(description,'${eventReference.description}')`);
    }
    if (eventReference.location) {
        criteria.push(`contains(location.displayName,'${eventReference.location}')`);
    }
    return criteria.join(" and ");
}
export function getTimeRangeBasedQuery(eventReference) {
    if (eventReference.day) {
...
export function getNextDaysDates(numDays) {
    const currentDate = new Date();
    const startYear = currentDate.getFullYear();
    const startMonth = currentDate.getMonth();
    const startDay = currentDate.getDate();
    const startDateLocal = new Date(startYear, startMonth, startDay, 0, 0, 0, 0);
    const endDateLocal = new Date(startYear, startMonth, startDay + numDays, 23, 59, 59, 999);
    const startDateUTC = new Date(startDateLocal.getTime() - startDateLocal.getTimezoneOffset() * 60000);
...
export function getNWeeksDateRangeISO(nWeeks) {
    const currentDate = new Date();
...
export function generateNaturalLanguageCriteria(input) {
    switch (input.toLowerCase()) {
        case "this week":
            const { startDate: thisWeekStart, endDate: thisWeekEnd } = getCurrentWeekDates();
            return `startdatetime ge '${thisWeekStart.toISOString()}' and enddatetime le '${thisWeekEnd.toISOString()}'`;
        case "this month":
            const { startDate: thisMonthStart, endDate: thisMonthEnd } = getCurrentMonthDates();
            return `startdatetime ge '${thisMonthStart.toISOString()}' and enddatetime le '${thisMonthEnd.toISOString()}'`;
        case "today":
            const { startDate: todayStart, endDate: todayEnd } = getNextDaysDates(0);
            return `startdatetime ge '${todayStart.toISOString()}' and enddatetime le '${todayEnd.toISOString()}'`;
        case "tomorrow":
            const { startDate: tomorrowStart, endDate: tomorrowEnd } = getNextDaysDates(1);
            return `startdatetime ge '${tomorrowStart.toISOString()}' and enddatetime le '${tomorrowEnd.toISOString()}'`;
        default:
            return undefined;
    }
}
export function generateQueryFromFuzzyDay(input) {
    switch (input.toLowerCase()) {
        case "this week":
            const { startDate: thisWeekStart, endDate: thisWeekEnd } = getCurrentWeekDates();
            return `startdatetime=${thisWeekStart.toISOString()}&enddatetime=${thisWeekEnd.toISOString()}`;
        case "next week":
            const { startDateTime: nextWeekStart, endDateTime: nextWeekEnd } = getNWeeksDateRangeISO(1);
        ...
        default:
            const curDate = getDateRelativeToDayV2(input) ?? parseFuzzyDateString(input);
            if (curDate != undefined) {
                return `startdatetime=${getISODayStartTime(curDate)}&enddatetime=${getISODayEndTime(curDate)}`;
            }
            return undefined;
    }
}
//# sourceMappingURL=calendarQueryHelper.js.map