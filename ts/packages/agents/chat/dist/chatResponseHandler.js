...
import { extractEntities, generateAnswerFromWebPages, promptLib, } from "typeagent";
import { bing, openai } from "aiclient";
import { createActionResult, createActionResultFromHtmlDisplay, createActionResultNoDisplay, } from "@typeagent/agent-sdk/helpers/action";
...
function isChatResponseAction(action) {
    return (action.actionName === "generateResponse" ||
        action.actionName === "lookupAndGenerateResponse" ||
        action.actionName === "chatImageResponse");
}
export async function executeChatResponseAction(chatAction, context) {
...
async function rehydrateImages(context, files) {
    let html = "<div>";
...
async function handleChatResponse(chatAction, context) {
    console.log(JSON.stringify(chatAction, undefined, 2));
    switch (chatAction.actionName) {
        case "generateResponse": {
            const generateResponseAction = chatAction;
        ...
        case "lookupAndGenerateResponse": {
            const lookupAction = chatAction;
            if (lookupAction.parameters.internetLookups !== undefined &&
                lookupAction.parameters.internetLookups.length > 0) {
                console.log("Running lookups");
                return handleLookup(lookupAction.parameters.internetLookups, context, await getLookupSettings(true));
            }
            if (lookupAction.parameters.conversationLookupFilters !== undefined) {
                const conversationManager = context.sessionContext.conversationManager;
                if (conversationManager !== undefined) {
                    let searchResponse = await conversationManager.getSearchResponse(lookupAction.parameters.originalRequest, lookupAction.parameters.conversationLookupFilters);
                    if (searchResponse) {
                        searchResponse.response?.hasHits()
                            ? console.log(`Search response has ${searchResponse.response?.messages?.length} hits`)
                            : console.log("No search hits");
                        const matches = await conversationManager.generateAnswerForSearchResponse(lookupAction.parameters.originalRequest, searchResponse);
                        if (matches &&
                            matches.response &&
                            matches.response.answer) {
                            console.log("CONVERSATION MEMORY MATCHES:");
                            conversation.log.logSearchResponse(matches.response);
                            if (lookupAction.parameters
                                .retrieveRelatedFilesFromStorage &&
                                lookupAction.parameters.relatedFiles !==
                                    undefined) {
                                return createActionResultFromHtmlDisplay(`<div>${matches.response.getAnswer()} ${await rehydrateImages(context, lookupAction.parameters.relatedFiles)}</div>`);
                            }
                            else {
                                console.log("Answer: " +
                                    matches.response.answer.answer
                                        ...
                                        .substring(0, 100) +
                                    "...");
                                return createActionResult(matches.response.answer.answer, undefined, matchedEntities(matches.response));
                            }
                        }
                        else {
                            console.log("bug bug");
            ...
            else if (lookupAction.parameters.retrieveRelatedFilesFromStorage &&
                lookupAction.parameters.relatedFiles !== undefined) {
                return createActionResultFromHtmlDisplay(`<div>${await rehydrateImages(context, lookupAction.parameters.relatedFiles)}</div>`);
            }
        }
    }
    return createActionResult("No information found");
}
function matchedEntities(response) {
    const entities = response.getEntities();
    return entities.length > 0
        ? entities.map((e) => compositeEntityToEntity(e))
        : [];
}
function compositeEntityToEntity(entity) {
    return {
        name: entity.name,
        type: [...entity.type, conversation.KnownEntityTypes.Memorized],
    };
}
export function logEntities(label, entities) {
...
function answerToHtml(answer, lookup) {
    let html = "<p><div>";
...
function answersToHtml(context) {
    let html = "";
...
function lookupProgressAsHtml(lookupContext) {
    ...
    for (const [lookup, lookupProgress] of lookupContext.inProgress) {
        if (lookupProgress.url) {
            ...
            const searchUrl = lookupProgress.counter > 1
...
async function getLookupConfig() {
    if (config === undefined) {
        const configFileName = fileURLToPath(new URL("./config.json", import.meta.url));
        const configContent = await fs.promises.readFile(configFileName, "utf8");
        config = JSON.parse(configContent);
    }
    return config.lookup;
}
export async function handleLookup(lookups, context, settings) {
    ...
    const results = await Promise.all(lookups.map((l) => runLookup(l, lookupContext, context, settings, documentConcurrency)));
    if (results.length > 0) {
        ...
        if (hasValidResults) {
            ...
            literalResponse = createActionResult("There was an error searching for information on Bing.\nPlease ensure that:\n- The BING_API_KEY is correctly configured.\n- Bing is available");
        }
    }
    return literalResponse;
}
export async function getLookupSettings(fastMode = true) {
    const lookupConfig = await getLookupConfig();
    ...
    let fastModel = openai.createLocalChatModel(fastModelName, undefined, [
        "chatResponseHandler",
    ]) ??
...
export function getLookupInstructions() {
    return [promptLib.dateTimePromptSection()];
}
function updateActionResult(context, literalResponse) {
...
async function runLookup(lookup, lookupContext, actionContext, settings, concurrency) {
    updateStatus();
    //
    ...
    const answer = await generateAnswerFromWebPages(settings.fastMode ? "Speed" : "Quality", settings.answerGenModel, urls, lookup, settings.lookupOptions, concurrency, getLookupInstructions(), (url, answerSoFar) => {
        if (firstToken) {
...
export async function runEntityExtraction(context, settings) {
    if (!settings.entityGenModel) {
        return [];
    }
    let entityText = "";
 ...
 */
export async function searchWeb(query, maxSearchResults = 3) {
...
function updateLookupProgress(context, lookup, url, answerSoFar) {
...
function streamPartialChatResponseAction(actionName, name, value, delta, context) {
...
//# sourceMappingURL=chatResponseHandler.js.map