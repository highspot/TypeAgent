...
import { loadProgramNameIndex } from "./programNameIndex.js";
...
const autoShellPath = new URL("../../../../../dotnet/autoShell/bin/Debug/autoShell.exe", import.meta.url);
...
async function ensureAutomationProcess(agentContext) {
...
export async function runDesktopActions(action, agentContext, sessionStorage) {
    ...
    switch (actionName) {
            ...
            if (file !== undefined) {
                if (file.startsWith("/") ||
                    ...
                    const files = fs
                        ...
                        .filter((allFilesPaths) => allFilesPaths.endsWith(path.basename(file)));
                    if (files.length > 0) {
                        actionData = path.join(rootTypeAgentDir, files[0]);
                    }
                    else {
                        actionData = file;
                    }
        ...
        case "launchProgram": {
            actionData = await mapInputToAppName(action.parameters.name, agentContext);
        ...
        case "tile": {
            const left = await mapInputToAppName(action.parameters.leftWindow, agentContext);
            const right = await mapInputToAppName(action.parameters.rightWindow, agentContext);
...
async function fetchInstalledApps(desktopProcess) {
    let timeoutHandle;
...
async function finishRefresh(agentContext) {
...
async function mapInputToAppNameFromIndex(input, programNameIndex) {
...
async function mapInputToAppName(input, agentContext) {
    const programNameIndex = agentContext.programNameIndex;
    if (programNameIndex) {
        let matchedNames = await mapInputToAppNameFromIndex(input, programNameIndex);
        if (matchedNames === undefined && (await finishRefresh(agentContext))) {
            matchedNames = await mapInputToAppNameFromIndex(input, programNameIndex);
        }
    }
    return input;
}
const programNameIndexPath = "programNameIndex.json";
async function readProgramNameIndex(storage) {
...
async function ensureProgramNameIndex(agentContext, storage) {
    if (agentContext.programNameIndex === undefined) {
        ...
        agentContext.refreshPromise = refreshInstalledApps(agentContext, programNameIndex, storage);
    }
}
export async function setupDesktopActionContext(agentContext, storage) {
    await ensureAutomationProcess(agentContext);
    return ensureProgramNameIndex(agentContext, storage);
}
export async function disableDesktopActionContext(agentContext) {
...
async function refreshInstalledApps(agentContext, programNameIndex, storage) {
    ...
    await storage?.write(programNameIndexPath, JSON.stringify(programNameIndex.toJSON()));
