...
function getListFilePath() {
    ...
    return path.join(getInstanceSessionsDirPath(), sessions[0], "list", "lists.json");
}
export function setupMiddlewares(middlewares, devServer) {
    ...
    if (!listsFilePath || !fs.existsSync(listsFilePath)) {
        console.error("There is no lists file in the current session. Make sure the Lists application agent is enabled.");
        process.exit(1);
    }
    let listsData = [];
    ...
    function compareListsData(current, updated) {
        const currentNames = current.map((list) => list.name);
        const updatedNames = updated.map((list) => list.name);
        const listsAdded = updatedNames.filter((name) => !currentNames.includes(name));
        const listsRemoved = currentNames.filter((name) => !updatedNames.includes(name));
        const listsEdited = updated
            .filter((updatedList) => {
            const currentList = current.find((list) => list.name === updatedList.name);
            if (!currentList)
                return false;
            const itemsAdded = updatedList.items.filter((item) => !currentList.items.includes(item));
            const itemsRemoved = currentList.items.filter((item) => !updatedList.items.includes(item));
            const itemsEdited = updatedList.items.filter((item) => currentList.items.includes(item) &&
                item !==
                    currentList.items[currentList.items.indexOf(item)]);
            return (itemsAdded.length > 0 ||
