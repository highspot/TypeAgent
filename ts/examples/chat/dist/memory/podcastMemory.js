...
import { arg, argBool, argNum, parseNamedArguments, } from "interactive-app";
import { argClean, argPause, argSourceFileOrFolder, argToDate, manageConversationAlias, } from "./common.js";
import path from "path";
import { asyncArray, createWorkQueueFolder, dateTime, ensureDir, getFileName, isDirectoryPath, isFilePath, removeDir, writeJsonFile, } from "typeagent";
import { runImportQueue } from "./importer.js";
import chalk from "chalk";
import * as kp from "knowpro";
import * as cm from "conversation-memory";
export async function createPodcastMemory(models, storePath, settings, useSqlite = false, useElastic = false, createNew = false) {
    const podcastStorePath = path.join(storePath, ReservedConversationNames.podcasts);
    ...
    if (useElastic) {
        ...
        storageProvider = await sqlite.createStorageDb(podcastStorePath, "podcast.db", createNew);
    }
    const cm = await conversation.createConversationManagerEx({
        model: models.chatModel,
        answerModel: models.answerModel,
    }, settings, ReservedConversationNames.podcasts, podcastStorePath, storageProvider);
...
export function createPodcastCommands(context, commands) {
    ...
    async function importPodcast(args) {
    ...
    async function podcastExport(args) {
        ...
        for await (const threadEntry of threads.entries()) {
            ...
            const messageIds = await messageStore.getIdsInRange(range.startDate, range.stopDate);
            let threadRange = {
                start: {
            ...
            for (let i = 0; i < messageIds.length; ++i) {
                ...
                const podcastMessage = podcastMessageFromEmailText(message.value.value);
                podcastMessage.addTimestamp(message.timestamp.toISOString());
                threadRange.end = {
                    messageIndex: podcastMessages.length,
                };
                podcastMessages.push(podcastMessage);
                knowledgeResponses.push(extractedKnowledgeToResponse(await knowledgeStore.get(messageId)));
            }
            podcastThreads.push({
        ...
        kp.addToConversationIndex(kpPodcast, podcastMessages, knowledgeResponses);
    ...
    async function searchEntities(query, name, exact, count, minScore, showMessages) {
    ...
    async function searchEntities_Multi(name, type, facet, count, faceCount, minScore, showMessages) {
        const index = await context.podcastMemory.conversation.getEntityIndex();
        let nameMatches;
        let typeMatches;
        let facetMatches;
    ...
    async function podcastEntities(args) {
        ...
        if (query) {
            ...
            if (namedArgs.exact || !isMultipart) {
                await searchEntities(query, namedArgs.name !== undefined, namedArgs.exact, namedArgs.count, namedArgs.minScore, namedArgs.showMessages);
            }
            else {
                // Multipart query
                await searchEntities_Multi(namedArgs.name, namedArgs.type, namedArgs.facet, namedArgs.count, namedArgs.facetCount, namedArgs.minScore, namedArgs.showMessages);
            }
            return;
        }
        const index = await context.podcastMemory.conversation.getEntityIndex();
    ...
    async function podcastSearch(args, io) {
        console.log("ABOUT TO SEARCH");
        await searchConversation(context.podcastMemory.searchProcessor, true, parseNamedArguments(args, podcastSearchDef()));
    }
    function podcastConvertDef() {
        return {
    ...
    async function podcastConvert(args) {
    ...
    async function podcastIndex(args) {
        const namedArgs = parseNamedArguments(args, podcastIndexDef());
        let sourcePath = namedArgs.sourcePath;
        let isDir = isDirectoryPath(sourcePath);
        if (isDir) {
            await indexTurns(sourcePath, namedArgs.maxItems ?? Number.MAX_SAFE_INTEGER, namedArgs.pauseMs, namedArgs.clean);
        }
        else {
            context.printer.writeError(`${sourcePath} is not a directory`);
        }
    }
    function podcastAddThreadDef() {
        return {
    ...
    async function podcastAddThread(args) {
        const namedArgs = parseNamedArguments(args, podcastConvertDef());
        const sourcePath = namedArgs.sourcePath;
        const timeRange = conversation.parseTranscriptDuration(namedArgs.startAt, namedArgs.length);
        if (!timeRange) {
            context.printer.writeError("Time range required");
            return;
        }
        const turns = await conversation.loadTurnsFromTranscriptFile(sourcePath);
        const metadata = {
            sourcePath,
            name: namedArgs.name,
    ...
    async function podcastListThreads(args) {
        const threads = await context.podcastMemory.conversation.getThreadIndex();
    ...
    function podcastAddThreadTagDef() {
        return {
    ...
    async function podcastAddThreadTag(args) {
        const namedArgs = parseNamedArguments(args, podcastAddThreadTagDef());
        const threadIndex = await context.podcastMemory.conversation.getThreadIndex();
        ...
        if (thread) {
            ...
            if (tags && tags.length > 0) {
                context.printer.writeLine(`Adding tags to: ${thread.description}\n---`);
    ...
    function podcastRemoveThreadTagDef() {
        return {
    ...
    async function podcastRemoveThreadTag(args) {
        const namedArgs = parseNamedArguments(args, podcastRemoveThreadTagDef());
        const threadIndex = await context.podcastMemory.conversation.getThreadIndex();
        const threadId = namedArgs.threadId;
        const thread = await threadIndex.getById(threadId);
        if (thread) {
            context.printer.writeLine(`Remove tag ${namedArgs.tag} from: ${thread.description}\n---`);
    ...
    async function podcastAlias(args) {
        const namedArgs = parseNamedArguments(args, podcastAliasDef());
        await manageConversationAlias(context.podcastMemory, context.printer, namedArgs.name, namedArgs.alias);
    }
    return;
    //---
    // END Commands
    //--
    async function indexTurns(sourcePath, maxItems, pauseMs, clean) {
        ...
        const queue = await createWorkQueueFolder(path.dirname(sourcePath), path.basename(sourcePath));
        await runImportQueue(queue, getStatsFilePath(), clean ?? false, maxItems, pauseMs ?? 0, context.printer, async (filePath) => {
            const turn = await conversation.loadTranscriptTurn(filePath);
            if (turn) {
                const turnCharsLength = turn.speech.value.length;
                context.printer.writeLine(`${filePath}\n${turnCharsLength} chars`);
                await conversation.addTranscriptTurnsToConversation(context.podcastMemory, turn);
                return turnCharsLength;
            }
            return 0;
        });
    }
    async function importTranscript(sourcePath, startAt, endAt) {
        const turns = await conversation.loadTurnsFromTranscriptFile(sourcePath);
        if (startAt && endAt) {
            conversation.timestampTranscriptTurns(turns, startAt, endAt);
        }
        const transcriptFileName = getFileName(sourcePath);
        await removeDir(path.join(path.dirname(sourcePath), transcriptFileName));
        const turnsFolderPath = getTurnsFolderPath(sourcePath);
        context.printer.writeLine(`Saving ${turns.length} turns to ${turnsFolderPath}`);
        await conversation.saveTranscriptTurnsToFolder(turnsFolderPath, transcriptFileName, turns);
    }
    function getTurnsFolderPath(transcriptFilePath) {
        const transcriptFileName = getFileName(transcriptFilePath);
        const turnsFolderPath = path.join(path.dirname(transcriptFilePath), transcriptFileName, "turns");
        return turnsFolderPath;
    }
    function getStatsFilePath() {
        return path.join(context.statsPath, `${context.podcastMemory.conversationName}_stats.json`);
    }
    function writeThread(t, tags = undefined) {
...
function podcastMessageFromEmailText(text) {
    let messageText = "";
    let speaker;
    ...
    return new cm.PodcastMessage([messageText], new cm.PodcastMessageMeta(speaker));
}
function extractedKnowledgeToResponse(extractedKnowledge) {
    if (extractedKnowledge) {
        const entities = extractedKnowledge.entities?.map((e) => e.value) ?? [];
        const actions = extractedKnowledge.actions?.map((a) => a.value) ?? [];
        const topics = extractedKnowledge.topics?.map((t) => t.value) ?? [];
        return {
            entities,
            actions,
            topics,
...
//# sourceMappingURL=podcastMemory.js.map