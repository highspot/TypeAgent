...
import { generateNotes, dateTime, dotProduct, euclideanLength, normalizeInPlace, writeBlobFile, generateNotesForWebPage, collections, readAllText, dotProductSimple, createSemanticMap, generateEmbedding, asyncArray, } from "typeagent";
...
export async function testEmbedding() {
    ...
    const j2 = dateTime.parseTimestamped(json);
...
export async function testSemanticMap() {
    ...
    const model = openai.createEmbeddingModel();
    const sm = await createSemanticMap(model);
...
export function generateMessageLines(count) {
    let lines = [];
...
export async function summarize() {
    ...
    const result = await generateNotes(text, 2048, openai.createChatModelDefault("chatTests"), (chunk, text) => {
...
export async function testBingSummary() {
    ...
    for (const result of results) {
        let chunkNumber = 0;
        const summary = await generateNotesForWebPage(model, result.url, 1024 * 16, () => {
            ++chunkNumber;
...
export function testCircularArray() {
    const buffer = new collections.CircularArray(4);
    for (let i = 0; i < 10; ++i) {
        buffer.push(i);
    }
...
export function testDotPerf2(size) {
