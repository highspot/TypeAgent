...
import { codeBlockNameFromFilePath, codeBlockNameToString, codeToLines, createCodeGenerator, createCodeReviewer, createDeveloperMemory, tsCode, } from "code-processor";
import { CodePrinter } from "./codePrinter.js";
import { addStandardHandlers, parseNamedArguments, runConsole, } from "interactive-app";
import { argConcurrency, argCount, argDestFile, argMaxMatches, argMinScore, argQuery, argSave, argSourceFile, argVerbose, createTypescriptBlock, getSourcePath, loadCodeChunks, loadTypescriptCode, } from "./common.js";
import { asyncArray, removeDir, writeAllLines } from "typeagent";
import chalk from "chalk";
import { createCommandTransformer } from "./commandTransformer.js";
export async function runCodeMemory() {
    ...
    async function codeDocument(args) {
        ...
        for (let i = 0; i < functions.length; i += concurrency) {
            ...
            printer.writeInColor(chalk.gray, `[Documenting functions ${i + 1} to ${i + slice.length}] / ${functions.length}`);
            const results = await asyncArray.mapAsync(slice, concurrency, (code) => codeReviewer.document({ code, language: "typescript" }));
    ...
    async function codeImport(args) {
        ...
        statements = statements.filter((s) => tsCode.getStatementName(s) !== undefined);
        ...
        for (let i = 0; i < statements.length; i += concurrency) {
            ...
            printer.writeInColor(chalk.gray, `[Indexing statements ${i + 1} to ${i + slice.length}] / ${statements.length}`);
            const importedNames = await asyncArray.mapAsync(slice, concurrency, (statement) => importStatement(sourceFile, statement, fullPath));
    ...
    async function codeSearch(args) {
        ...
        const matches = await developerMemory.searchCode(namedArgs.query, namedArgs.maxMatches, namedArgs.minScore);
    ...
    async function bugs(args) {
        const namedArgs = parseNamedArguments(args, bugsDef());
        if (namedArgs.query) {
            const searchResults = await developerMemory.bugs.search(namedArgs.query, namedArgs.maxMatches, namedArgs.minScore);
            await showSearchResults(developerMemory.bugs, searchResults, (l) => printer.writeBug(l));
        }
        else {
            await showReviews(developerMemory.bugs, (l) => printer.writeBug(l));
        }
    }
    function commentsDef() {
        return {
    ...
    async function comments(args) {
        ...
        if (namedArgs.query) {
            const searchResults = await developerMemory.comments.search(namedArgs.query, namedArgs.maxMatches, namedArgs.minScore);
            await showSearchResults(developerMemory.comments, searchResults, (l) => printer.writeComment(l));
        }
        else {
            await showReviews(developerMemory.comments, (l) => printer.writeComment(l));
        }
    }
    async function cwd() {
    ...
    async function saveGeneratedCode(generatedCode, destFilePath) {
    ...
    async function importStatement(sourceFile, statement, fullPath) {
        ...
        await developerMemory.add(createTypescriptBlock(codeText, fullPath), codeName);
        return codeName;
    }
    async function createMemory() {
        return createDeveloperMemory({ codeReviewer }, memoryFolderPath);
    }
    async function showReviews(index, callback, count = Number.MAX_SAFE_INTEGER) {
        let counter = 0;
    ...
    async function showSearchResults(index, matchIds, callback) {
    ...
    async function printReview(index, entry, callback) {
...
//# sourceMappingURL=codeMemory.js.map