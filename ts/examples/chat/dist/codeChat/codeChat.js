...
import { createCodeReviewer, codeToLines, tsCode, createSemanticCodeIndex, } from "code-processor";
import { CodePrinter } from "./codePrinter.js";
import path from "path";
import { openai } from "aiclient";
import chalk from "chalk";
import { pathToFileURL } from "url";
import { getSourcePath, loadCodeChunks, loadTypescriptCode, sampleFiles, } from "./common.js";
import { createCommandTransformer } from "./commandTransformer.js";
export async function runCodeChat() {
    ...
    async function codeDebug(args) {
        const namedArgs = parseNamedArguments(args, debugDef());
        printer.writeLine(`Source file:\n${namedArgs.sourceFile}`);
        printer.writeLine(`Bug Description:\n${namedArgs.bug}\n`);
        const code = await loadTypescriptCode(namedArgs.sourceFile, namedArgs.moduleDir);
        if (namedArgs.verbose) {
            printer.writeCodeLines(code.sourceText);
        }
        const review = await codeReviewer.debug(namedArgs.bug, code.sourceText, code.modules);
    ...
    async function codeBreakpoints(args) {
        const namedArgs = parseNamedArguments(args, breakpointDef());
        const code = await loadTypescriptCode(namedArgs.sourceFile, namedArgs.moduleDir);
        printer.writeLine(`Source file:\n${namedArgs.sourceFile}`);
        printer.writeLine(`Bug Description:\n${namedArgs.bug}\n`);
        const suggestions = await codeReviewer.breakpoints(namedArgs.bug, code.sourceText, code.modules);
        if (namedArgs.verbose) {
            printer.writeCodeLines(code.sourceText);
        }
    ...
    async function codeAnswer(args) {
        const namedArgs = parseNamedArguments(args, answerDef());
        const question = namedArgs.question ??
        ...
        await asyncArray.mapAsync(functions, 2, (funcCode, index) => codeReviewer.answer(question, funcCode), (funcCode, index, answers) => {
            if (answers.answerStatus !== "NotAnswered" ||
                namedArgs.verbose) {
    ...
    async function codeDocument(args) {
        ...
        const functions = await loadCodeChunks(namedArgs.sourceFile);
        await asyncArray.mapAsync(functions, 2, (code, index) => codeReviewer.document({ code, language: "typescript" }), (code, index, docs) => {
    ...
    async function indexCode(args) {
        ...
        const moduleName = namedArgs.module ?? path.basename(fullPath, path.extname(fullPath));
        const sourceFile = await tsCode.loadSourceFile(fullPath);
        const statements = tsCode.getTopLevelStatements(sourceFile);
        await asyncArray.forEachAsync(statements, 4, async (statement, index) => {
            let name = tsCode.getStatementName(statement);
            if (name) {
                name = `${moduleName}.${name}`;
                let code = tsCode.getTextOfStatement(sourceFile, statement);
                const docs = await codeIndex.put({ code, language: "typescript" }, name, fullPath);
                let text = "";
    ...
    async function findCode(args) {
        const namedArgs = parseNamedArguments(args, findCodeDef());
        const matches = await codeIndex.find(namedArgs.query, namedArgs.maxMatches);
    ...
    async function ensureCodeIndex(createNew = false) {
        if (createNew) {
            await removeDir(codeIndexPath);
        }
        return await createSemanticCodeIndex(codeIndexPath, codeReviewer, vectorModel);
    }
    function printBreakpoints(lines, suggestions) {
    ...
    function printAnswers(lines, answer) {
    ...
    function printCodeBlock(codeBlock) {
        if (codeBlock) {
            ...
            if (codeBlock.sourcePath) {
                printer.writeInColor(chalk.gray, pathToFileURL(codeBlock.sourcePath).toString());
            }
        }
    }
...
//# sourceMappingURL=codeChat.js.map