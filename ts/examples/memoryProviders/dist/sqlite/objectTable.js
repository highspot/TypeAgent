...
import { createFileNameGenerator, FileNameType, generateTicksString, generateTimestampString, } from "typeagent";
export function createObjectTable(db, tableName, settings, ensureExists = true) {
    ...
    const sql_size = db.prepare(`SELECT count(name) as count from ${tableName}`);
    const sql_exists = db.prepare(`SELECT 1 from ${tableName} WHERE name = ?`);
    const sql_get = db.prepare(`SELECT text, blob FROM ${tableName} WHERE name = ?`);
    const sql_getNames = db.prepare(`SELECT name FROM ${tableName}`);
    const sql_writeText = db.prepare(`INSERT OR REPLACE INTO ${tableName} (name, text, blob) VALUES (?, ?, NULL)`);
    const sql_writeBlob = db.prepare(`INSERT OR REPLACE INTO ${tableName} (name, text, blob) VALUES (?, NULL, ?)`);
    ...
    function newest() {
        const sql_getObjects = db.prepare(`SELECT name FROM ${tableName} ORDER BY name DESC`);
        return asyncIterateObjects(sql_getObjects);
    }
    async function* asyncIterateObjects(stmt) {
    ...
    function put(obj, name, onNameAssigned) {
        let objectName;
    ...
    function createNameGenerator() {
        const fileNameType = tableSettings.fileNameType ?? FileNameType.Timestamp;
