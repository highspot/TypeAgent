...
import { createStringTable, createTextIndex, } from "../src/sqlite/textTable.js";
import { composers, countSourceIds, createEmbeddingModel, ensureTestDir, fruits, hasEmbeddingEndpoint, testFilePath, testIf, uniqueSourceIds, } from "./testCore.js";
...
describe("sqlite.textTable", () => {
    ...
    test("getNearest_exact_number", async () => {
        ...
        const table = await createTextIndex({ caseSensitive: false, concurrency: 2, semanticIndex: false }, db, "NameIndex_Exact", "INTEGER", "INTEGER");
        // Add composers
    ...
    test("getNearest_exact_string", async () => {
        const table = await createTextIndex({ caseSensitive: false, concurrency: 2, semanticIndex: false }, db, "NameIndex_Exact_string", "TEXT", "INTEGER");
        ...
        const gotNames = await asyncArray.mapAsync(allTextIds, 1, (id) => table.getText(id));
    ...
    testIf("getNearest", () => hasEmbeddingEndpoint(smallEndpoint), async () => {
        const embeddingModel = createEmbeddingModel(smallEndpoint, 256);
        const table = await createTextIndex({
            caseSensitive: false,
            concurrency: 2,
            semanticIndex: true,
            embeddingModel,
        }, db, "NameIndex", "INTEGER", "INTEGER");
        const composerBlocks = composers();
        await table.putMultiple(composerBlocks);
        ...
        matches = await table.getNearest("Beethoven", composerBlocks.length);
        expect(matches.length).toBeGreaterThan(0);
        expect(matches).toEqual(uniqueIds);
        const hits = knowLib.sets.createHitTable();
        await table.getNearestHits("Beethoven", hits, composerBlocks.length);
        expect(hits.size).toEqual(uniqueIds.length);
        for (const hit of hits.values()) {
            expect(hit.score).toBeGreaterThan(0);
        }
        hits.clear();
        await table.getNearestHitsMultiple(["Beethoven", "Mozart"], hits, composerBlocks.length);
        expect(hits.size).toEqual(uniqueIds.length);
    }, testTimeout);
    test("getNearest_exact_range", async () => {
        // This index does *not* have semantic indexing
        const index = await createTextIndex({ caseSensitive: false, concurrency: 2, semanticIndex: false }, db, "Exact_Timestamp", "INTEGER", "INTEGER");
        const log = await createTemporalLogTable(db, "Exact_Timestamp_Log", "INTEGER", "INTEGER");
        let timestamps = [];
    ...
    function checkIsNew(ids, expected) {
        for (const id of ids) {
            expect(id.isNew).toEqual(expected);
        }
    }
    function checkExists(table, strings, expectedIds) {
    ...
    function compareHitScores(hits, expectedHits) {
    ...
    async function indexBlocks(index, log, blocks, day) {
...
//# sourceMappingURL=sqlite.textTable.spec.js.map