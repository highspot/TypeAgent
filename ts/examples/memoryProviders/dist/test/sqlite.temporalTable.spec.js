...
describe("sqlite.temporalTable", () => {
    ...
    test("addIds", async () => {
        const table = createTemporalLogTable(db, "idLog", "INTEGER", "TEXT");
        ...
        for (let i = 1; i <= addCount; ++i) {
            ...
            let ids = blocks.map((b) => table.addSync(b.sourceIds, timestamp));
        ...
        expect(latest[0].timestamp).toEqual(timestamps[timestamps.length - 1]);
        ...
        const entries = await table.getEntriesInRange(timestamps[iStart], timestamps[iEnd]);
        expect(entries).toHaveLength((iEnd - iStart + 1) * blocks.length);
    }, testTimeout);
    test("addIds_number", async () => {
        const table = createTemporalLogTable(db, "idLog_number", "INTEGER", "INTEGER");
        let ids = [1, 2, 3, 4, 5];
...
//# sourceMappingURL=sqlite.temporalTable.spec.js.map