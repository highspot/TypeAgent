...
async function runPlayground() {
    ...
    //
    let rawChatMode = false;
    let streamChatMode = false;
     ...
     * @param io
     */
    async function nextChatTurn(userInput, io) {
        if (rawChatMode) {
            if (streamChatMode) {
    ...
    async function getTypeChatResponse(userInput, io) {
        stopWatch.start();
    ...
    async function getRawResponse(userInput, io) {
        const userMessage = {
    ...
    async function getStreamResponse(userInput, io) {
        const userMessage = {
            role: MessageSourceRole.user,
            content: userInput,
        };
    ...
    async function saveHistory(args, io) {
    ...
    async function loadHistory(args, io) {
        const json = await fs.promises.readFile(getArg(args, 0), "utf-8");
        const loadedHistory = json && json.length > 0 ? JSON.parse(json) : [];
