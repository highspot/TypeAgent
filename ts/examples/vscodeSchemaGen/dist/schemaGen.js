...
import { dedupeList, generateEmbeddingWithRetry } from "typeagent";
const envPath = new URL("../../../.env", import.meta.url);
dotenv.config({ path: envPath });
async function getModelCompletionResponse(chatModel, prompt, jsonNode) {
...
async function writeSchemaEmbeddingsDataToFile(schemaData, outputPath) {
...
export async function createVSCODESchemaGen(model, jsonSchema) {
    console.log("Generating VSCODE schema...");
    model.complete("Generate VSCODE schema").then((response) => {
        if (response.choices) {
...
async function genActionSchemaForNode(jsonNode, verbose = false) {
...
function parseTypeComponents(schema) {
    return {
...
export async function genEmbeddingDataFromActionSchema(
    ...
    if (fs.existsSync(schemaFilePath)) {
        ...
        for (const schemaStr of schemaDefinitions) {
            let actionSchemaData = parseTypeComponents(schemaStr);
            const actionString = `${actionSchemaData.typeName} ${actionSchemaData.actionName} ${actionSchemaData.comments.join(" ")}`;
            let actionEmbedding = await generateEmbeddingWithRetry(
                embeddingModel,
            ...
            let actionRequests = await generateActionRequests(
                "variations",
                model,
                typeSchema,
...
export async function processVscodeCommandsJsonFile(
    ...
    for (const node of jsonData) {
        try {
            ...
            const schemaStr = await genActionSchemaForNode(node, verbose);
            processedNodeCount++;
