...
export async function normalizeCommandsandKBJson() {
    ...
    async function convertTxtToJSON(filePath) {
    ...
    async function mergeKBNCommandJsonFiles(
        ...
        const secondMap = new Map(
            secondArray.map((item) => [item.command, item]),
        );
        const mergedArray = [];
        let countOfNodesNotInKbFile = 0;
        firstArray.forEach((item) => {
            const matchingItem = secondMap.get(item.id);
        ...
        secondMap.forEach((item) => {
            const mergedItem = { id: item.command };
            if (item.when) {
...
//# sourceMappingURL=normalizeVscodeJson.js.map
