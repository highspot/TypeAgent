...
import { dispatchCommand, getArg, parseCommandLine, parseNamedArguments, runConsole, } from "interactive-app";
import { getAbsolutePath, generateVariationsRecursive, readAllLines, writeAllLines, dedupeList, dedupeLineFile, } from "typeagent";
...
async function runStudio() {
    const playerSchemaFile = "../../../packages/player/src/agent/playerSchema.ts";
    ...
    async function temperature(args, io) {
        if (args.length === 0) {
            io.writer.writeLine(studio.model.completionSettings.temperature ?? 0);
        }
        else {
    ...
    async function dedupeFile_(args, io) {
        const filePath = getArg(args, 0);
        await dedupeLineFile(filePath);
    }
    function fromSchemaArgs() {
        return {
            description: "Generate phrases that can translate to an action schema",
            options: {
                action: {
                ...
                type: {
                    description: "Type of variation. E.g. likely, unlikely, similar, alternatives, etc",
     ...
     * @returns
     */
    async function fromSchema(args, io) {
        ...
        if (examplePath) {
            ...
            for (const example of lines) {
                let exampleList = await generateActionPhrases(namedArgs.type, studio.model, schema, namedArgs.description, namedArgs.count, namedArgs.facets, example, namedArgs.language);
                if (existingData) {
            ...
            list = await generateActionPhrases(namedArgs.type, studio.model, schema, namedArgs.description, namedArgs.count, namedArgs.facets, example, namedArgs.language);
            if (existingData) {
     ...
     * @param io
     */
    async function variations(args, io) {
        const phrase = args[0];
        args.shift();
        ...
        if (typeName) {
            variationOptions.schema = loadActionSchema(playerSchemaPath, typeName);
        }
        // Facets are any remaining options
        let list = await generateVariationsRecursive(studio.model, phrase, variationOptions, variationOptions.depth, showProgress
            ? (s, l) => generationProgress(s, l, io)
            : undefined);
        list.sort();
        io.writer.writeList(list);
    }
    function getVariationOptions(args) {
        const variationOptions = {
            type: args.type ?? "most likely",
    ...
    function generationProgress(seedPhrase, list, io) {
    ...
    function removeExisting(items, existing) {
        return items.filter((v) => !existing.has(v.toLocaleLowerCase()));
    }
    function getUnique(items, caseSensitive = false) {
...
function getActionSchema(filePath) {
