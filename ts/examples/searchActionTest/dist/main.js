 ...
 * @returns
 */
export function createChatTranslator(model, schema, typeName, preamble = "") {
...
function createModel(preferLocal) {
    // First see if there is a local model
    let apiSettings;
    if (preferLocal) {
        apiSettings = openai.localOpenAIApiSettingsFromEnv(openai.ModelType.Chat, undefined, undefined, ["action tester"]);
    }
    if (!apiSettings) {
        // Create default model
        apiSettings = openai.apiSettingsFromEnv(openai.ModelType.Chat, undefined, "GPT_4_O");
    }
...
function convertCalDateTime(calDateTime, isStart = true) {
    ...
    if (calDateTime.month !== undefined) {
        ...
        if (calDateTime.day !== undefined) {
            if (calDateTime.day === "StartOfMonth" ||
        ...
        if (calDateTime.week === "0") {
            ...
            if (calDateTime.day !== undefined) {
                accum += ".";
                if (calDateTime.day === "StartOfWeek" ||
    ...
    else if (calDateTime.day !== undefined &&
        calDateTime.day !== "0" &&
        calDateTime.day !== "Now" &&
        calDateTime.day.toLocaleLowerCase() !== "today") {
        accum += ".";
...
export function refineTest(testoutfile) {
    const testJSON = fs.readFileSync(new URL(testoutfile, import.meta.url), "utf8");
    ...
    for (let i = 0; i < testData.length; ++i) {
        ...
        const state = getState(chatAction.parameters.start, chatAction.parameters.end);
        if (state !== undefined) {
            syntheticResult.state = state;
            // console.log(state);
        }
        else if (chatAction.parameters.start) {
            const outdtr = getDateTimeRange(chatAction.parameters.start, chatAction.parameters.end);
...
export function createTabSeparatedOuput(testoutfile) {
    const testJSON = fs.readFileSync(new URL(testoutfile, import.meta.url), "utf8");
    const testData = JSON.parse(testJSON);
...
function createSyntheticLabel(syntheticResult) {
...
export async function runTest() {
    ...
    const chatTranslator = createChatTranslator(chatModel, schemaText, "CalendarSearchAction", preamble);
    // read JSON from test file
    const testJSON = fs.readFileSync(new URL(testfile, import.meta.url), "utf8");
