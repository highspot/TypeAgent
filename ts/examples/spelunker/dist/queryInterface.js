...
function writeColor(io, color, message) {
...
function writeNote(io, message) {
    writeColor(io, chalk.gray, message);
}
function writeMain(io, message) {
...
function writeHeading(io, message) {
    writeColor(io, chalk.green, message);
}
export async function interactiveQueryLoop(chunkyIndex, verbose = false) {
    const handlers = {
    ...
    function importDef() {
        return {
            ...
            options: {
                fileName: {
                    description: "File to import (or multiple files separated by commas)",
                    type: "string",
                },
                files: {
    ...
    async function importHandler(args, io) {
        const namedArgs = iapp.parseNamedArguments(args, importDef());
        const files = namedArgs.fileName
            ? namedArgs.fileName.trim().split(",")
            : namedArgs.files
                ? fs
                    .readFileSync(namedArgs.files, "utf-8")
                    .split("\n")
                    .map((line) => line.trim())
    ...
    async function clearMemory(args, io) {
                    ...
                    for (const [name, _] of chunkyIndex.allIndexes()) {
                        if (name == "summaries") {
                            if (chunkDoc.summary) {
                                ...
                                writeMain(io, 
                                // Indent by two
                                wordWrap(chunkDoc.summary).replace(/^/gm, "  "));
                            }
                            else {
                            ...
                            const docItem = chunkDoc[name];
                            if (docItem?.length) {
                                writeNote(io, `${name.toUpperCase()}: ${docItem.join(", ")}`);
                            }
                        }
                    }
                }
    ...
    function commonOptions() {
        return {
            ...
            maxHits: {
                description: "Maximum number of hits to return (only for query)",
                type: "integer",
                defaultValue: 10,
            },
            minScore: {
    ...
    async function summaries(args, io) {
        await _reportIndex(args, io, "summaries");
    }
    function keywordsDef() {
        return {
    ...
    async function keywords(args, io) {
    ...
    async function tags(args, io) {
        await _reportIndex(args, io, "tags");
    }
    function synonymsDef() {
        return {
    ...
    async function synonyms(args, io) {
        await _reportIndex(args, io, "synonyms");
    }
    function dependenciesDef() {
        return {
    ...
    async function dependencies(args, io) {
    ...
    async function files(args, io) {
        ...
        if (!filesPopularity.size) {
            ...
            const sortedFiles = Array.from(filesPopularity)
                ...
                .sort();
            writeNote(io, `Found ${sortedFiles.length} ${filter ? "matching" : "total"} files.`);
            for (const [file, count] of sortedFiles) {
                writeMain(io, `${chalk.blue(count.toFixed(0).padStart(7))} ${chalk.green(file)}`);
            }
        }
    }
    function purgeFileDef() {
        return {
    ...
    async function purgeFile(args, io) {
        const namedArgs = iapp.parseNamedArguments(args, purgeFileDef());
        const file = namedArgs.fileName;
    ...
    async function _reportIndex(args, io, indexName) {
        ...
        let matches = [];
        if (namedArgs.query) {
            matches = await index.nearestNeighborsPairs(namedArgs.query, namedArgs.maxHits, namedArgs.minScore);
        }
        else {
        ...
        if (!namedArgs.filter) {
            ...
            const filterWords = namedArgs.filter
                .split(" ")
                .filter(Boolean)
                .map((w) => w.toLowerCase());
            for await (const match of matches) {
                const valueWords = match.item.value
                    .split(/\s+/)
                    .filter(Boolean)
                    .map((w) => w.toLowerCase());
        ...
        if (!hits.length) {
            ...
            for (const hit of hits) {
                const numSourceIds = hit.item.sourceIds?.length ?? 0;
            ...
            for (const hit of hits) {
                writeMain(io, `${hit.score.toFixed(3).padStart(7)}: ${chalk.green(hit.item.value)} :: ${(hit.item.sourceIds ?? []).join(", ")}`);
            }
        }
    }
    async function _debugIndex(io, index, indexName, verbose) {
        const allTexts = Array.from(index.text());
        for (const text of allTexts) {
            if (verbose)
                writeNote(io, `Text: ${text}`);
            const hits = await index.nearestNeighborsPairs(text, allTexts.length);
            if (verbose) {
                for (const hit of hits) {
                    writeNote(io, `${hit.score.toFixed(3).padStart(7)} ${hit.item.value}`);
                }
            }
            if (hits.length < 2) {
                ...
                writeMain(io, `${chalk.green(hits[0].item.value)}: ${chalk.blue(hits[1].item.value)} (${hits[1].score.toFixed(3)}) -- ` +
                    `${chalk.blue(hits[end].item.value)} (${hits[end].score.toFixed(3)})`);
            }
        }
    }
    async function _inputHandler(input, io) {
...
export async function purgeNormalizedFile(io, chunkyIndex, fileName, verbose) {
    ...
    writeNote(io, `[Need to purge ${toDelete.size} chunks for file ${fileName}]`);
    // Step 2: Remove chunk ids from indexes.
    const chunkIdsToDelete = Array.from(toDelete);
...
async function processQuery(input, chunkyIndex, io, queryOptions) {
    // ** Step 0:** Find most recent answers.
    const recentAnswers = await findRecentAnswers(input, chunkyIndex);
    // **Step 1:** Ask LLM (queryMaker) to propose queries for each index.
    const proposedQueries = await proposeQueries(input, recentAnswers, chunkyIndex, io, queryOptions);
    if (!proposedQueries)
        return; // Error message already printed by proposeQueries.
    ...
    if ("answer" in proposedQueries) {
        ...
        reportQuery(undefined, undefined, proposedQueries.answer, io, queryOptions.verbose);
        writeNote(io, "[Answer produced by stage 1]");
        return;
    }
    // **Step 2:** Run those queries on the indexes.
    const [hitsByIndex, chunkIdScores] = await runIndexQueries(proposedQueries, chunkyIndex, io, queryOptions);
    if (!chunkIdScores)
        return; // Error message already printed by runIndexQueries.
    // **Step 3:** Ask the LLM (answerMaker) to answer the question.
    const answer = await generateAnswer(input, hitsByIndex, chunkIdScores, recentAnswers, chunkyIndex, io, queryOptions);
    if (!answer)
        return; // Error message already printed by generateAnswer.
...
async function proposeQueries(input, recentAnswers, chunkyIndex, io, queryOptions) {
    const t0 = Date.now();
    ...
    const result = await chunkyIndex.queryMaker.translate(input, promptPreamble);
    if (!result.success) {
        const t1 = Date.now();
    ...
    if (queryOptions.verbose) {
        ...
        io.writer.writeLine(util.inspect(specs, { depth: null, colors: true, compact: false }));
    }
    const t1 = Date.now();
    writeNote(io, `[proposeQueries took ${((t1 - t0) * 0.001).toFixed(3)} seconds]`);
    return specs;
}
async function runIndexQueries(proposedQueries, chunkyIndex, io, queryOptions) {
    const t0 = Date.now();
    const hitsByIndex = new Map();
    const chunkIdScores = new Map(); // Record score of each chunk id.
    ...
    for (const [indexName, index] of chunkyIndex.allIndexes()) {
        const spec = proposedQueries[indexName];
        if (spec === undefined) {
        ...
        const maxHitsDisplay = maxHits === specMaxHits
            ? maxHits.toString()
            : `${specMaxHits} ?? ${defaultMaxHits}`;
        const hits = await index.nearestNeighborsPairs(spec.query, maxHits, queryOptions.minScore);
        if (!hits.length) {
            writeNote(io, `[${indexName}: query ${spec.query} (maxHits ${maxHitsDisplay}) no hits]`);
            continue;
        }
        ...
        for (const hit of hits) {
            ...
            const fraction = totalNumChunks / (1 + (hit.item.sourceIds?.length ?? 0));
        ...
        if (queryOptions.verbose) {
            ...
            for (const hit of hits) {
                writeNote(io, `${hit.score.toFixed(3).padStart(7)}: ${hit.item.value} -- ${hit.item.sourceIds?.join(", ")}`);
            }
        }
        // Regular logging.
        const numChunks = new Set(hits.flatMap((h) => h.item.sourceIds)).size;
        const end = hits.length - 1;
        writeNote(io, `[${indexName}: query '${spec.query}' (maxHits ${maxHitsDisplay}); ${hits.length} hits; ` +
...
async function generateAnswer(input, hitsByIndex, chunkIdScores, recentAnswers, chunkyIndex, io, queryOptions) {
    const t0 = Date.now();
    // Step 3a: Compute array of ids sorted by score, truncated to some limit.
    const scoredChunkIds = Array.from(chunkIdScores.values());
    ...
    for (const scoredChunkId of scoredChunkIds) {
        const maybeChunk = await chunkyIndex.chunkFolder.get(scoredChunkId.item);
        if (maybeChunk) {
            chunks.push(maybeChunk);
            if (queryOptions.verbose) {
                writeVerboseReferences(io, [scoredChunkId.item], hitsByIndex, chunkIdScores);
            }
    ...
    const answerResult = await chunkyIndex.answerMaker.translate(input, preamble);
    if (!answerResult.success) {
        writeError(io, `[Error: ${answerResult.message}]`);
        return undefined;
    }
    if (queryOptions.verbose) {
        writeNote(io, `AnswerResult: ${JSON.stringify(answerResult.data, null, 2)}`);
    }
    const t1 = Date.now();
    writeNote(io, `[generateAnswer took ${((t1 - t0) * 0.001).toFixed(3)} seconds]`);
    return answerResult.data;
}
async function findRecentAnswers(input, chunkyIndex) {
...
function reportQuery(hitsByIndex, chunkIdScores, answer, io, verbose) {
    ...
    if (answer.references.length) {
        ...
        if (verbose && chunkIdScores && hitsByIndex) {
            writeVerboseReferences(io, answer.references, hitsByIndex, chunkIdScores);
        }
    }
    // NOTE: If the user wants to see the contents of any chunk, they can use the @chunk command.
}
function writeVerboseReferences(io, references, hitsByIndex, chunkIdScores) {
        ...
        for (const indexName of hitsByIndex.keys()) {
            ...
            for (const hit of hits) {
                if (hit.item.sourceIds?.includes(ref)) {
                    writeColor(io, chalk.green, `    ${chalk.gray(indexName)}: ${hit.item.value} (${chalk.white(hit.score.toFixed(3))})`);
                }
            }
        }
    }
}
function makeQueryMakerPrompt(recentAnswers) {
    const prompt = `
...
function makeAnswerPrompt(recentAnswers, chunks) {
    const prompt = `\
...
function makeAnyPrompt(recentAnswers, prompt) {
    const preamble = [];
...
function writeChunkLines(chunk, io, lineBudget = 10) {
    ...
    outer: for (const blob of chunk.blobs) {
        for (let i = 0; i < blob.lines.length; i++) {
            ...
            writeMain(io, `${(1 + blob.start + i).toString().padStart(6)}: ${blob.lines[i].trimEnd()}`);
        }
    }
}
// Wrap long lines.
export function wordWrap(text, wrapLength = 80) {
    let inCodeBlock = false;
    const lines = [];
    const prefixRegex = /^\s*((-|\*|\d+\.)\s+)?/;
    for (let line of text.split(/[ ]*\r?\n/)) {
        if (line.startsWith("```"))
            inCodeBlock = !inCodeBlock; // TODO: Colorize code blocks.
        ...
        while (line.length > wrapLength) {
            ...
            if (index <= prefixLength) {
                ...
                if (index < 0)
                    break; // The rest of the line is one "word".
            }
