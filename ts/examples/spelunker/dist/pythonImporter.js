...
export async function importAllFiles(files, chunkyIndex, io, verbose) {
    log(io, `[Importing ${files.length} files]`, chalk.grey);
    const t0 = Date.now();
    await importPythonFiles(files, chunkyIndex, io, verbose);
    const t1 = Date.now();
    log(io, `[Imported ${files.length} files in ${((t1 - t0) * 0.001).toFixed(3)} seconds]`, chalk.grey);
}
async function importPythonFiles(files, chunkyIndex, io, verbose = false) {
    ...
    if (results.length !== filenames.length) {
        log(io, `[Some over-long files were split into multiple partial files]`, chalk.yellow);
    }
    ...
    log(io, `[Chunked ${results.length} files ` +
        `(${numLines} lines, ${numBlobs} blobs, ${numChunks} chunks, ${numErrors} errors) ` +
        `in ${((t1 - t0) * 0.001).toFixed(3)} seconds]`, chalk.gray);
    const chunkingErrors = results.filter((result) => "error" in result);
    for (const error of chunkingErrors) {
        log(io, `[Error: ${error.error}; Output: ${error.output ?? ""}]`, chalk.redBright);
    }
    const chunkedFiles = results.filter((result) => "chunks" in result);
    ...
    let nChunks = 0;
    await asyncArray.forEachAsync(chunkedFiles, concurrency, async (chunkedFile) => {
        const t0 = Date.now();
        let docs;
        nChunks += chunkedFile.chunks.length;
        try {
            docs = await exponentialBackoff(io, chunkyIndex.fileDocumenter.document, chunkedFile.chunks);
        }
        catch (error) {
            const t1 = Date.now();
            log(io, `  [Error documenting ${chunkedFile.fileName} in ${((t1 - t0) * 0.001).toFixed(3)} seconds: ${error}]`, chalk.redBright);
            return;
        }
        const t1 = Date.now();
        if (verbose) {
            log(io, `  [Documented ${chunkedFile.chunks.length} chunks in ${((t1 - t0) * 0.001).toFixed(3)} seconds for ${chunkedFile.fileName}]`, chalk.grey);
        }
        documentedFiles.push(docs);
    });
    const tt1 = Date.now();
    log(io, `[Documented ${documentedFiles.length} files (${nChunks} chunks) in ${((tt1 - tt0) * 0.001).toFixed(3)} seconds]`, chalk.grey);
    const nonEmptyFiles = chunkedFiles.filter((cf) => cf.chunks.filter((c) => c.docs).length);
    ...
    if (nonEmptyFiles.length) {
        ...
        const ttt1 = Date.now();
        log(io, `[Embedded ${documentedFiles.length} files in ${((ttt1 - ttt0) * 0.001).toFixed(3)} seconds]`, chalk.grey);
    }
}
export async function embedChunkedFile(chunkedFile, chunkyIndex, io, verbose = false) {
    ...
    const t1 = Date.now();
    if (verbose) {
        log(io, `  [Embedded ${chunks.length} chunks in ${((t1 - t0) * 0.001).toFixed(3)} seconds for ${chunkedFile.fileName}]`, chalk.grey);
    }
}
async function embedChunk(chunk, chunkyIndex, io, verbose = false) {
    const t0 = Date.now();
    const lineCount = chunk.blobs.reduce((acc, blob) => acc + blob.lines.length, 0);
    ...
    const t1 = Date.now();
    if (verbose) {
        log(io, `  [Embedded ${chunk.id} (${lineCount} lines @ ${chunk.blobs[0].start + 1}) ` +
            `in ${((t1 - t0) * 0.001).toFixed(3)} seconds for ${chunk.fileName}]`, chalk.gray);
    }
}
async function writeToIndex(io, chunkId, phrases, // List of summaries, keywords, tags, etc. in chunk
index) {
    for (const phrase of phrases ?? []) {
        await exponentialBackoff(io, index.put, phrase, [chunkId]);
    }
}
async function exponentialBackoff(io, callable, ...args) {
    let timeout = 1;
    for (;;) {
        try {
            ...
            log(io, `[Error: ${error}; retrying in ${timeout} ms]`, chalk.redBright);
...
//# sourceMappingURL=pythonImporter.js.map