...
export async function chunkifyPythonFiles(filenames) {
    let output, errors, success = false;
    try {
        const chunkerPath = path.join(__dirname, "chunker.py");
        let { stdout, stderr } = await execPromise(`python3 -X utf8 ${chunkerPath} ${filenames.join(" ")}`, { maxBuffer: 64 * 1024 * 1024 });
        output = stdout;
        errors = stderr;
        success = true;
    }
...
function splitLargeFiles(items) {
    const results = [];
...
function splitFile(file) {
    ...
    const results = []; // Where output accumulates.
    ...
    outer: while (true) {
        ...
        for (let i = 0; i < chunks.length; i++) {
            ...
            if (rest.shift() !== currentChunk)
                throw Error("Internal error: expected current chunk at head of rest");
...
//# sourceMappingURL=pythonChunker.js.map